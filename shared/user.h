...
public:
	...
	DropItemData				sInventoryItems[316];		//0x5594
	...
	char						szEmpty06[0x08];
	CCharacterSaveData *		pcCharacterSaveData;		//0x6E10
	...
	DWORD						dwLastTradeChecksum;		//0x6E74
	DWORD						dwLastTradeTime;			//0x6E78
	short						saLowLevelPresentItem;		//0x6E7C
	DWORD						dwStartUserPresentItemTime;	//0x6E80
	DWORD						dwStartUserPresentItemCount;//0x6E84

	int							iSendAttackCount;			//0x6E88
	int							iSendAttackDamage;			//0x6E8C
	int							iReceiveAttackCount;		//0x6E90
	int							iReceiveAttackDamage;		//0x6E94

	DWORD						dwMacroTime;				//0x6E98
	int							iMacroPositionX;			//0x6E9C
	int							iMacroPositionZ;			//0x6EA0
	int							iMacroAngleY;				//0x6EA4
	DWORD						dwMacroAngleTime;			//0x6EA8
	DWORD						dwMacroAngleCheckTime;		//0x6EAC
	...
	BOOL						bBlock;						//0xAEB4
	int							iBadPlayer;					//0xAEB8 - Not Used
	int							iPacketCounter;				//0xAEBC
	DWORD						dwTickPacketCounter;		//0xAEC0
	char						szEmpty07aA[0x04];
	PacketCharacterRecordData * psCharacterFile;			//0xAEC8
	...
	int							iaServerPotion[3][4];		//0xAFFC

	char						szEmpty09[0x70];			//0xB02C

	char						szLoginCode[16];			//0xB09C

	char						szEmpty09a[0x224];			//0xB0AC

	MinMax						sDamageLimit;				//0xB2D0
	MinMax						sCriticalLimit;				//0xB2D4

	char						szEmpty09b[0x30];
	...
	DWORD						dwCurseDefenceTime; // 0xB400
	DWORD						dwCurseDefenceParam; // 0xB404
...
public:
	...
	std::vector<PersonalShopOffline>		vPersonalShopItems;

	UINT									uAntiCheat;
	UINT									uTimeAntiCheat;

	UINT									uSavePacketCounter;
...
public:
	...
	int										GetCharacterLevel() { return pcUserData ? pcUserData->sCharacterData.iLevel : 0; };
	int										GetCharacterClanID() { return pcUserData ? pcUserData->sCharacterData.iClanID : -1; };
	ECharacterClass							GetCharacterClass() { return pcUserData ? pcUserData->sCharacterData.iClass : ECharacterClass::CHARACTERCLASS_None; };

	Point3D									GetPosition() { return pcUserData ? pcUserData->sPosition : Point3D( 0, 0, 0 ); }
	Point3D									GetAngle() { return pcUserData ? pcUserData->sAngle : Point3D( 0, 0, 0 ); }
