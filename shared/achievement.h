...
struct ValueTaskAchievement
{
	...
	int					GetInt() const { return atoi( szValue ); }
	float				GetFloat() const { return (float)atof( szValue ); }
	const char * GetString() const { return szValue; }
};
...
public:
	...
	const char			* GetValueString( int iIndex );
