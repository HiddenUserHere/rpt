...
BOOL UnitData::AttackFromPosition( int iX, int iY, int iZ, int iRange, BOOL bAttack )
{
	int iAngleY = abs( sPosition.iY - iY );

	/*FIXME! GameStuff*/
	/*if ( iLoadedMapIndex >= 0 && StageField[iLoadedMapIndex]->FieldCode == rsCASTLE_FIELD )
	{
		...
		if ( ((sPosition.iZ<CastleBattleZone_DoorLineZ) && (iZ>CastleBattleZone_DoorLineZ)) ||
			(sPosition.iZ > CastleBattleZone_DoorLineZ && (iZ < CastleBattleZone_DoorLineZ)) )
		{
	...
	int iXComp = (sPosition.iX - iX) >> 8;
	int iZComp = (sPosition.iZ - iZ) >> 8;

	int iDistance = (iXComp * iXComp) + (iZComp * iZComp);

	int iDistanceRange = (iRange >> 8);
	iDistanceRange *= iDistanceRange;

	if ( iDistance < iDistanceRange )
	{
		if ( GetAnimation() != ANIMATIONTYPE_Attack &&
			GetAnimation() != ANIMATIONTYPE_Skill && GetAnimation() != ANIMATIONTYPE_Die )
		{

			if ( bAttack )
			{
				//FIXME! GAME STUFF
				if ( (sCharacterData.iType != CHARACTERTYPE_Player) /* || (iLoadedMapIndex >= 0 && StageField[iLoadedMapIndex]->State != FIELD_STATE_VILLAGE) */)
				{
					Animate( ANIMATIONTYPE_Attack );
					sAngle.iY = GetRadian2D( sPosition.iX, sPosition.iZ, iX, iZ );
					sAttackPosition.iX = iX;
					sAttackPosition.iY = iY;
					sAttackPosition.iZ = iZ;
				}
			}
...
NAKED void UnitData::SetIceFootstep( int iType, DWORD dwTimeOut, int iX, int iZ )
{
	JMP( pfnSetIceFootstep );
}

NAKED BOOL UnitData::HandleAttackEvent( BOOL bEvent )
{
	JMP( pfnHandleEventAttack );
}
...
BOOL UnitData::Move( int iDistance )
{
	...
	auto UpdateDenyArea = [this, &sPositionSafeOld, &pcaBaseMap]()-> void
	{
		auto IsInArea = [this]( Point3D sDenyPosition, int iRange ) -> bool
		{
			...
			const Point2D & p = Point2D( sDenyPosition.iX, sDenyPosition.iZ );
