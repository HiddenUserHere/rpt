...
const bool IOCP_USE_HOSTNAME_INFO = false;
...
FNetConnectionInfo CIocpUtils::GetConnectionInfo( SOCKET _s )
{
	FNetConnectionInfo sConnInfo;
	sConnInfo.iPort = 0;
...
//				cIocpData.cAcceptBase.sSocket = CreateOverlappedSocket();
...
int CIocpUtils::PostReceive( CIocpBase & cBase )
{
	...
	//WRITEERR( "%s", cBase.sWSABuffer.buf );
