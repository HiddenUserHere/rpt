...
void Debug::WriteLine( const char * file, int line, const char * msg, ... )
{
	pcMutex->Lock();
	...
	static char szFormattedMessage[_countof( szMessage )];
	StringCbPrintfA( szFormattedMessage, _countof( szMessage ), "[%04d] [%04d] %s(%d): %s", GetCurrentProcessId(), GetCurrentThreadId(), file, line, szMessage );

	WriteDebugger( szFormattedMessage );
	WriteFile( szFormattedMessage );

	std::cout << szFormattedMessage << std::endl;
