...
const char * QuestData::GetMonsterName( int i )
{
...
const char * QuestData::GetItemName( int i )
{
	static const char * pszNull = "";
	if ( i < 0 || i >= QUEST_MONSTERITEM_MAX )
		return pszNull;
...
void QuestData::SetItemName( int i, const char * pszName )
{
	if ( i < 0 || i >= QUEST_MONSTERITEM_MAX )
		return;
	...
	STRINGCOPY( szaItemName[i], pszName );
}
