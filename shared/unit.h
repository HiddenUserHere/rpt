...
enum EAnimationType : int
{
	...
	ANIMATIONTYPE_True					= 0x0001,	//1 -- I don't know what this is for
...
enum EUpdateMode : int
{
	...
	UPDATEMODE_ServerOrClientNPC		= 11, //Not sure if used
...
enum EPotionStatus : int
{
	...
	POTIONSTATUS_Flinch					= 5,
	POTIONSTATUS_Yahoo					= 7,
	POTIONSTATUS_EtherCore				= 0x40,

};

enum QuestCodeSkillUnit : int
{
	QUESTCODESKILLUNIT_NPCMorion		= 0x0002,
	QUESTCODESKILLUNIT_NPCTempriskron	= 0x0003,
	QUESTCODESKILLUNIT_Tier4			= 0x0010,
	QUESTCODESKILLUNIT_FuryQuest		= 0x0028,
};
...
struct MoveSequenceBuffer
{
	int					  iCount;					//0x00
	MoveSequence		  saMoveSequenceFirst;		//0x04
}; //Size = 0x28
...
private:
	...
	IMPFNC									pfnSetIceFootstep			= 0x004215D0;
	IMPFNC									pfnHandleEventAttack		= 0x00438EF0;
	...
	IMPFNC									pfnChangeAnimationID		= 0x00416CA0;
	...
	IMPFNC									pfnSetIceFootstep			= 0x00000000;
	IMPFNC									pfnHandleEventAttack		= 0x00000000;
#endif
...
public:
...
#if defined(_GAME)
	PTMesh							* pcTargetBoneFighterT5;
	UnitData						* pcTargetUnitDataFighterT5;
	DWORD							dwTimeTargetFighterT5;
	...
	ModelDataOld					* psBody;
	ModelDataOld					* psHead;
	ModelAnimationOld				* psModelAnimation;

	ModelDataOld					* psTalkModelData;
	ModelAnimationOld				* psTalkModelAnimation;
#endif
	...
	void							* pcaUnitDataTargetList;		//0x4AB0

	PTMesh							* pcBoneShamanBackWeapon;

	int								iPadding13[184];				//0x4AB8
#else
...
#if defined(_GAME)
	ModelAnimation					* GetModelAnimationByIndex( int iIndex ) { return ((iIndex >= 0 && iIndex < 512) && psBody) ? &psBody->saModelAnimation[iIndex] : nullptr; };
#else
	ModelAnimationOld				* GetModelAnimationByIndex( int iIndex ) { return ((iIndex >= 0 && iIndex < 512) && psBody) ? &psBody->saModelAnimation[iIndex] : nullptr; };
#endif
	...
	//GAME SIDE NEED TO BE FIX!
	BOOL							AttackFromPosition( int iX, int iY, int iZ, int iRange, BOOL bAttack = TRUE );
	...
	BOOL							HasWaterHeight() { return iWaterHeight != (-32767); }
	int								GetWaterHeight() { return iWaterHeight; }
	
	void							SetWaterHeight( int iHeight ) { iWaterHeight = iHeight; }
	void							InvalidateWaterHeight() { SetWaterHeight( -32767 ); }

	void							SetIceFootstep( int iType, DWORD dwTimeOut, int iX, int iZ );

	int								GetFrameEvent( int iEventID ) { return (psModelAnimation && (iEventID >= 0) && (iEventID < 4)) ? psModelAnimation->iaEventFrame[iEventID] : 0; }
	
	void							DecreaseFrame( int iPercent ) { iFrame -= (int)((float)iPercent * 0.01f) * iFPS; }

	int								GetAnimationBeginFrame( bool bGameFrameType = true ) { return (psModelAnimation) ? psModelAnimation->iBeginFrame * (bGameFrameType ? 160 : 1) : 0; }
	int								GetAnimationEndFrame( bool bGameFrameType = true ) { return (psModelAnimation) ? psModelAnimation->iEndFrame * (bGameFrameType ? 160 : 1) : 0; }

	BOOL							IsRepeatAnimation() { return psModelAnimation ? psModelAnimation->bLoop : FALSE; }

	BOOL							HandleAttackEvent( BOOL bEvent = FALSE );
...
static ECharacterClass GetCharacterClassByAnimationClassFlag( EAnimationClassFlag eAnimationClassFlag )
{
	ECharacterClass eCharacterClass = ECharacterClass::CHARACTERCLASS_None;

	switch ( eAnimationClassFlag )
	{
		case ANIMATIONCLASSFLAG_Fighter:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Fighter;
			break;
		case ANIMATIONCLASSFLAG_Mechanician:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Mechanician;
			break;
		case ANIMATIONCLASSFLAG_Archer:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Archer;
			break;
		case ANIMATIONCLASSFLAG_Pikeman:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Pikeman;
			break;
		case ANIMATIONCLASSFLAG_Atalanta:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Atalanta;
			break;
		case ANIMATIONCLASSFLAG_Knight:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Knight;
			break;
		case ANIMATIONCLASSFLAG_Magician:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Magician;
			break;
		case ANIMATIONCLASSFLAG_Priestess:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Priestess;
			break;
		case ANIMATIONCLASSFLAG_Assassin:
			eCharacterClass = ECharacterClass::CHARACTERCLASS_Assassin;
			break;
		case ANIMATIONCLASSFLAG_Shaman:
...
static int GetRadian2D( int x1, int y1, int x2, int y2 )
{
