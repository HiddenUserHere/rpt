...
void User::Swap( User * pcUser )
{
	...
	PacketCharacterRecordData * psCharacterFileOld	= pcUserData->psCharacterFile;
	...
	vPersonalShopItems.clear();
	for ( auto & cItemData : pcUser->vPersonalShopItems )
		vPersonalShopItems.push_back( cItemData );

	uAntiCheat 						= pcUser->uAntiCheat;
	uTimeAntiCheat 					= pcUser->uTimeAntiCheat;

	uSavePacketCounter				= pcUser->uSavePacketCounter;

	saEquippedItems[0]				= pcUser->saEquippedItems[0];
	saEquippedItems[1]				= pcUser->saEquippedItems[1];
	saEquippedItems[2]				= pcUser->saEquippedItems[2];
	saEquippedItems[3]				= pcUser->saEquippedItems[3];
}
