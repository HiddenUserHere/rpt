...
struct CharacterStatusBaseData
{
...
static const CharacterStatusBaseData saCharacterStatusBaseData[] =
{
	//Class							Strength	Spirit		Talent		Agility		Health
	{ CHARACTERCLASS_None,			0,			0,			0,			0,			0  },
	{ CHARACTERCLASS_Fighter,		28,			6,			21,			17,			27 },
	{ CHARACTERCLASS_Mechanician,	24,			8,			25,			18,			24 },
	{ CHARACTERCLASS_Archer,		17,			11,			21,			27,			23 },
	{ CHARACTERCLASS_Pikeman,		26,			9,			20,			19,			25 },
	{ CHARACTERCLASS_Atalanta,		23,			15,			19,			19,			23 },
	{ CHARACTERCLASS_Knight,		26,			13,			17,			19,			24 },
	{ CHARACTERCLASS_Magician,		16,			29,			19,			14,			21 },
	{ CHARACTERCLASS_Priestess,		15,			28,			21,			15,			20 },
	{ CHARACTERCLASS_Assassin,		25,			10,			22,			20,			22 },
	{ CHARACTERCLASS_Shaman,		15,			27,			20,			15,			22 },
};

static const CharacterStatusBaseData & GetCharacterStatusBaseData( ECharacterClass eCharacterClass )
{
	for ( int i = 0; i < _countof( saCharacterStatusBaseData ); ++i )
	{
		if ( saCharacterStatusBaseData[i].eCharacterClass == eCharacterClass )
			return saCharacterStatusBaseData[i];
	}

	return saCharacterStatusBaseData[0];
}
...
//	{(EItemID)0x02015300, CHARACTERCLASS_None, "char\\tmABCD\\ar0nugly02.ini"},	//Shemale					
	...
	{ ITEMID_First130Costume, CHARACTERCLASS_Fighter, "char\\tmABCD\\rPT_Costume_5_FS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Mechanician, "char\\tmABCD\\rPT_Costume_5_MS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Archer, "char\\tmABCD\\rPT_Costume_5_AS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Pikeman, "char\\tmABCD\\rPT_Costume_5_PS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Atalanta, "char\\tmABCD\\rPT_Costume_5_ATA.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Knight, "char\\tmABCD\\rPT_Costume_5_KS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Magician, "char\\tmABCD\\rPT_Costume_5_MG.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Priestess, "char\\tmABCD\\rPT_Costume_5_PRS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Assassin, "char\\tmABCD\\rPT_Costume_5_ASS.ini" },
	{ ITEMID_First130Costume, CHARACTERCLASS_Shaman, "char\\tmABCD\\rPT_Costume_5_SS.ini" },
...
#define MAX_POTION_SAVE_SLOTS	3
#define MAX_POTION_SAVE_COUNT	4
...
struct CharacterSave
{
	...
	short				bShowInitialPet;
	short				sElementPet;

	short				sPotionUpdate[2];
	short				sPotionCount[MAX_POTION_SAVE_SLOTS][MAX_POTION_SAVE_COUNT];
...
struct	PacketCharacterRecordData : Packet
{
	...
	int				iDataSize;
	BYTE			baData[sizeof(RecordItem) * MAX_RECORD_ITEMS];
};

#define MAX_PKTSIZE_SAVEDATA			(0x2000 - 0x20)
...
#define NAX_PACKET_SAVERDATA_BLOCKS		64
...
public:
	...
	bool						bPacketBlockReceived[NAX_PACKET_SAVERDATA_BLOCKS];	//Used to check if a packet has been received
...
struct PersonalShopOfflineUser
{
	int						iID;
	int						iCharacterID;
	int						iServerID;
	...
	PersonalShopOfflineUser( int _iID, int iServerID, int _iCharacterID, const CharacterData & _sCharacterData, const Point3D & _sPosition, const Point3D & _sAngle, const char * _szPersonalShop, UINT _iTotalItems, const char * _szShopCode )
	{
		iID = _iID;
		iCharacterID = _iCharacterID;
		sPosition = _sPosition;
		sAngle = _sAngle;
		STRINGCOPY( szPersonalShop, _szPersonalShop );
		iTotalItems = _iTotalItems;
		STRINGCOPY( szShopCode, _szShopCode );

		sCharacterData = _sCharacterData;
	};

	~PersonalShopOfflineUser() {};
};
