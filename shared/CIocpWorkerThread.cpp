...
void CIocpWorkerThread::HandleReceive( CIocpBase & cBase, DWORD dwTransferred )
{
    if ( auto cConnection = cIocpData.cConnectionManager.GetConnection( cBase.iID ) )
    {
        ...
        if ( dwTransferred )
        {
            ...
            {
                if ( cConnection->IsFirstPacket() )
                {
                    cConnection->SetFirstPacket();

#if defined(_SERVER)
                    if ( dwTransferred >= 8 )
                    {
                        UINT uHeader = *(UINT *)&cBase.vData.data()[0];

                        //Check if the header is CONNECT, GET, POST, PATCH, PUT, DELETE, HEAD, OPTIONS, TRACE
                        if ( (uHeader == 0x43415254) || (uHeader == 0x4954504F) || (uHeader == 0x44414548) || (uHeader == 0x454C4544) || (uHeader == 0x20545550) || (uHeader == 0x43544150) || (uHeader == 0x54534F50) || (uHeader == 0x20544547) || (uHeader == 0x4E4E4F43) )
                        {
                            if ( cConnection->CloseReceive() )
                                CIocpUtils::PostDisconnect( cIocpData, (*cConnection) );

                            return;
						}
                    }
#endif
                    cIocpData.pHandler->OnNewConnection( cConnection->iID, cConnection->GetConnectionInfo() );
                }
...
{
    std::scoped_lock<std::mutex> l( sAcceptMutex );
    if ( dwTransferred == 0 )
    {
        ...
        {
            FNetConnectionInfo sConnInfo = CIocpUtils::GetConnectionInfo( cBase.sSocket );

            std::shared_ptr<CIocpConnection> pConnection( new CIocpConnection( cBase.sSocket, cIocpData.GetNextID(), cIocpData.iBufferSize, sConnInfo ) );
