...
{	
	...
	float4 diffuse = input.color * 0.5f + input.colorLight * 0.5f + AddColor * 0.5f;
	...
	shadedColor *= (diffuseLight + AmbientColor * 0.5f) * diffuse + specularLight * 0.25f;

	shadedColor += (specularLight * 0.35f) * diffuse;
	...
	//Back Alpha
	shadedColor.rgb *= textureColor.a;
