...
float4 PS( PS_INPUT input ) : COLOR0
{
	...
	float4 diffuse = saturate((input.color * 0.75f) + AddColor + input.colorLight);
	...
	//Shaded Color
	float4 shadedColor = BlendColor(textureColor, diffuse, Tex1ColorBlendOp, Tex1AlphaBlendOp);

	input.normal = normalize(input.normal);

	float diffuseLight = DiffuseLightningFromView(input.normal, GlobalLightDirection);

	float4 specularLight = SpecularLightningFromView(1.0f, input.normal, input.viewDirection, GlobalLightDirection);

	input.normal = ComputeNormalMap(input.normal, input.tangent, input.bitangent, NormalMapSampler, input.tex1.xy);
	
	shadedColor += (diffuseLight * specularLight * input.colorLight) * 0.1f + AmbientColor;
