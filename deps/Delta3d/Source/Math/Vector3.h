...
public:
	...
	//! Rotate Vector from an Axis
	Vector3 & Rotate( const Vector3 & vUp, float axis )
	{
        float cos_theta = cosf( axis );
        float sin_theta = sinf( axis );

		auto v = *this;

        *this = (v * cos_theta) + (vUp.CrossProduct( v ) * sin_theta) + (vUp * vUp.DotProduct( v )) * (1.0f - cos_theta);
	...
	//Slerp Function
	Vector3 Slerp( Vector3 other, float percent ) const;

	//Lerp Function
	Vector3 Lerp( Vector3 end, float percent ) const
	{
		auto v = *this;

		return (v + (end - v) * percent);
	};
	...
	Matrix4 ToMatrixScale() const;

	Matrix4 ToMatrixTranslation() const;
