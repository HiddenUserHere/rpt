...
public:
	...
	//! Multiply operator with another Quaternion.
    Quaternion operator *( const Quaternion & other ) const { return Quaternion( _mm_mul_ps( _mm_loadu_ps( &x ), _mm_loadu_ps( &other.x ) ) ); }
	...
	Vector3 operator *( const Vector3& other ) const
	{
		...
		return other + ( cross1 * w + cross2 ) * 2.0f;
	}
