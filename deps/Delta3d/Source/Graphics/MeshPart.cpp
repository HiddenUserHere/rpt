...
MeshRenderResult MeshPart::CanRender()
{
	...
	if( material == nullptr )
		...
		canRender = MeshRenderResult::NotRender;
	else if( material->legacyMaterial->WindMeshBottom & 0x0400 )
		canRender = MeshRenderResult::NotRender;
	else if( material->legacyMaterial->UseState & 0x0400 )
		canRender = MeshRenderResult::NotRender;
	else if( material->legacyMaterial->Transparency > 0.05f )
		canRender = MeshRenderResult::Transparent;
	else if( material->legacyMaterial->MapOpacity )
...
bool MeshPart::Render( std::shared_ptr<VertexBuffer> vertexBuffer, bool skinnedMesh )
{
	...
	{
		//Set Index Buffer
		if( FAILED( Graphics::GetInstance()->GetDevice()->SetIndices( indexBuffer->Get() ) ) )
			return false;

		//Set Vertex Declaration
		if( FAILED( Graphics::GetInstance()->GetDevice()->SetVertexDeclaration( skinnedMesh ? VertexDeclaration::SkinnedTex[1]->Get() : VertexDeclaration::Tex[2]->Get() ) ) )
			return false;
		...
		if( material->Prepare() )
		{
			if( material->GetEffect()->Begin() > 0 )
			{
				for( unsigned int i = 0; i < material->GetEffect()->NumPasses(); i++ )
				{
					...
					{
						Graphics::GetInstance()->GetDevice()->DrawIndexedPrimitive( D3DPT_TRIANGLELIST, 0, 0, vertexBuffer->ElementCount(), 0, indexBuffer->PrimitiveCount() / 3 );
