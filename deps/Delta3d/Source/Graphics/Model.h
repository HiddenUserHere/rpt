...
public:
	...
	/**
	 * Get a specific Mesh from Model by name
	 * @param iMeshID Index of Mesh
	 * @return Index of Mesh if that was found
	 */
	int GetOrderedMeshIndex( int iMeshID );
	...
	//! Load Model.
	bool Load( std::string filePath, Model* skeleton_ = nullptr, bool temporaryTextures = false, Legacy::Model* legacyModel_ = nullptr );

	void CleanMemory();

	void GetBlendingAnimationValues( std::vector<MeshAnimationDataOld> & vMeshAnimations );
	void SetBlendingAnimationValues( float _fAnimationBlendingTime, std::vector<MeshAnimationDataOld> & vMeshAnimations );
...
public:
	...
	//! Cache
	int lastFrame;
	int lastFrameBaseStartFrame;
	int lastFrameBaseEndFrame;
	BYTE lastFrameUsedBones;

	Matrix4 localMatrix;

	float fAnimationBlendingTime;
	std::vector<MeshAnimationDataOld>	vMeshAnimationOld;
