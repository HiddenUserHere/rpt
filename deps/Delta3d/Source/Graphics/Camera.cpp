...
void Camera::UpdateProjection()
{
	if( isOrthographic )
	{
		projection.m[0][0] = 2.f / w;
		projection.m[1][1] = 2.f / h;
		projection.m[2][2] = 1.f / (farClip - nearClip);
		projection.m[3][3] = 1.0f;
		projection.m[3][2] = nearClip / (nearClip - farClip);

		frustum.DefineOrtho( w, aspectRatio, 1.0f, nearClip, farClip );
	}
	else
	{
		projection.m[0][0] = w;
