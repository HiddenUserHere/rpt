...
std::function<int( char *, int, const char * )>	TextureFactory::fDecryptExternal = nullptr;
	...
	if( filesystem::exists( fs ) && filesystem::is_regular_file( fs ) && fs.has_extension() )
	{
		...
		if( file.is_open() )
		{
			...
			if ( fileSize > 0 )
			{
				...
				int iNewLength = (int)fileSize;
				if ( TextureFactory::fDecryptExternal && ((iNewLength = TextureFactory::fDecryptExternal( fileBuffer, (int)fileSize, filePath.c_str() )) > 0) )
				{
					//Reduce File Size
					fileSize -= iNewLength;
				}
				else if( _strnicmp( ext.c_str(), ".png", ext.length() ) == 0 )
...
void TextureFactory::SetExternalDecrypter( std::function<int( char *, int, const char * )> f )
{
	fDecryptExternal = f;
}
...
IDirect3DTexture9* TextureFactory::CreateTextureFromFileInMemory( HRESULT& hr, const char* buffer, unsigned int size, int mipLevels, Math::Color colorKey, unsigned int reduceQualityLevel )
{
	...
	hr = D3DXCreateTextureFromFileInMemoryEx( graphics->GetDevice(), buffer, size, width, height, mipLevels, 0, D3DFMT_FROM_FILE, graphics->IsDeviceEx() ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED, D3DX_DEFAULT,
...
bool TextureFactory::DecryptPNG( char* buffer, unsigned int size )
{
	if( size < 0x6E )
		return false;

	bool shouldDecrypt = (BYTE)buffer[0x6A] == (BYTE)'i';
	BYTE key = buffer[0x6D];

	if( !shouldDecrypt )
		return true;

	if( key )
		for( unsigned char c = 16; c < 37; c++ )
			buffer[c] ^= key;
