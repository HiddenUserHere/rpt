...
static const bool USE_ENCODED_MATRIX = false;
...
enum class RendererVersion : int
{
	RENDERERVERSION_DirectX9,
	RENDERERVERSION_DirectX9Ex,
};
...
public:
	...
	IDirect3D9 * GetInterface() const { return bDeviceEx ? (IDirect3D9 *)EXDevice.interfaced3d : CommonDevice.interfaced3d; }
	IDirect3D9Ex * GetInterfaceEx() const { return (IDirect3D9Ex *)GetInterface(); }

	//! Get Direct3D Device.
	IDirect3DDevice9 * GetDevice() const { return bDeviceEx ? (IDirect3DDevice9 *)EXDevice.device : CommonDevice.device; }
	IDirect3DDevice9Ex * GetDeviceEx() const { return (IDirect3DDevice9Ex *)GetDevice(); }
	 ...
	 */
	bool SetMode( int width_, int height_, bool windowed_, int colorDepth_ = 32, bool antiAliasing_ = false, bool useSoftwareSkinning_ = false, HWND hWnd_ = nullptr, RendererVersion eVersion = RendererVersion::RENDERERVERSION_DirectX9Ex );

	bool IsDeviceEx() { return bDeviceEx; }
...
public:
	...
	union
	{
		...
		struct
		{
			IDirect3D9Ex * interfaced3d;	//!< Direct3D Interface
			IDirect3DDevice9Ex * device;	//!< Direct3D Device

			D3DDISPLAYMODEEX displayMode; //!< Display Mode
		} EXDevice;
	};
