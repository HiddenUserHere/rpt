...
public:
	...
	void SetProjection( float fov_, float aspectRatio_, float nearClip_ = 0.01f, float farClip_ = 1000.0f ) { isOrthographic = false; fov = fov_; aspectRatio = aspectRatio_; nearClip = nearClip_; farClip = farClip_; h = 1.0f / tanf( fov * 0.5f ); w = h / aspectRatio; UpdateProjection(); }

	//! Set Camera Ortographic Projection.
	void SetOrthoProjection( float w_, float h_, float nearClip_ = 0.01f, float farClip_ = 1000.0f ) { isOrthographic = true; w = w_; h = h_; aspectRatio = w /h; nearClip = nearClip_; farClip = farClip_; UpdateProjection();  }
...
private:
	...
	void UpdateView();
private:
	bool isOrthographic;	//!< Orthographic Camera Flag
