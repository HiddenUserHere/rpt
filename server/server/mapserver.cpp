...
void MapServer::SendStageItem( Map * pcMap, UserData * pcUserData )
{
	...
	std::vector<PacketStageItemData> vPackets;
...
void MapServer::ProcessPacket( UserData * pcUserData, PacketStageItemData * psPacket )
{
	psPacket->iLength = (sizeof( PacketStageItemData::StageItemData ) * psPacket->iCount) + 16;
	psPacket->iHeader = PKTHDR_PlayItem;
	psPacket->bActive = TRUE;
			...
			*/
			...
			if ( pcMap )
			{
				if ( pcMap->pcBaseMap )
				{
					...
					UpdateUserBossTime( pcUser, pcUserData->iMapID );
...
void MapServer::Update()
{
	...
	static int iLastTime = -1;
	if ( iLastTime != GetServerTime()->wHour )
	{
		iLastTime = GetServerTime()->wHour;
	...
	{
		//Update Boss Time
		SQLUpdateBossTimeBotDB( BOSS_TIME, SERVER_CODE, (int)(*(WORD *)0x07AC9F54) );
	}
...
NAKED UnitData * MapServer::SpawnSubMonster( UnitData * pcUnitDataMaster, int iMonsterEffectID, int iX, int iY, int iZ, int iMaxMonstersOnSpawn )
{
	JMP( 0x00559610 );
}
...
void MapServer::SQLUpdateBossTimeBotDB( int iMinute, int iServerID, int iDay )
{
	if ( LOGIN_SERVER && (GAME_SERVER == FALSE) )
		return;

	SQLConnection * pcDB = SQLCONNECTION( DATABASEID_BotDB );
	if ( pcDB->Open() )
	{
		for ( int i = 0; i < NUM_MAPS; i++ )
		{
			...
			{
				SYSTEMTIME sBossTime;
				SYSTEMTIME sLessBossTime;
				CopyMemory( &sBossTime, GetServerTime(), sizeof( SYSTEMTIME ) );
				CopyMemory( &sLessBossTime, GetServerTime(), sizeof( SYSTEMTIME ) );

				UINT64 uServerTime = SystemTimeToQWORD( GetServerTime() );

				//Get Boss Time
				for ( int j = 0; j < pcMap->sSpawnSetting.iTotalBosses; j++ )
				{
					UINT64 iLessTimeBoss = 0;
					for ( int k = 0; k < pcMap->sSpawnSetting.saBoss[j].iBossHours; k++ )
					{
						//Get Boss Time
						sBossTime.wHour = pcMap->sSpawnSetting.saBoss[j].caBossHour[k];
						sBossTime.wMinute = BOSS_TIME;
						sBossTime.wSecond = 0;
						...
						if ( sBossTime.wMinute >= GetServerTime()->wMinute )
						{
							//If Boss was spawned at the current boss time on loop, go to the next day...
							if ( sBossTime.wHour < GetServerTime()->wHour )
								AddDaysToSystemTime( sBossTime, 1 );
						}

						//Difference Time get the lowest
						UINT64 uBossTime = SystemTimeToQWORD( &sBossTime );

						if ( uBossTime >= uServerTime )
						{
							UINT64 iDifference = uBossTime - uServerTime;
							if ( (iLessTimeBoss > iDifference) || (iLessTimeBoss == 0) )
							{
								iLessTimeBoss = iDifference;
								sLessBossTime = sBossTime;
							}
						}
					...
					int iMonsterID = pcMap->sSpawnSetting.saBoss[j].cdBoss->iID;

					//Check if Boss is already in the DB
					int iID = -1;
					pcDB->Clear();
					if ( pcDB->Prepare( "SELECT TOP 1 ID FROM BossTimeMonster WHERE ([ServerID]=?) AND ([MapID]=?) AND ([MonsterName]=?)" ) )
					{
						pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &iServerID );
						pcDB->BindParameterInput( 2, PARAMTYPE_Integer, &i );
						pcDB->BindParameterInput( 3, PARAMTYPE_String, pcMap->sSpawnSetting.saBoss[j].cdBoss->szName );
					...
					if ( iID == -1 )
					{
						if ( pcDB->Prepare( "INSERT INTO BossTimeMonster([ServerID],[MapID],[MonsterID],[MonsterName],[MonsterPriorityType],[BossTimeDate],[ReadDate],[Enabled]) VALUES(?, ?, ?, ?, 0, ?, NULL, 1)" ) )
						{
							pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &iServerID );
							pcDB->BindParameterInput( 2, PARAMTYPE_Integer, &i );
							pcDB->BindParameterInput( 3, PARAMTYPE_Integer, &iMonsterID );
							pcDB->BindParameterInput( 4, PARAMTYPE_String, pcMap->sSpawnSetting.saBoss[j].cdBoss->szName );
							pcDB->BindParameterInput( 5, PARAMTYPE_Time, &sLessBossTime );
					...
					{
						if ( pcDB->Prepare( "UPDATE BossTimeMonster SET [BossTimeDate]=?, [ReadDate]=NULL WHERE [ID]=?" ) )
						{
							pcDB->BindParameterInput( 1, PARAMTYPE_Time, &sLessBossTime );
		...
		if ( pcDB->Prepare( "SELECT [Name], [MonsterEffectName], [MapID], [MapX], [MapZ] FROM MapBossPosition WHERE Enabled=1" ) )
		{
			...
			while ( pcDB->Fetch() )
			{
				...
				if ( szMonsterEffectName[0] != 0 )
				{
					...
					{
						if ( STRINGCOMPAREI( sEffect.pszCodeName, szMonsterEffectName ) )
						{
