...
void CReconnectServerHandler::Init()
{
	...
	psaCharacterFile		= (PacketCharacterRecordData *)(new char[(MAX_RECONNECT_USERS * 0x6100) + 4]);
...
BOOL CReconnectServerHandler::AddUser( User * pcUser )
{
	...
	if ( pcNewUser )
	{
		...
		if ( iIndex >= 0 )
		{
			...
			//??
			if ( pcUser->pcUserData->pcCharacterSaveData )
			{
				DELET( pcUser->pcUserData->pcCharacterSaveData );
				pcNewUser->pcUserData->pcCharacterSaveData = NULL;
			}
		}
	}
...
{
	BOOL bInGame = FALSE;
	PacketCharacterRecordData * psCharacterFile = NULL;
...
PacketCharacterRecordData * CReconnectServerHandler::GetNewCharacterFile( int iIndex )
{
