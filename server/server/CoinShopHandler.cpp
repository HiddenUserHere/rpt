...
int CCoinShopHandler::GetTimePointAmount( const char * pszAccountName )
{
	if ( (pszAccountName == NULL) || (pszAccountName[0] == 0) )
		return 0;
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "SELECT Coins FROM UserTimeCoin WHERE AccountName=?" ) )
		{
			pcDB->BindParameterInput( 1, PARAMTYPE_String, pszAccountName, lstrlenA( pszAccountName ) );
...
BOOL CCoinShopHandler::PerfectizeItem( User * pcUser, PacketCoinServiceBuy * psPacket )
{
	...
	if ( pcItemData->sItem.bPerfectItem == FALSE )
	{
		...
		{
			//Delete Old Item
			if ( ITEMSERVER->CheckValidItem( &pcItemData->sItem ) && (ITEMSERVER->UseItem( pcUser->pcUserData, pcItemData->sItem.sItemID.ToInt(), pcItemData->sItem.iChk1, pcItemData->sItem.iChk2 ) >= 0) )
			{
				Item sItem;
...
void CCoinShopHandler::HandlePacket( User * pcUser, PacketCoinShopNPC * psPacket )
{
	...
	const auto & vTabs = pcCoinShop->GetTabs();
	for ( auto it = vTabs.begin(); it != vTabs.end(); it++ )
	{
		...
		const auto & vChildTabs = pcTab->GetChildTabs();
		for( auto it = vChildTabs.begin(); it != vChildTabs.end(); it++ )
		{
...
void CCoinShopHandler::HandlePacket( User * pcUser, PacketCoinOpenTab * psPacket )
{
	...
	const auto & vItems = pcTab->GetItems();
