...
ItemData * ItemServer::FindItemPointerTable( const UINT uCode )
{
	if ( !pAllocItemTable )
		return NULL;
...
int ItemServer::UseItem( UserData * pcUserData, DWORD dwCode, DWORD dwHead, DWORD dwChkSum )
{
	...
	for ( int i = 0; i < 300; i++ )
	{
		if ( pcUserData->sInventoryItems[i].iItemID )
		{
			if ( (pcUserData->sInventoryItems[i].iItemID == sItemID.ToInt()) &&
				(pcUserData->sInventoryItems[i].iChk1 == dwHead) &&
				(pcUserData->sInventoryItems[i].iChk2 == dwChkSum) )
			{
				pcUserData->sInventoryItems[i].iItemID = 0;
				iRet = i;
				break;
			}
		}
...
int ItemServer::AddItemInventory( UserData * pcUserData, DWORD dwCode, DWORD dwHead, DWORD dwChkSum )
{
	...
	if ( iRet < 0 )
	{
		if ( iNewIndex >= 0 )
		{
			pcUserData->sInventoryItems[iNewIndex].iItemID	= sItemID.ToInt();
			pcUserData->sInventoryItems[iNewIndex].iChk1	= dwHead;
			pcUserData->sInventoryItems[iNewIndex].iChk2	= dwChkSum;
...
BOOL ItemServer::GetItemInventoryCheksum( User * pcUser, EItemID iItemID, int & iChk1, int & iChk2 )
{
	...
	for ( int i = 0; i < 300; i++ )
	{
		if ( pcUser->pcUserData->sInventoryItems[i].iItemID )
		{
			if ( pcUser->pcUserData->sInventoryItems[i].iItemID == ItemID( iItemID ).ToInt() )
			{
				iChk1 = pcUser->pcUserData->sInventoryItems[i].iChk1;
				iChk2 = pcUser->pcUserData->sInventoryItems[i].iChk2;
...
BOOL ItemServer::CheckValidItem( const Item * psItem )
{
...
NAKED int ItemServer::DecodeItemsDataPotion( UserData * pcUserData, void * pItemData, void * pItemDest, int iSize, void * pRecorData )
{
	JMP( 0x004452C0 );
}

NAKED int ItemServer::ComparePotions( UserData * pcUserData, CharacterSave * psCharacterSave )
{
	JMP( 0x0044E120 );
}

NAKED int ItemServer::LoadForceOrb( UserData * pcUserData, CharacterSave * psCharacterSave )
{
	JMP( 0x0044DFB0 );
}
...
void ItemServer::SQLItemLogBox( const char * pszAccountName, ItemLogBox * psItemLogBox, BOOL bAccepted )
{
	SQLConnection * pcDB = SQLCONNECTION( DATABASEID_LogDB );
...
int ItemServer::SQLItemBoxCount( const char * pszAccountName )
{
...
int ItemServer::SQLItemBoxCount( const char * pszAccountName, EItemID eItemID )
{
	if ( (pszAccountName == NULL) || (pszAccountName[0] == 0) )
		return 0;
...
BOOL ItemServer::AddItemOpenBox( UserData * pcUserData, const char * pszItemName, int iSpec, int iQuantity )
{
	return AddItemOpenBox( pcUserData->szAccountName, pszItemName, iSpec, iQuantity );
}

BOOL ItemServer::AddItemOpenBox( const char * pszAccountName, const char * pszItemName, int iSpec, int iQuantity )
{
	char szItemName[32] = { 0 };
...
BOOL ItemServer::AddItemOpenBox( const char * pszAccountName, Item * psItem, const char * pszSender )
{
	...
	for ( int i = 0; i < psPacket->iItemListCount; i++ )
	{
		...
		switch ( dwCode )
		{
			...
			case 0x01010000:
				...
				if ( ItemID( psPacket->sItemList[i].iItemID ).ToItemType() == ITEMTYPE_Dagger )
				{
					bCanUse = TRUE;
					if ( (iPosition != ITEMSLOT_RightHand) && (iPosition != ITEMSLOT_LeftHand) )
						bCanUse = FALSE;
				}
				break;
...
void ItemServer::HandlePacket( User * pcUser, PacketRequestBeginnerItemsTimer * psPacket )
{
	...
	if ( pcUser->pcUserData->sCharacterData.iLevel <= MAX_LEVEL_BEGINNERSBUFF )
	{
		...
		{
			CreatePremium( pcUser->pcUserData, ITEMID_ExpUp24H, ITEMTIMERTYPE_ExpUp, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_HPUp12H, ITEMTIMERTYPE_HPUp, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_MPDown12H, ITEMTIMERTYPE_MPDown, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_SPDown12H, ITEMTIMERTYPE_SPDown, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_HPBooster, ITEMTIMERTYPE_HPBooster, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_STMBooster, ITEMTIMERTYPE_STMBooster, ITEMTIMERTIME_6H);
			CreatePremium( pcUser->pcUserData, ITEMID_MPBooster, ITEMTIMERTYPE_MPBooster, ITEMTIMERTIME_6H);

			CHATSERVER->SendChatAllEx(CHATCOLOR_Global, "Regnum> new player %s boosted to 70!", pcUser->pcUserData->sCharacterData.szName);
			CHATSERVER->SendChatEx(pcUser, CHATCOLOR_Whisper, "Guide: You can now train your skills at 0 cost at your skillmaster.");
			CHATSERVER->SendChatEx(pcUser, CHATCOLOR_Whisper, "Guide: As level 70 you can start by questing in Cursed Temple 1");
		...
		//Add level
		if ( pcUser->pcUserData->sCharacterData.iLevel < 69 )
			CHARACTERSERVER->GiveEXP( pcUser, CHARACTERSERVER->GetExpFromLevel( 70 ) - CHARACTERSERVER->GetExp( pcUser->pcUserData ) );

		//QUESTSERVER->SkipTier4Quests(pcUser);
...
#ifndef BOOSTEVENT
	CHATSERVER->SendChatMessageBox(pcUser, "Boost event is currently not active.");
#endif // !BOOSTEVENT
