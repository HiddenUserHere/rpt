...
{
	switch ( pcUser->GetCharacterClass() )
	{
		...
		case CHARACTERCLASS_Archer:
			switch ( psPacket->iID )
			{
				...
				case SKILLID_LethalSight:
					...
					pcUser->uLethalSightCritical 					= 0;
					pcUser->uLethalSightDamageBoost 				= 0;
					pcUser->uLethalSightRange 						= 0;
					break;
		...
		case CHARACTERCLASS_Knight:
			switch ( psPacket->iID )
			{
				...
				case SKILLID_HolyIncantation:
					if ( pcUser->pcUserData->dwHolyIncantationID )
					{
						UnitData * pcUnitDataIncantaten = UNITSERVER->GetUnit( pcUser->pcUserData->dwHolyIncantationID );
						if ( pcUnitDataIncantaten && pcUnitDataIncantaten->sCharacterData.iOwnerID == pcUser->pcUserData->iID && pcUnitDataIncantaten->sCharacterData.sHP.sCur != 0 &&
							pcUnitDataIncantaten->sHolyIncantation.sMax > 0 )
						{
							pcUnitDataIncantaten->sCharacterData.sHP.sCur = 0;
							pcUnitDataIncantaten->Animate( ANIMATIONTYPE_Die );
						}

						pcUser->pcUserData->dwHolyIncantationID = 0;
						pcUser->pcUserData->pcHolyIncantatedUnitData = nullptr;
					}
					break;
...
NAKED BOOL CDamageHandler::SendDamageCodeToClient( UserData * pcUserData )
{
	JMP( 0x00579540 );
}
