...
BOOL BAN_HANDLE_GUARD_IDLE = TRUE;
...
//				Disconnect = TRUE;
		...
		//		Disconnect = TRUE;
			...
			case CHEATLOGID_ProcessHook:
			{
				...
				if( (strProcessName.find( "uc.exe" ) != std::string::npos) || (strProcessName.find( "uce.exe" ) != std::string::npos) )
					Banned = TRUE;
			...
			case CHEATLOGID_SavingUnkownReturn:
				STRINGFORMAT( szCheatLog, "Character[%s] Saving Unkown Return[%d]", pszCharacterName, psPacket->LParam );
				break;
			...
			case CHEATLOGID_WeightPotionPositionError:
				// no need
				break;
			...
			case CHEATLOGID_WrongSavingCharacterName:
				STRINGFORMAT( szCheatLog, "Character[%s] Wrong Saving Character Name", pszCharacterName );
				Banned = TRUE;
				break;

			case CHEATLOGID_AntiCheatOffline:
				STRINGFORMAT( szCheatLog, "Character[%s] Handle Guard Idle Time[%d secs]", pszCharacterName, psPacket->LParam );
				Banned = BAN_HANDLE_GUARD_IDLE;
				break;

			case CHEATLOGID_CharacterSaveChecksumError:
				STRINGFORMAT( szCheatLog, "Character[%s] Character Save Checksum Error Game[%d] Server[%d]", pszCharacterName, psPacket->LParam, psPacket->SParam );
				break;

			case CHEATLOGID_ModelFileNotFound:
				STRINGFORMAT( szCheatLog, "Character[%s] Model not found or modified [%s]", pszCharacterName, ((PacketLogCheatEx *)psPacket)->szParam );
				Disconnect = TRUE;
				break;
...
void LogServer::LogSocketClose( SocketData * pcSocketData, const char * pszLocation, void * p )
{
	...
	{
		const char * pszAccountName = pcSocketData->ud != 0 ? pcSocketData->ud->szAccountName : "";
		...
		const char * pszServer = GAME_SERVER != 0 ? "GameServer" : "LoginServer";
