...
void Server::Init( HWND hWnd )
{
	...
	ShowWindow( hWnd, CServerWindow::IsConsoleEnabled() ? SW_HIDE : SW_SHOW );
	UpdateWindow( hWnd );
}
...
void Server::Load()
{
	...
	CreateSQLConnection( DATABASEID_BotDB );
...
const char * Server::GetServerName( int iIndex )
{
...
int Server::GetServerIndex( const char * pszIP, int iPort )
{
