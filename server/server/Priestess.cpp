...
BOOL CPriestess::OnHolyReflection( UserData * pcUserData, UnitData * pcUnitData )
{
	User * pcUser = USERDATATOUSER( pcUserData );
	if( pcUser == nullptr )
		return TRUE;

	if ( pcUserData->dwHolyReflectionEndTime == 0 ) 
		return TRUE;

	if ( pcUserData->dwHolyReflectionEndTime < TICKCOUNT )
	{
	...
	int iDamage = RandomI( pcUnitData->sCharacterData.iMinDamage, pcUnitData->sCharacterData.iMaxDamage );
	iDamage = (iDamage * pcUserData->iHolyReflectionDamage) / 100;
	int iResistance = pcUnitData->sCharacterData.sElementalDef[ELEMENTID_Organic];
	...
	if ( iResistance )
	{
		...
		iDamage -= ((iDamage * iResistance) / 100);
	}
	iDamage -= (iDamage * pcUnitData->sCharacterData.iAbsorbRating) / 100;

	if ( iDamage > 0 )
	{
		pcUnitData->sCharacterData.sHP.sCur -= iDamage;
	...
	sPacket.iUnk = 0x2B;
	sPacket.iUnk2 = CHARACTERCLASS_Priestess;
	sPacket.iUnk3 = 0;
	sPacket.iUnk4 = 0;
...
BOOL CPriestess::RestoreHPPacketParty( UserData * pcUserData, PacketRestoreHP * psPacket )
{
	...
	if( pcUser )
	{
		if( pcUser->bParty && pcUser->psParty )
		{
			...
			PARTYHANDLER->GetPartyMembers( vUsers, pcUser->psParty );
			for( auto &p : vUsers )
			{
				if( p && p != pcUser )
				{
