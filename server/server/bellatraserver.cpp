...
void BellatraServer::UpdateClanCrown()
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "SELECT TOP 3 IconID FROM ClanList WHERE BellatraPoints > 0 ORDER BY BellatraPoints DESC" ) )
		{
			if ( pcDB->Execute() )
			{
				while ( pcDB->Fetch() )
...
BOOL BellatraServer::IsRoomInParty( std::vector<UserData*> & vUsersRoom )
{
	...
	if ( vUsersRoom.size() > 0 )
	{
		...
		if ( pcUser )
		{
			...
			{
				std::vector<User *> vUsersParty;
				PARTYHANDLER->GetPartyMembers( vUsersParty, pcUser->psParty );
