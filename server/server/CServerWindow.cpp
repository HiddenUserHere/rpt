...
CServerWindow::CServerWindow() : CWindow()
{
	...
	pServer = new CServer();
	pcConsoleHandler = new CConsoleHandler();
}

CServerWindow::~CServerWindow()
{
	DELET( pcConsoleHandler );
	DELET( pServer );
	DELET( pConfig );
}
...
UINT CServerWindow::Init()
{
	...
	std::string strServerType = SERVER_TYPE == SERVERTYPE_Login ? "Login" : SERVER_TYPE == SERVERTYPE_Game ? "Game" : "Multi";

	WRITEERR( "[SERVER] [%s Server] STARTED", strServerType.c_str() );

	pcConsoleHandler->SetTitle( "[" + strServerType + "] Fortress World Server");

	bExit = FALSE;

	CHARACTERDATAHANDLER->DoImport();
	WAREHOUSEHANDLER->DoImport();
