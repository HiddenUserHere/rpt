...
protected:
	...
	FNetHandlerImpl										* pcSocketServerHandler = nullptr;


	std::vector<CIocpGameHandler *>						vIocpClients;
	std::vector<CFNetClientHandler *>					vRakNetClients;

public:

	static UINT											uBytePacketFlow;

	FNetHandlerImpl										* GetServerSocketHandler() { return pcSocketServerHandler; }
	...
	CFNetClientHandler									* GetRakNetHandler( SocketData * sd );
	CFNetClientHandler									* AddRakNetHandler();
	void												RemoveRakNetHandler( SocketData * sd );
	...
	void												ProcessPacketRequestInitialData( User * pcUser );
};
