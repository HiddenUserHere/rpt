				...
				if ( pcUser->IsConnected() && pcUser->IsInGame() && (psDamageData = UNITSERVER->GetUserInDamageList( pcUnitData, pcUser )) )
				{
					...
					if ( iDistance < DISTANCE_UNIT_FAR )
					{
						...
						if ( (psDamageData->iDamage >= iDamageNeeded) || (psDamageData->iCounter >= ROLLDICE_DAMAGE_COUNT) )
						{
							...
							int iCount = 0;
							int iIpCount = 0; // Add a counter for IP
							for (auto pc : v)
							{
								if (pc->pcUser->pcUserData->pcSocketData
									&& pc->pcUser->pcUserData->pcSocketData->lIP == pcUser->pcUserData->pcSocketData->lIP)
								{
									iIpCount++; // Increase the counter when we found a matching IP
									if (iIpCount >= 2) // If we have seen this IP twice or more, we will set bAllowedUser to FALSE
									{
										bAllowedUser = FALSE;
										break;
									}
...
void CRollDiceHandler::HandleQuest( User * pcUser, UnitData * pcUnitData )
{
	...
	if ( pcUnitData->pcMap && pcUnitData->pcMap->pcBaseMap )
	{
		...
		if ( (pcUnitData->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_BABEL) || (pcUnitData->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_DEATHKNIGHT) ||
			(pcUnitData->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_KELVEZU) || (pcUnitData->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_MOKOVA) ||
			(pcUnitData->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_DarkGuardian) || (STRINGCOMPARE( pcUnitData->sCharacterData.szName, "Gorgonyte" )) )
			bSend = true;
