...
void AccountServer::LogAccountLogin( UserData * pcUserData, AccountLogin & al, ELogAccount eLogAccount )
{
	...
	const char * pszSystemOS = "Unknown";
	...
	switch ( eLogAccount )
	{
		case ACCLOGID_LoginSuccess:
			STRINGFORMAT( szLogDescription, "[%s] (Login Success) -> Password[%s] MacAddr[%s] PCName[%s] SerialHD[%d] VideoName[%s] VideoGUID[%d] HardwareID[%s][%s] FolderID[%d] Width[%d] Height[%d] SystemOS[%s] PacketChecksum(Game[%d] Server[%d])",
				al.szAccountName, al.szPassword, al.szMacAddress, al.szPCName, al.uSerialHD, al.szVideoName, al.uVideoGUID, al.szHardwareID, al.szNewHardwareID, al.uUniqueFolderID, al.uWidthScreen, al.uHeightScreen, pszSystemOS,
				al.uPacketChecksumGame, al.uPacketChecksumServer );
			break;

		case ACCLOGID_IncorrectAccount:
			STRINGFORMAT( szLogDescription, INCORRECT_ACCOUNT_LOGACCTEXT, 
				al.szAccountName, al.szPassword, al.szMacAddress, al.szPCName, al.uSerialHD, al.szVideoName, al.uVideoGUID, al.szHardwareID, al.szNewHardwareID, al.uUniqueFolderID, al.uPacketChecksumGame, al.uPacketChecksumServer );
			break;
...
{
	//Prog
	if ( STRINGCOMPAREI( pszMacAddress, "F4-B5-20-41-05-B6" ) )
	{
		//HD
		if ( uSerialHD == 169323131 )
			return TRUE;
	}
/*
	//Teylor
	if ( STRINGCOMPAREI( pszMacAddress, "E0-D5-5E-F6-7F-F1" ) )
	{
		//HD
		if ( uSerialHD == 2063469280 )
			return TRUE;
	}

	//Guilherme
	if ( STRINGCOMPAREI( pszMacAddress, "8C-1D-96-09-02-CD" ) )
	{
		//HD
		if ( uSerialHD == 610344555 )
			return TRUE;
	}

	//Thiago PC
	if ( STRINGCOMPAREI( pszMacAddress, "04-92-26-48-ED-47" ) )
	{
		//HD
		if ( uSerialHD == -389571219 )
			return TRUE;
	}

	
	//Igor
	if ( STRINGCOMPAREI( pszMacAddress, "38-2C-4A-8C-92-91" ) || STRINGCOMPAREI( pszMacAddress, "7A-79-19-49-D9-83" ) || STRINGCOMPAREI( pszMacAddress, "02-50-F2-49-B7-00" ) )
	{
		//HD
		if ( (uSerialHD == 337751611) || (uSerialHD == 306302523) )
			return TRUE;
	}

	//Cackto Note
	if ( STRINGCOMPAREI( pszMacAddress, "C4-85-08-9E-5E-2B" ) || STRINGCOMPAREI( pszMacAddress, "50-B7-C3-00-25-46" ) )
	{
		//HD
		if ( uSerialHD == (976021919) )
			return TRUE;
	}

	//Cackto New
	if ( STRINGCOMPAREI( pszMacAddress, "4C-CC-6A-0A-B1-8D" ) )
	{
		//HD
		if ( uSerialHD == 140130492 )
			return TRUE;
	}
*/
	...
	//CHAOS GOD
	if ( STRINGCOMPAREI( pszMacAddress, "D7-68-01-9A-A5-9E" ) )
	{
		//HD
		if ( uSerialHD == -285238439)
			return TRUE;
	}

	//Hadder
	if (STRINGCOMPAREI( pszMacAddress, "60-45-CB-6C-74-F5" ) )
	{
		//HD
		if ( uSerialHD == 40795605 )
			return TRUE;
	}
...
{
	//Prog
	if ( STRINGCOMPAREI( pszMacAddress, "FC-AA-14-FD-3D-B2" ) )
	{
		return TRUE;
	}
...
BOOL ENABLE_PACKET_LOGIN_CHECKSUM_BAN = FALSE;
BOOL AccountServer::IsPacketChecksumNotMatch( UserData * pcUserData, UINT uPacketChecksumGame, UINT uPacketChecksumServer )
{
	if ( ENABLE_PACKET_LOGIN_CHECKSUM_BAN == FALSE )
		return FALSE;

	return uPacketChecksumGame != uPacketChecksumServer;
}
...
void AccountServer::PHAccountLogin( User * pcUser, PacketLoginUser * psPacket )
{
	if ( LOGIN_SERVER )
	{
		...
		al.uPacketChecksumGame = psPacket->uPacketChecksum;
		al.uPacketChecksumServer = uPacketChecksum;
		...
		STRINGCOPY( al.szNewHardwareID, psPacket->szNewHardwareID );
	...
	if ( bContinue )
	{
		...
		if ( iCode == ACCOUNTLOGIN_ConnectionFailed )
		{
			...
			if ( iCode == ACCOUNTLOGIN_ConnectionFailed )
			{
				...
				if ( ACCOUNTSERVER->IsHardwareIDBanned( pcUserData, al.szNewHardwareID ) )
				{
					ACCOUNTSERVER->LogAccountLogin( pcUserData, al, ACCLOGID_IncorrectAccount );
				...
				if ( ACCOUNTSERVER->IsPacketChecksumNotMatch( pcUserData, al.uPacketChecksumGame, al.uPacketChecksumServer ) )
				{
					ACCOUNTSERVER->LogAccountLogin( pcUserData, al, ACCLOGID_IncorrectAccount );
...
BOOL AccountServer::OnLoginSuccess( UserData * pcUserData )
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( R"Query(SELECT TOP 6
				cd.CharacterName AS CharacterName,
				cd.BodyModel AS BodyModel,
				cd.HeadModel AS HeadModel,
				cd.[Level] AS [Level],
				cd.Class AS Class,
				cs.MapID AS MapID,
				cs.LastPositionX AS LastPositionX,
				cs.LastPositionZ AS LastPositionZ,
				ci.ItemRightHandID AS ItemRightHandID,
				ci.ItemLeftHandID AS ItemLeftHandID
			FROM
				CharacterData cd
			JOIN
				CharacterSave cs ON cd.CharacterID = cs.CharacterID
			JOIN
				CharacterInfo ci ON cd.CharacterID = ci.ID
			WHERE
				ci.AccountName = ?)Query" ) )
		{
			...
			if ( pcDB->Execute() )
			{
				...
				while ( pcDB->Fetch() )
				{
					int iItemRightHandID = ITEMID_None;
					int iItemLeftHandID = ITEMID_None;

					// Get Character Data file path
					pcDB->GetData( 1, PARAMTYPE_String, sPacketUserInfoLogin.saData[iCount].szName, _countof( sPacketUserInfoLogin.saData[iCount].szName ) );
					pcDB->GetData( 2, PARAMTYPE_String, &sPacketUserInfoLogin.saData[iCount].szBodyModel, _countof( sPacketUserInfoLogin.saData[iCount].szBodyModel ) );
					pcDB->GetData( 3, PARAMTYPE_String, &sPacketUserInfoLogin.saData[iCount].szHeadModel, _countof( sPacketUserInfoLogin.saData[iCount].szHeadModel ) );
					pcDB->GetData( 4, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iLevel );
					pcDB->GetData( 5, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iClass );
					pcDB->GetData( 6, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iMapID );
					pcDB->GetData( 7, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iX );
					pcDB->GetData( 8, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iZ );
					pcDB->GetData( 9, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iWeaponID );
					pcDB->GetData( 10, PARAMTYPE_Integer, &sPacketUserInfoLogin.saData[iCount].iShieldID );

					sPacketUserInfoLogin.saData[iCount].iArmorID = ITEMID_None;
					sPacketUserInfoLogin.saData[iCount].iType = EMonsterType::MONSTERTYPE_Normal;
