...
SocketServer::SocketServer()
{
	...
	if( IOCP_SERVER_TYPE )
		...
		pcSocketServerHandler = new CFNetServerHandler();
}
...
CFNetClientHandler * SocketServer::GetRakNetHandler( SocketData * sd )
{
...
CFNetClientHandler * SocketServer::AddRakNetHandler()
{
...
void SocketServer::RemoveRakNetHandler( SocketData * sd )
{
...
SocketData * SocketServer::Connect( const char * pszIP, int iPort )
{
	auto ConnectServer = [this]( SocketData * pcSocketData, const char * _pszIP, int _iPort )-> bool
	{
		...
		{
			auto pcClient = SOCKETSERVER->AddRakNetHandler();

			if ( pcClient->Init( _pszIP, _iPort ) )
			{
				sockaddr_in sSockAddr;
				ZeroMemory( &sSockAddr, sizeof( sockaddr_in ) );
				sSockAddr.sin_family = AF_INET;
				...
				pcSocketData->Open( 0, &sSockAddr );

				pcSocketData->iConnectionID = pcClient->GetInternalID();
	...
	if ( sd && sd->bInUse )
	{
		...
		if ( u && ud )
		{
			if ( !u->bNetServer )
			{
				...
				if ( ud->pcCharacterSaveData )
					DELET( ud->pcCharacterSaveData );
			}
...
void SocketServer::Load()
{
	...
	for ( int i = 0; i < iMaxConnections; i++ )
	{
		...
		if ( IOCP_SERVER_TYPE || RAKNET_SERVER_TYPE )
			sd->Prepare( 1, 1 );
		else
		{
			sd->Prepare( 128, 128 );
...
void SocketServer::ProcessPacketRequestInitialData( User * pcUser )
{
	//Update Keys
	{
		BYTE bObfuscator = GetPacketObfuscatorByte();
		...
		for ( int i = 0; i < NUM_ENCKEYS; i++ )
			pcUser->pcUserData->pcSocketData->baKeySet[i] = RandomI( 0, 256 );
		...
		for ( int i = 0; i < NUM_ENCKEYS; i++ )
			s.baKeySet[i] = (pcUser->pcUserData->pcSocketData->baKeySet[i] ^ bObfuscator);

		//WRITEDBG( "First Key[0]: %d", baKeySet[0] );

		SENDPACKETSOCKET( pcUser->pcUserData->pcSocketData, &s );

		pcUser->pcUserData->pcSocketData->bKeySet = true;
	}
	...
	sPacket.iUnk2 = 0;
	sPacket.iVersion = (*(int *)0x006E4F5C);
	SENDPACKETSOCKET( pcUser->pcUserData->pcSocketData, &sPacket );
}
