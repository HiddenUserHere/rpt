		...
		case PacketNPCEasterEgg::ACTION_GetReward:
			...
			if ( iCount == EASTER_ITEMS_MAX )
			{
				...
				for ( int i = 0; i < EASTER_ITEMS_MAX; i++ )
				{
					...
					//Get Item Name
					const char * pszItemName	= "Red Easter Egg";
...
void CEventServer::SQLOnlineRewardLog( const char * pszAccountName, const char * pszName, const char * pszItemName, int iQuantity, bool bDailyReward )
{
...
void CEventServer::SQLAddAccountGift( const char * pszAccountName, const char * pszAccountIP, const char * pszCharacterName )
{
...
void CEventServer::OnSayTime( UnitData * pcUnitData )
{
	...
	if ( EVENT_WANTEDMORIPH && eEffectID == MONSTEREFFECTID_NPC_MORIF )
	{
		...
		{
			const char * pszaMoriphMessage[5] =
			{
...
void CEventServer::AddXmasPrize( User * pcUser )
{
	...
	{
		const char * pszIP = pcUser->pcUserData->pcSocketData ? pcUser->pcUserData->pcSocketData->szIP : "127.0.0.1";
