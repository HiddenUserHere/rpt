...
{
	if ( ActionFieldRound * psRound = pcInstance->GetActionField()->GetRound( pcInstance->GetRound() ) )
	{
...
void CActionFieldHandler::UpdatePrizes( CActionFieldInstance * pcInstance )
{
	...
	if ( (pcInstance->iTimeFinished != 0) && (pcInstance->IsSentPrizes() == FALSE) )
	{
		...
		if ( pcActionField->GetChests().size() > 0 )
		{
			...
			ROOMINSTANCEHANDLER->GetUsersRoom( vUsers, pcInstance->GetRoom()->GetID() );
				...
				for ( UINT i = 0; i < vUsersAC.size(); i++ )
				{
					...
					for ( auto pcUser : vUsers )
					{
						if ( sUser.iID == pcUser->GetID() )
						{
							...
							{
								INT64 iEXP = 500000000 - ((INT64)i * 10000000);
								if ( pcInstance->GetActionField()->GetModeID() == ACTIONFIELDID_GhostCastle2 )
									iEXP = 800000000 - ((INT64)i * 10000000);

								//Forest Dungeon EXP table
...
void CActionFieldHandler::ProcessPacket( User * pcUser, PacketActionFieldFail * psPacket )
{
...
void CActionFieldHandler::Request( User * pcUser, EActionFieldModeID iModeID )
{
	...
	{
		std::vector<User *> vUsers;
		PARTYHANDLER->GetPartyMembers( vUsers, pcUser->bParty ? pcUser->psParty : NULL );
		if ( vUsers.size() == 0 )
		...
		for ( auto pcPartyUser : vUsers )
		{
			...
			pcInstance->SetActiveUsersCount( pcInstance->GetActiveUsersCount() + 1 );
		}
...
bool CActionFieldHandler::CanEnter(User* pcUser, EActionFieldModeID iModeID)
{
	...
	if (pcActionField)
	{
		...
		else
		{
			...
			for (auto pcPartyUser : vUsers) {
				BOOL bContinue = TRUE;
				int sameHwIdCount = 0; // Counter for same HWID
					...
					if (pcPartyUserDistinct != pcPartyUser) {
						if ((pcPartyUserDistinct->szHardwareID[0] != 0) && (pcPartyUser->szHardwareID[0] != 0)) {
							if (STRINGCOMPARE(pcPartyUserDistinct->szHardwareID, pcPartyUser->szHardwareID)) {
								sameHwIdCount++; // Increment count if HWIDs match

								// If count exceeds 2 (indicating more than 2 users from the same HWID), disallow
								if (sameHwIdCount > 2) {
									CHATSERVER->SendChat(pcPartyUser, CHATCOLOR_Error, "> More than two players from the same computer are not allowed!");
...
void CActionFieldHandler::OnDisconnectedUser( User * pcUser )
{
	...
	if ( pcInstance )
	{
		...
		//Remove User
		if ( ROOMINSTANCEHANDLER->RemoveUnit( pcUser->GetID() ) )
			pcInstance->SetActiveUsersCount( pcInstance->GetActiveUsersCount() - 1 );

		//Delete
		if ( pcInstance->GetActiveUsersCount() == 0 )
		{
			pcInstance->Delete( TRUE );
		}
	}
...
void CActionFieldHandler::OnLeaveParty( User * pcUser )
{
	...
	if ( pcInstance )
	{
		...
		//Fail Packet
		ProcessPacket( pcUser, &PacketActionFieldFail( ACTIONFIELDFAIL_LeaveParty ) );
	...
	if ( pcInstance )
	{
		if ( pcInstance->IsStarted() )
		{
			if ( GetMapID( pcInstance->GetActionField()->GetModeID() ) != iMapID )
			{
				...
				//Fail Packet
				ProcessPacket( pcUser, &PacketActionFieldFail( ACTIONFIELDFAIL_LeaveMap ) );
