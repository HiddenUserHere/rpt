...
BOOL QuestServer::HandleFinishedQuestUser( User * pcUser, int iQuestID )
{
	...
	if ( pcDB->Open() )
	{
		const char * pszQuery = "SELECT QuestID, QuestRewardID, StartDate, EndDate FROM CharacterQuest WHERE (CharacterID=?) AND (EndDate IS NOT NULL)";
...
BOOL QuestServer::SkipTier4Quests(User* pcUser)
{
	BOOL bRet = TRUE;
	...
	SQLStartQuest(pcUser, QUESTID_BlacksmithGodenGold, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_WarehouseGold, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_Quest110A, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_AnAncientThreat, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_AnEndlessBattle, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_AnEndlessSuffering, szMonsterID, szMonsterCounter, szItemID, szItemCounter);
	SQLStartQuest(pcUser, QUESTID_ATestOfBravery, szMonsterID, szMonsterCounter, szItemID, szItemCounter);

	SQLFinishQuest(pcUser, QUESTID_BlacksmithGodenGold);
	SQLFinishQuest(pcUser, QUESTID_WarehouseGold);
	SQLFinishQuest(pcUser, QUESTID_Quest110A);
	SQLFinishQuest(pcUser, QUESTID_AnAncientThreat);
	SQLFinishQuest(pcUser, QUESTID_AnEndlessBattle);
	SQLFinishQuest(pcUser, QUESTID_AnEndlessSuffering);
	SQLFinishQuest(pcUser, QUESTID_ATestOfBravery);

	HandleFinishedQuestUser(pcUser, QUESTID_BlacksmithGodenGold);
	HandleFinishedQuestUser(pcUser, QUESTID_WarehouseGold);
	HandleFinishedQuestUser(pcUser, QUESTID_Quest110A);
	HandleFinishedQuestUser(pcUser, QUESTID_AnAncientThreat);
	HandleFinishedQuestUser(pcUser, QUESTID_AnEndlessBattle);
	HandleFinishedQuestUser(pcUser, QUESTID_AnEndlessSuffering);
	HandleFinishedQuestUser(pcUser, QUESTID_ATestOfBravery);
