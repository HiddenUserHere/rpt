...
void CBattleRoyaleHandler::TeleportUser( CBattleRoyaleInstance * pcInstance, User * pcUser, bool bArena, bool bRandomPosition, bool bCountdownPosition )
{
	...
	if ( bCountdownPosition )
	{
		if ( bRoyalDesert )
		{
			sSpawn.iX = RandomI(-21051, -21696) << 8;
			sSpawn.iZ = (8637001);
			//sSpawn.iX = RandomI( -21887, -20879 ) << 8;
			//sSpawn.iZ = RandomI( 27900, 28077 ) << 8;
	...
	if ( psPacket->bReward == FALSE )
	{
		if ( psPacket->iBattleID >= 0 )
		{
			...
			for ( UINT i = 0; i < USERSERVER->uUsersInGame; i++ )
			{
				...
				{
					if (pc->pcUserData->pcSocketData
						&& pc->pcUserData->pcSocketData->lIP == pcUser->pcUserData->pcSocketData->lIP)
					{
						CHATSERVER->SendChat(pcUser, CHATCOLOR_Error, "BattleRoyale> Only one person per network can connect!");
					...
					if ( pc->szHardwareID[0] != 0 )
					{
						...
						{
							CHATSERVER->SendChat( pcUser, CHATCOLOR_Error, "BattleRoyale> Players must be on different computers!" );
					...
					if ( SQLGetRewardList( psPacket->iBattleID, vRewards ) )
					{
						for ( auto psReward : vRewards )
						{
							...
							if ( psReward->iID == psPacket->iRewardID )
							{
								...
								if ( iBattleID == BATTLEROYALE_Normal || iBattleID == BATTLEROYALE_NormalLow )
								{
		...
		if (auto pcInstance = GetInstance(pcUser))
		{
			for (auto psPlayer : pcInstance->GetPlayers())
			{
				if ((psPlayer->eState == PLAYERSTATE_Alive) && (pcUser->GetID() == psPlayer->iID))
				{
					...
					// Remove user from instance, indicating they were not disconnected, but killed
					pcInstance->RemoveUser(pcUser, false);
			...
			for (auto psPlayer : pcInstance->GetPlayers())
			{
				if ((psPlayer->eState == PLAYERSTATE_Alive) && (pcVictim->GetID() == psPlayer->iID))
				{
					...
					if ((pcInstance->GetBattleRoyale()->IsTeamRoyalTickets() == false))
					{
						...
						// Remove the user after they are killed by another user
						pcInstance->RemoveUser(pcVictim, false);
...
{
	if ( pcUser->bInBattleRoyale || pcUser->GetMapID() == MAPID_RoyalDesert || pcUser->GetMapID() == MAPID_OldRuinen2 )
		return TRUE;
			...
			for ( auto psDrop : vDropData )
			{
				if ( (iRandom >= iRandomCount) && (iRandom < (iRandomCount + psDrop->iPercent)) )
				{
					...
					if ( psDefinitionItem )
					{
						...
						{
							AGEHANDLER->OnUpAgeHandler( &sLootItem.sItem, RandomI( 15, 18 ) );
						...
						else
						{
								...
								(sLootItem.sItem.sItemID.ToItemType() == ITEMTYPE_Boots) )
							{
								...
								EMixTypeName eMixName = MIXTYPENAME_Bellum;
...
void CBattleRoyaleHandler::SendEXPPrizeGold( User * pcUser, EBattleRoyaleID iBattleID, int iInstanceID, int iKills, bool bWinner, int iMinutes, SYSTEMTIME sDate )
{
	if ( pcUser->pcUserData->sCharacterData.iLevel < SERVER_LEVEL_MAX )
	{
		...
		if (iBattleID == BATTLEROYALE_NormalLow)
		{
			iEXP = 25000000;
		...
		//EXP per Mode
		INT64 iEXPGainMode = 150000000;
		if ( CBattleRoyale::IsTeamRoyalTickets( iBattleID ) )
			iEXPGainMode = 150000000;
...
void CBattleRoyaleHandler::SendUserCoordinateList( User * pcUser, CBattleRoyaleInstance * pcInstance )
{
	...
	if ( pcInstance->GetBattleRoyale()->GetMapID() == MAPID_RoyalDesert )
	{
		sPacket.saCoordinates[sPacket.iCount].iWest = -21245;
		sPacket.saCoordinates[sPacket.iCount].iEast = -21545;
		sPacket.saCoordinates[sPacket.iCount].iNorth = 33678;
		sPacket.saCoordinates[sPacket.iCount].iSouth = 33658;
