...
{
	return UNITSERVER->GetUnitFollowType( pcUnitData );
}
...
void __cdecl AddItemServerStatusGate( Item * psItem, DefinitionItem * psDefinitionItem )
{
	...
	if ( psDefinitionItem->sItem.sItemID.ToItemID() == ITEMID_MokovaBoots )
	{
		psItem->bEvade = 1;
	}
	...
	switch ( eItemID )
	{
		...
		case ITEMID_ThirdEye3H:
			psItem->bLockedItemType = ITEMLOCKEDTYPE_Forever;
			break;
	}
