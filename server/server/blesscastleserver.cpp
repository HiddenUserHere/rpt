...
void BlessCastleServer::SpeedUpDeathTimer()
{
	if ( pcBlessCastleTower )
	{
		pcBlessCastleTower->iDeathCounter = 1;
	}
}

void BlessCastleServer::RemoveTimeAlive()
{
	if ( pcBlessCastleTower )
	{
		pcBlessCastleTower->dwLastUpdateTime = 1;
	}
}
...
{
	WRITEERR( "BlessCastleServer::End( %s )::[%u - %u - %s][%s]", bUnitsAlive ? "TRUE" : "FALSE", psBlessCastleStatusData->dwBattleOverTime, TICKCOUNT,
		psBlessCastleStatusData->dwBattleOverTime < TICKCOUNT ? "TRUE" : "FALSE", pcBlessCastleTower ? "TRUE" : "FALSE" );
...
{
	WRITEERR( "BlessCastleServer::EndBattle()::[%u - %u - %s][%s]", psBlessCastleStatusData->dwBattleOverTime, TICKCOUNT,
		psBlessCastleStatusData->dwBattleOverTime < TICKCOUNT ? "TRUE" : "FALSE", pcBlessCastleTower ? "TRUE" : "FALSE" );
