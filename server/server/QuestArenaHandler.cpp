            ...
            if ( pcUser->bInT5QuestArena )
            {
				...
				else
				{
					...
					if ( pcUser->dwTimeT5QuestArenaUpdate < TICKCOUNT )
					{
						...
						{
							//Finished quest
							CHARACTERSERVER->SetPositionWarp( pcUser->pcUserData, QUESTT5ARENA_X_LEAVE, QUESTT5ARENA_Z_LEAVE, MAPID_RicartenTown );
...
void CQuestArenaHandler::SpawnUnitCrystalT5Quest( User * pcUser )
{
    ...
    if ( pcUnitData )
    {
        ...
        pcUnitData->sCharacterData.iAbsorbRating    = 30;
