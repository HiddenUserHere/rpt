...
User* UserServer::GetUserById(UINT userId)
{
	for (UINT i = 0; i < uUsersInGame; i++)
	{
		if (pcaUserInGame[i]->GetID() == userId)
		{
			return pcaUserInGame[i];
		}
	}
	return nullptr; // Return nullptr if no user with the given ID is found.
}
...
void UserServer::HandlePacket( User * pcUser, PacketPlayData * psPacket )
{
	...
	if ( (pcUser->uAntiCheat != psPacket->iMountTimer) || (pcUser->GetGameLevel() >= GAMELEVEL_Four) || (pcUser->uTimeAntiCheat == 0) )
	{
		pcUser->uTimeAntiCheat = TICKCOUNT + (210 * 1000);

		pcUser->uAntiCheat = psPacket->iMountTimer;
	}
	else
	{
		if ( pcUser->uTimeAntiCheat < TICKCOUNT )
		{
			PacketLogCheat s;
			s.LParam = ((TICKCOUNT - pcUser->uTimeAntiCheat) / 1000) + 210;
			s.iCheatID = CHEATLOGID_AntiCheatOffline;
	...
	if ( (psPacket->iLength > sizeof( PacketPlayData )) && (psPacket->iPlayBuffCount > 0) )
	{
		MoveSequenceBuffer * psMoveSequenceBuffer = &((MoveSequenceBuffer *)((BYTE*)psPacket + sizeof( PacketPlayData )))[psPacket->iPlayBuffCount - 1];
		pcUser->saEquippedItems[0] = psMoveSequenceBuffer->saMoveSequenceFirst.saEquippedItems[0];
		pcUser->saEquippedItems[1] = psMoveSequenceBuffer->saMoveSequenceFirst.saEquippedItems[1];
		pcUser->saEquippedItems[2] = psMoveSequenceBuffer->saMoveSequenceFirst.saEquippedItems[2];
		pcUser->saEquippedItems[3] = psMoveSequenceBuffer->saMoveSequenceFirst.saEquippedItems[3];
	}
}
	...
	for( UINT i = 0; i < PLAYERS_MAX; i++ )
	{
		...
		if( pcUser && pcUserData && pcUser->IsConnected() )
		{
			if( pcUser->IsInGame() )
			{
				...
				//Anti Cheat Disconnect Time
				///*{
				//	if ( pcUser->uTimeAntiCheat < TICKCOUNT )
				//	{
				//		PacketLogCheat s;
				//		s.LParam = (TICKCOUNT - pcUser->uTimeAntiCheat) / 1000;
				//		s.iCheatID = CHEATLOGID_AntiCheatOffline;
				//		LOGSERVER->OnLogCheat( pcUserData, &s );

				//		pcUser->uTimeAntiCheat = TICKCOUNT + (5 * 60 * 1000);
				//	}
				//}*/
			}
				...
				if ( (iDistanceA < USER_SIGHTRANGE_FAR) && (iDistanceB < MISC_DISTANCE) && (iDistanceC < MISC_DISTANCE) )
				{
					...
					else
					{
						if ( pcUnitData->GetAnimation() != ANIMATIONTYPE_Die )
						{
							...
							sMovePacket.bSelfUpdate = false;
							sMovePacket.bOfflineShop = false;
							
							sMovePacket.saEquippedItems[0] = -1;
							sMovePacket.saEquippedItems[1] = -1;
							sMovePacket.saEquippedItems[2] = -1;
							sMovePacket.saEquippedItems[3] = -1;
...
{ //0x005501C0
	...
	if ( bChat || bStatus )
	{
		...
		PERSONALSHOPHANDLER->GetPersonalShopOfflineHandler()->OnLoopUsers( pcUser );
...
void UserServer::HandlePacket( User * pcUser, PacketGetUnitData * psPacket )
{
	//Monster
	if ( UnitData * pcUnitData = UNITSERVER->GetUnit( psPacket->iID ) )
	{
		SendUnitInfoToUser( pcUser, pcUnitData->iID, pcUnitData->sCharacterData, pcUnitData->sPosition, pcUnitData->sAngle, true );
	}
	//User
	else if ( UserData * pcUserData = USERDATABYID( psPacket->iID ) )
	{
		SendUnitInfoToUser( pcUser, pcUserData->iID, pcUserData->sCharacterData, pcUserData->sPosition, pcUserData->sAngle, false );
	}
	else
	{
		PERSONALSHOPHANDLER->GetPersonalShopOfflineHandler()->HandlePacket( pcUser, psPacket );
	}
}

void UserServer::SendUnitInfoToUser( User * pcUser, int iID, CharacterData & sCharacterData, Point3D & sPosition, Point3D & sAngle, bool bIsMonster )
{
	//Monster
	if ( bIsMonster )
	{
		...
		sPacket.sPosition = sPosition;
		sPacket.sAngle = sAngle;

		if ( pcUser->GetGameLevel() < GAMELEVEL_Two )
		{
			sPacket.sCharacterData.iMinDamage = 1000;
			sPacket.sCharacterData.iMaxDamage = 9999;
		}
	...
	else
	{
		if ( pcUser->GetGameLevel() < GAMELEVEL_Two )
		{
			...
			UINT uSizeData = 192;
			if ( sCharacterData.cNewLoad != 0 )
				uSizeData = 352;

			sPacket.iLength = sizeof( PacketUnitInfoQuick ) - (sizeof( CharacterData ) - uSizeData);

			CopyMemory( &sPacket.sCharacterData, &sCharacterData, uSizeData );
