...
BOOL CAchievementHandler::SQLHaveAchievementValue( EAchievementType iType, const ValueTaskAchievement & sValue1, const ValueTaskAchievement & sValue2, const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = FALSE;
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "SELECT ID FROM UserAchievement WHERE (AchievementID=?) AND (Value1=?) AND (Value2=?) AND (Value3=?)" ) )
		{
			...
			pcDB->BindParameterInput( 2, PARAMTYPE_String, sValue1.GetString(), STRLEN( sValue1.GetString() ) );
			pcDB->BindParameterInput( 3, PARAMTYPE_String, sValue2.GetString(), STRLEN( sValue2.GetString() ) );
			pcDB->BindParameterInput( 4, PARAMTYPE_String, sValue3.GetString(), STRLEN( sValue3.GetString() ) );
...
BOOL CAchievementHandler::CompleteAchievement( User * pcUser, EAchievementType iType,const ValueTaskAchievement & sValue1,const ValueTaskAchievement & sValue2,const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = TRUE;
...
BOOL CAchievementHandler::OnTask( User * pcUser, EAchievementTask iTask, const ValueTaskAchievement & sValue1, const ValueTaskAchievement & sValue2, const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = FALSE;
	...
	switch ( iTask )
	{
		case ACHIEVEMENTTASK_LevelUP:
			...
			{
				auto pcAchievement = GetLevelAchievement( sValue1.GetInt() );
				if ( pcAchievement )
				{
					if ( SQLHaveAchievementValue( (EAchievementType)pcAchievement->GetType(), ValueTaskAchievement(pcUser->pcUserData->sCharacterData.iClass), ValueTaskAchievement(0), ValueTaskAchievement(0)) == FALSE )
					{
						CompleteAchievement( pcUser, (EAchievementType)pcAchievement->GetType(), ValueTaskAchievement( pcUser->pcUserData->sCharacterData.iClass ), ValueTaskAchievement( 0 ), ValueTaskAchievement( 0 ) );
...
BOOL CAchievementHandler::SQLNewAchievement( User * pcUser, EAchievementType iType,const ValueTaskAchievement & sValue1,const ValueTaskAchievement & sValue2,const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = FALSE;
	...
	if ( pcDB->Open() )
	{
		if ( IsCharacterAchievement( iType ) )
		{
			if ( pcDB->Prepare( "INSERT INTO UserAchievement([AccountID], [CharacterID], [AchievementID], [Level], [Value1], [Value2], [Value3], [Date]) VALUES(?,?,?,1,?,?,?,GETDATE())" ) )
			{
				...
				pcDB->BindParameterInput( 4, PARAMTYPE_String, sValue1.GetString(), STRLEN( sValue1.GetString() ) );
				pcDB->BindParameterInput( 5, PARAMTYPE_String, sValue2.GetString(), STRLEN( sValue2.GetString() ) );
				pcDB->BindParameterInput( 6, PARAMTYPE_String, sValue3.GetString(), STRLEN( sValue3.GetString() ) );
		...
		else
		{
			if ( pcDB->Prepare( "INSERT INTO UserAchievement([AccountID], [CharacterID], [AchievementID], [Level], [Value1], [Value2], [Value3], [Date]) VALUES(?,NULL,?,1,?,?,?,GETDATE())" ) )
			{
				pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &pcUser->iAccountID );
				pcDB->BindParameterInput( 2, PARAMTYPE_Integer, &iAchievementID );
				pcDB->BindParameterInput( 3, PARAMTYPE_String, sValue1.GetString(), STRLEN( sValue1.GetString() ) );
				pcDB->BindParameterInput( 4, PARAMTYPE_String, sValue2.GetString(), STRLEN( sValue2.GetString() ) );
				pcDB->BindParameterInput( 5, PARAMTYPE_String, sValue3.GetString(), STRLEN( sValue3.GetString() ) );
...
BOOL CAchievementHandler::SQLUpdateAchievement( User * pcUser, EAchievementType iType, BOOL bIncreaseLevel,const ValueTaskAchievement & sValue1,const ValueTaskAchievement & sValue2,const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = FALSE;
...
BOOL CAchievementHandler::UpdateAchievement( User * pcUser, EAchievementType iType, BOOL bIncreaseLevel, const ValueTaskAchievement & sValue1, const ValueTaskAchievement & sValue2, const ValueTaskAchievement & sValue3 )
{
	BOOL bRet = FALSE;
...
CAchievement * CAchievementHandler::GetLevelAchievement( int iCharacterLevel )
{
	//Get Achievements
	if ( vAchievements.size() > 0 )
	{
		for ( std::vector<CAchievement *>::iterator it = vAchievements.begin(); it != vAchievements.end(); it++ )
		{
			auto pcAchievement = (*it);
			//Get Events of that Achievement
			for ( std::vector<CAchievementEvent *>::iterator itE = pcAchievement->GetEvents().begin(); itE != pcAchievement->GetEvents().end(); itE++ )
			{
				CAchievementEvent * pcAchievementEvent = (*itE);
				if ( pcAchievementEvent->IsLevelAchievement() )
				{
					if ( pcAchievementEvent->GetValueInt( 0 ) >= iCharacterLevel )
					{
