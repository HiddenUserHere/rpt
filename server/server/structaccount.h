...
//											Definitions
#define LOGIN_SUCCESS_LOGACCTEXT						"[%s] (Login Success) -> Password[%s] MacAddr[%s] PCName[%s] SerialHD[%d] VideoName[%s] VideoGUID[%d] HardwareID[%s][%s] FolderID[%d] PacketChecksum(Game[%d] Server[%d])"
#define INCORRECT_ACCOUNT_LOGACCTEXT					"Unknown_Account (Incorrect Account) -> AccountName[%s] Password[%s] MacAddr[%s] PCName[%s] SerialHD[%d] VideoName[%s] VideoGUID[%d] HardwareID[%s][%s] FolderID[%d] PacketChecksum(Game[%d] Server[%d])"
