...
BOOL PacketServer::AnalyzePacket( User * pcUser, PacketReceiving * p )
{
	...
	{
		case PKTHDR_RequestInitialData:
			SOCKETSERVER->ProcessPacketRequestInitialData( pcUser );
			break;
		...
		case PKTHDR_SaveData:
			...
			CHARACTERSERVER->HandlePacket( pcUser, (PacketCharacterRecordDataSplit *)psPacket );
			break;
		...
		case PKTHDR_GetUnitInfoData:
			...
			USERSERVER->HandlePacket( pcUser, (PacketGetUnitData*)psPacket );
			break;
		...
		case PKTHDR_PersonalShopMessage:
			PERSONALSHOPHANDLER->HandlePacket( pcUser, (PacketPersonalShopRequestMessage *)psPacket );
			break;
	...
	if ( pcUser )
	{
		...
		else if ( RAKNET_SERVER_TYPE )
		{
			if( pcUser->pcUserData )
				((CFNetServerHandler *)SOCKETSERVER->GetServerSocketHandler())->Send( pcUser->pcUserData->pcSocketData, (Packet *)pPacket, bEncrypted );
		}
	}
}
...
void PacketServer::SendPacketSocket( SocketData * pcSocketData, void * pPacket, BOOL bEncrypted )
{
	...
	{
		if( pcSocketData )
			((CFNetServerHandler *)SOCKETSERVER->GetServerSocketHandler())->Send( pcSocketData, (Packet *)pPacket, bEncrypted );
	}
}
