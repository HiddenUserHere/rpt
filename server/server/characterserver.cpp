...
BOOL CharacterServer::SQLGetCharacter( int iCharacterID, SQLCharacter * ps )
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( R"Query(SELECT ci.[ID] AS [ID],
									uin.[ID] AS [AccountID], 
									ci.[AccountName] AS [AccountName], 
									ci.[Name] AS [Name], 
									ci.[Level] AS [Level], 
									ci.[Experience] AS [Experience], 
									ci.[Gold] AS [Gold], 
									ci.[JobCode] AS [JobCode], 
									ci.[ClanID] AS [ClanID], 
									ci.[ClanPermission] AS [ClanPermission], 
									ci.[LastSeenDate] AS [LastSeenDate], 
									ci.[LastStage] AS [LastStage] 
								FROM 
									CharacterInfo ci 
								JOIN 
									UserInfo uin ON ci.[AccountName] = uin.[AccountName]
								WHERE 
									ci.[ID]=?)Query" ) )
		{
			...
			if ( pcDB->Execute() && pcDB->Fetch() )
			{
				...
				pcDB->GetData( 2, PARAMTYPE_Integer, &ps->iAccountID );
				pcDB->GetData( 3, PARAMTYPE_String, ps->szAccountName, _countof( ps->szAccountName ) );
				pcDB->GetData( 4, PARAMTYPE_String, ps->szCharacterName, _countof( ps->szCharacterName ) );
				pcDB->GetData( 5, PARAMTYPE_Integer, &ps->iLevel );
				pcDB->GetData( 6, PARAMTYPE_Int64, &ps->iEXP );
				pcDB->GetData( 7, PARAMTYPE_Integer, &ps->iGold );
				pcDB->GetData( 8, PARAMTYPE_Integer, &ps->iClass );
				pcDB->GetData( 9, PARAMTYPE_Integer, &ps->iClanID );
				pcDB->GetData( 10, PARAMTYPE_Integer, &ps->iClanPermission );
				pcDB->GetData( 11, PARAMTYPE_Time, &ps->sLastSeenDate );
				pcDB->GetData( 12, PARAMTYPE_Integer, &ps->iLastMapID );
...
BOOL CharacterServer::SQLGetCharacter( const std::string strCharacterName, SQLCharacter * ps )
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( R"Query(SELECT ci.[ID] AS [ID],
								...
								WHERE 
									ci.[Name]=?)Query" ) )
		{
			pcDB->BindParameterInput( 1, PARAMTYPE_String, strCharacterName.c_str() );
...
{
	return GetCharacterName( pcUser->pcUserData );
}
...
void CharacterServer::SetExpOnCharacterData( CharacterData & sCharacterData, INT64 iExp )
{
	LARGE_INTEGER liEXP;
	liEXP.QuadPart = iExp;
...
INT64 CharacterServer::GetExpFromCharacterData( const CharacterData & sCharacterData )
{
	LARGE_INTEGER liEXP;
...
void CharacterServer::OnLoseExp( UserData * pcUserData, PacketLoseExperience * psPacket )
{
	...
	if ( pcUserData && pcUserData->pcSocketData && psPacket->iLevel < iLevelCap )
	{
		...
		INT64 iCurrentEXP = GetExp( pcUserData );
		psPacket->iLevel = GetLevelFromEXP( iCurrentEXP );

		INT64 iExpLevel = GetExpFromLevel( psPacket->iLevel );

		INT64 iExpCharacter = iCurrentEXP - psPacket->iSubExp;
...
void CharacterServer::CharacterCreate( UserData * pcUserData, PacketCreateCharacter * psPacket )
{
	...
	if ( pcUserData )
	{
		...
		{
			bool bFound = false;
			for ( int j = 0; j < STRLEN( ALLOWED_CHARS_NICK ); j++ )
			{
				if ( psPacket->szCharname[i] == ALLOWED_CHARS_NICK[j] )
				{
					bFound = true;
					break;
				}
		...
		if ( !FILEEXIST( szDataPath ) && bContinue )
		{
			...
			{ 
				...
				if ( pcDB->Open() )
				{
					if ( pcDB->Prepare( "INSERT INTO CharacterInfo VALUES (?, ?, ?, 1, 0, 0, 0, 0, 0, 0, GETDATE(), 0, 0, 0, 0, 0, 1, GETDATE())" ) )
					{
						char szNull[2] = { 0,0 };
...
void CharacterServer::SendCharacterDataEx( User * pcUser )
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "SELECT OldHead, FSP, ItemRightHandID, ItemLeftHandID FROM CharacterInfo WHERE ID=?" ) )
		{
			...
			if ( pcDB->Execute() )
			{
				if ( pcDB->Fetch() )
				{
					...
					pcDB->GetData( 3, PARAMTYPE_Integer, &s.sData.eWeaponID );
					pcDB->GetData( 4, PARAMTYPE_Integer, &s.sData.eShieldID );
				}
			}
		}
		...
		//Action Bar
		{
			...
			pcDB->Clear();
			if ( pcDB->Prepare( "SELECT ActionBarData FROM CharacterSettings WHERE CharacterID=?" ) )
			{
				pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &pcUser->iCharacterID );
				...
				if ( pcDB->Execute() )
				{
					if ( pcDB->Fetch() )
					{
						pcDB->GetData( 1, PARAMTYPE_Binary, &s.sActionBarData, sizeof( ActionBarData ) );
			...
			if ( bExists == false )
			{
				for( int i = 0; i < _countof( s.sActionBarData.iaBuffer ); i++ )
					s.sActionBarData.iaBuffer[i] = -1;

				s.sActionBarData.sWindowX = -1;
				s.sActionBarData.sWindowY = -1;

				s.sActionBarData.bOpen = true;

				pcDB->Clear();
				if ( pcDB->Prepare( "INSERT INTO CharacterSettings([CharacterID], [ActionBarData]) VALUES(?,?)" ) )
				{
					pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &pcUser->iCharacterID );
					pcDB->BindParameterInput( 2, PARAMTYPE_Binary, &s.sActionBarData, sizeof( ActionBarData ) );
...
void CharacterServer::GetCharacterWeaponShieldID( const char * pszCharacterName, int & iWeaponID, int & iShieldID )
{
	iWeaponID = ITEMID_None;
	iShieldID = ITEMID_None;

	SQLConnection * pcDB = SQLCONNECTION( DATABASEID_UserDB );
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "SELECT ItemRightHandID, ItemLeftHandID FROM CharacterInfo WHERE Name=?" ) )
		{
			...
			if ( pcDB->Execute() )
			{
				if ( pcDB->Fetch() )
				{
					pcDB->GetData( 1, PARAMTYPE_Integer, &iWeaponID );
					pcDB->GetData( 2, PARAMTYPE_Integer, &iShieldID );
				}
			}
		}
		pcDB->Close();
...
void CharacterServer::OnCharacterDataExSave( User * pcUser )
{
	...
	if ( pcDB->Open() )
	{
		if ( pcDB->Prepare( "UPDATE CharacterInfo SET FSP=?, ItemRightHandID=?, ItemLeftHandID=? WHERE Name=?" ) )
		{
			pcDB->BindParameterInput( 1, PARAMTYPE_Integer, &pcUser->sCharacterDataEx.iFSP );
			pcDB->BindParameterInput( 2, PARAMTYPE_Integer, &pcUser->sCharacterDataEx.eWeaponID );
			pcDB->BindParameterInput( 3, PARAMTYPE_Integer, &pcUser->sCharacterDataEx.eShieldID );
			pcDB->BindParameterInput( 4, PARAMTYPE_String, GetCharacterName( pcUser->pcUserData ), STRLEN( GetCharacterName( pcUser->pcUserData ) ) );
			pcDB->Execute();
		}
		pcDB->Close();
	}
...
const char * CharacterServer::GetServerName( User * pcUser )
{
...
int CharacterServer::SQLGetCharacterID( const char * pszName )
{
	int iID = -1;
...
void CharacterServer::HandlePacket( User * pcUser, PacketCharacterDataEx * psPacket )
{
	...
	{	
		...
		pcDB->Clear();
		if ( pcDB->Prepare( "UPDATE CharacterSettings SET [ActionBarData]=? WHERE [CharacterID]=?" ) )
		{
			pcDB->BindParameterInput( 1, PARAMTYPE_Binary, &psPacket->sActionBarData, sizeof( ActionBarData ) );
...
void CharacterServer::HandlePacket( User * pcUser, PacketCharacterRecordDataSplit * psPacket )
{
	...
	{
		CHARACTERDATAHANDLER->HandlePacket( pcUser, psPacket );
...
{
	return CALL_WITH_ARG1( 0x00446050, (DWORD)psCharacterData );
}

NAKED BOOL CharacterServer::IsValidCharacterData( CharacterData * psCharacterData )
{
	JMP( 0x00446080 );
}
...
NAKED DWORD CharacterServer::GetHashCharacterDataOld( CharacterData * psCharacterData )
{
	JMP( 0x0044BA70 );
}
