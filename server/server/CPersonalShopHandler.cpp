...
{
	pcPersonalShopOfflineHandler = new CPersonalShopOfflineHandler();
}


CPersonalShopHandler::~CPersonalShopHandler()
{
	DELET( pcPersonalShopOfflineHandler );
}

void CPersonalShopHandler::Init()
{
	pcPersonalShopOfflineHandler->Init();
}

void CPersonalShopHandler::Shutdown()
{
	pcPersonalShopOfflineHandler->Shutdown();
}
...
void CPersonalShopHandler::HandlePacket( User * pcUser, PacketPersonalShopRequestMessage * psPacket )
{
	if ( UserData * pcUserData = USERDATABYID( psPacket->iID ) )
	{
		if ( pcUserData->iPersonalShop == psPacket->iPersonalShopCode )
		{
			CHATSERVER->SendPersonalShopChat( pcUser->pcUserData, pcUserData->iID, "%s", pcUserData->szPersonalShopMessage );
		}
	}
	else
	{
		GetPersonalShopOfflineHandler()->HandlePacket( pcUser, psPacket );
	}
