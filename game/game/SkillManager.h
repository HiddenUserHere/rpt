...
#define SKILL_LEFTBUTTON_INDEX				(*(int*)0x0367DF98)
#define SKILL_RIGHTBUTTON_INDEX				(*(int*)0x0367DF9C)
#define SKILL_LEFTSIDE						(*(Skill**)0x03673B68)
#define SKILL_RIGHTSIDE						(*(Skill**)0x03673B6C)
...
#define SKILLS_ICON							((EXESurfaceOld**)(0x00367E148 + 0x198))
#define SKILLS_ICON_GRAY					((EXESurfaceOld**)(0x00367E148 + 0x144))
...
struct SkillData
{
	const char						* pszSkillName;
	int								  iSkillCode;
};

struct SkillSoundData
{
	const char						* pszSkillSoundFile;
	int								iID;
};
...
public:
	...
	Skill										* GetSkillHover( bool bIncludePassiveSkill );
	Skill										* GetOnlySkillHoverLeveled( bool bIncludePassiveSkill );

	void										ResetSkillsShortcut();
	void										ResetSkillShortcut( int iSkillIndex );

	void										SetUseSkillSlot( int iSkillIndex, ESkillType eSlot );

	void										SetSkillShortcut( int iSkillIndex, int iShortKey, int iMouseIndexPosition );

	BOOL										SkillKeyDown( int iKeyDown );
	...
	Skill										* GetSkill( UINT iSkillID );
	Skill										* GetSkillByIndex( UINT iSkillIndex );
	...
	static void									SetForceOrbParticle( UnitData * pcUnitData, int iTime, int iCode );

	BOOL										CancelSkill( int iSkillID );	
	...
	bool										IsLeftSkill( int iIndex );
	bool										IsRightSkill( int iIndex );
	...
	void										OnLoadSkillsIcon();

	void										OnRenderShortKeys( int iSkillStartIndex );

	bool										CanLoopSkill( ECharacterClass eCharacterClass, const ActiveSkill & sActiveSkill );

	static BOOL									ChecksumSkills();
};
