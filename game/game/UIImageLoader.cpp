...
UI::Image * ImageLoader::LoadImage( const char * pszFile, ... )
{
	...
	if ( fp )
	{
		...
		{
			if ( !EXETexture::DecryptBMP( pBuffer, uFileSize ) )
			{
				DELET( pBuffer );
				return NULL;
			}
		...
		{
			if ( !EXETexture::DecryptTGA( pBuffer, uFileSize ) )
			{
				DELET( pBuffer );
				return NULL;
			}			
		...
		{
			if ( EXETexture::EncryptDecryptTextureBuffer( pBuffer, uFileSize, ENCRYPT_HASH_PNGPT, false ) == false )
				EXETexture::DecryptPNG( pBuffer, uFileSize );
		}
