...
void CMessageBox::SetItemPriceMax( int iMinPrice, int iMaxPrice, int iCurrentPrice )
{
	if ( iBoxType == BOXTYPEID_PersonalShop )
	{
		auto pInputField = pWindow1->GetElement<UI::InputField>( INPUTID_ItemPrice );
		pInputField->SetIsNumber( TRUE, iMinPrice, iMaxPrice );

		if ( iCurrentPrice > 0 )
			pInputField->SetText( FormatString( "%d", iCurrentPrice <= iMaxPrice ? iCurrentPrice : 1 ) );
	}
...
int CMessageBox::GetCountItem()
{
	if ( (iBoxType == BOXTYPEID_PersonalShop) || (iBoxType == BOXTYPEID_PersonalShopBuy) )
	{
		if ( auto pCounter = pWindow1->GetElement<UI::Counter>( COUNTID_Quantity ) )
			return pCounter->GetCurrent();
	}
...
void CMessageBox::SetBoxType( int iType )
{
	...
	else if ( iBoxType == BOXTYPEID_PersonalShop )
	{
		...
		pWindow1->GetElement<UI::CheckBox>( CHECKBOXID_CanBuyItem )->SetSelected( FALSE );
	}
...
void CMessageBox::Init()
{
	...
	//Window Sell Item
	{
		...
		pCheckBoxBuyItem->SetSelected( FALSE );
		...
		pCheckSellByCreditsItem->SetSelected( FALSE );
