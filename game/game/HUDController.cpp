...
BOOL HUDController::UpdateTargetInfo( UnitData * pcUnitData )
{
    ...
    if ( pTextRange )
    {
		...
		int iDistanceUnit = (int)sqrt( iDistanceA );
		if( iDistanceUnit > 0 )
			pTextRange->SetTextEx( "%d (%dm)", iDistanceUnit, iDistanceUnit / 27 );
		else
			pTextRange->SetText( "" );
    }
	...
	if( pTextLife )
	{
		bool bIsNPC = pcUnitData->sCharacterData.iType == CHARACTERTYPE_NPC;
	...
	if( pTextLevel )
	{
		...
		if( pcUnitData->sCharacterData.iType == CHARACTERTYPE_Player || pcUnitData->iCharacterType == CHARACTERTYPE_Player )
			...
			pTextLevel->SetText( strLevel.c_str() );
	}
...
int HUDController::GetShortcutSkill( int iShortcut )
{
	...
	{
		if( USESKILLS.saUseSkill[i].iShortKey == iShortcut )
			return i;
	}
