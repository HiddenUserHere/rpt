...
int TOTAL_ITEM_STAGE_GAME = 0;

extern DropItemView saItemsStage[MAX_ITEM_STAGE_GAME];

UI::Image * pcLowHPWarningImage = nullptr;
UI::Image * pcBossFixedPositionImages[4] = { 0 };

float GLOBAL_FPS_UPDATE_TIME = 0;

inline int GetValueFromGameBaseTime( int iValue )
{
	const float FPS_MULT_COUNTER = 1.0f; //GLOBAL_FPS_UPDATE_TIME / (1000.0f / 60.0f);

	return (int)((float)iValue * FPS_MULT_COUNTER);
}

NAKED inline int GetAngleByMousePosition()
{
	JMP( 0x00417110 );
}
...
NAKED void __cdecl SetTerrainEffect( int iX, int iY, int iZ, int iEffectType )
{
	JMP( 0x00565850 );
}
...
void RenderShadowMap()
{
	...
	for( int i = 0; i < MAX_UNITS; i++ )
	{
		...
		{
			bool bRender = true;
			if ( (pc->iVanishTimeLeft > 0) || (pc->iBlindSkill > 0) )
			{
				if ( pc->sRenderPoint.iZ >= 0x30000 )
					bRender = false;
			}
...
BOOL Render3D( int iX, int iY, int iZ, int iAX, int iAY, int iAZ )
{
	...
	auto v = Delta3D::Math::Vector3( 1.0f, 1.0f, -0.75f );
	...
	TOTAL_ITEM_STAGE_GAME = 0;

	for ( int i = 0; i < MAX_ITEM_STAGE_GAME; i++ )
	{
		DropItemView * ps = saItemsStage + i;

		if ( ps->bActive )
		{
			TOTAL_ITEM_STAGE_GAME++;
	...
	for ( int i = 0; i < CHARACTERMESSAGE_COUNT; i++ )
	{
		...
		if ( ((pc->szMessage[0] || pc->iPersonalShop) && (pc->sRenderPoint.iZ < (64 * 64 * 256)) || pc == UNITDATA) && (!BATTLEROYALEHANDLER->InWaiting() && !BATTLEROYALEHANDLER->InBattle()) )
		{
			...
			if ( pc->szMessage[0] )
			{
				...
				MESSAGEBALLOON->SetHover( bSelected );
				...
				else
				{
					...
					if ( pszClanName )
					{
						...
						if ( ACHIEVEMENTHANDLER->HaveAchievement( pcUnit, ACHIEVEMENTTYPE_LegacyMember ) )
							MESSAGEBALLOON->AddExtraText( 1, UI::EPositionText::POSITIONTEXT_Middle, "[Legacy Member]", D3DCOLOR_XRGB( 139, 69, 19 ) );
...
int __stdcall RenderStageMesh( EXEStageModel * psStageModel )
{
	...
	psStageModel->pcModel->pcNewModel->SetPositionRotation( &Math::Vector3( 0, 0, 0 ), &Math::Vector3Int( 0, 0, 0 ) );

	psStageModel->pcModel->pcNewModel->Animate( iFrame );
	...
	if ( psStageModel->pcModel->pcNewModel->Render() )
		iRenderedCount++;
...
Stage * LoadStageFromField( BaseMap * pcBaseMap1, BaseMap * pcBaseMap2 )
{
	if ( pcBaseMap1 == FIELDGAME[0] )
		return STAGE1;
	else if ( pcBaseMap1 == FIELDGAME[1] )
		return STAGE2;

	Stage * pcStage = new Stage;

	pcStage->sVectorLight.iX = GAMECONFIG_MAPLIGHT_VECTOR->iX;
	pcStage->sVectorLight.iY = GAMECONFIG_MAPLIGHT_VECTOR->iY;
	pcStage->sVectorLight.iZ = GAMECONFIG_MAPLIGHT_VECTOR->iZ;

	pcStage->iBright = GAMECONFIG_MAPLIGHT_BRIGHT;
	pcStage->iContrast = GAMECONFIG_MAPLIGHT_CONTRAST;

	Game::AddStageLoader( pcStage, pcBaseMap1->szStageFilePath );
...
//	GRAPHICENGINE->GetSRenderer()->PushWorldMatrix( mMatrixWorld );
	...
	if ( pcUnitData->psBody )
	{
		...
		if ( (pcUnitData->iAnimationMode & 1) && bRenderUnitDataBody )
		{
			...
			else
			{
				...
				pcBodyMeshContainer->pcNewModel->SetDiffuseColor( Math::Color( 255, 255, 255, 255 + pcUnitData->iTransparencySelf ) );

				if ( pcUnitData->pcBody->m_ModelsGroup )
				{
					if ( pcUnitData->pcBody->m_ModelsGroup->HighModel.meshes.size() > 0 )
						pModelsGroup = &pcUnitData->pcBody->m_ModelsGroup->HighModel;
					else if( pcUnitData->pcBody->m_ModelsGroup->DefaultModel.meshes.size() > 0 )
						pModelsGroup = &pcUnitData->pcBody->m_ModelsGroup->DefaultModel;
					else
						pModelsGroup = &pcUnitData->pcBody->m_ModelsGroup->LowModel;
				}
				else
	...
	if ( pcUnitData->pcHead && pcHeadMeshContainer && pcHeadMeshContainer->pcNewModel && (pcUnitData->iAnimationMode & 2) && bRenderUnitDataHead )
	{
		...
		pcHeadMeshContainer->pcNewModel->SetDiffuseColor( Math::Color( 255, 255, 255, 255 + pcUnitData->iTransparencySelf ) );
...
//	GRAPHICS->GetRenderer()->PopWorldMatrix();
	...
	if ( (pcUnitData->sCharacterData.iType == CHARACTERTYPE_Player || pcUnitData->iCharacterType == CHARACTERTYPE_Player) && bRenderUnitDataWeapon )
	{
			...
			if ( pcUnitData->sLeftHandTool.pModel )
			{
				...
				else
				{
					if ( pcUnitData->sLeftHandTool.pBone )
					{
						...
						{
							if ( ItemID( pcUnitData->sLeftHandTool.eItemID ).ToItemType() == ITEMTYPE_Dagger )
							{
								EXEMatrixI sMatrix;
				...
				else
				{
					...
					if ( pcBoneWeapon && pcModelWeapon->pcNewModel )
					{
						...
						if ( (pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Shaman) && (pcUnitData->eToolEquipModeTown > 0) && (pcUnitData->eToolEquipModeTown <= TOOLEQUIPMODETOWN_BackBow) )
						{
							...
							{
								if ( pcUnitData->pcBoneShamanBackWeapon )
								{
									pcBoneWeapon = pcUnitData->pcBoneShamanBackWeapon;
								}
							}
						}
		...
		else
		{
			...
			auto UpdateDaggerMatricesTown = [pcUnitData, &sAngle](PTModel * pcModelWeapon, PTMesh * pcBoneWeapon, int iDaggerIndex) -> bool
			{
				//Assassin & Dagger? ( Town )
				if ( pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Assassin )
				{
					EItemID eItemID = iDaggerIndex == 0 ? pcUnitData->sRightHandTool.eItemID : pcUnitData->sLeftHandTool.eItemID;

					if ( ItemID( eItemID ).ToItemType() == ITEMTYPE_Dagger )
					{
						PTMesh * pcBone = pcBoneWeapon;
						if ( pcBone )
						{
							EXEMatrixI sMatrix;
							EXEMatrixIdentityI( sMatrix );

							if ( pcUnitData->eWeaponItemID == eItemID && iDaggerIndex == 1 )
								EXEMatrixRotateZI( sMatrix, PTANGLE_180 & PTANGLE_Mask );
			...
			bool bHandledRenderWeapons = false;
			...
			if ( pcUnitData->eWeaponItemID )
			{
				...
				if ( pcModelWeapon && pcModelWeapon->pcNewModel )
				{
					...
					{
						auto pcBoneWeapon = pcUnitData->pcaBackBone[pcUnitData->eToolEquipModeTown - 1];
						...
						if ( ItemID( pcUnitData->eWeaponItemID ).ToItemID() == ITEMID_XmasCostumeOldBow )
						{
							...
							if ( pcBoneWeapon->pcNewMesh )
							{
								pcBoneWeapon->pcNewMesh->Animate( pcModelWeapon->uFrame, Delta3D::Math::Vector3Int( sAngle.iX, sAngle.iY, sAngle.iZ ) );
								...
								pcBoneWeapon->pcNewMesh->world = sMatrix * pcBoneWeapon->pcNewMesh->world;
							}
						}
					...
					//Assassin & Dagger? ( Town )
					{
						PTModel * pcModelWeaponOld = pcModelWeapon;

						pcModelWeapon = pcUnitData->sRightHandTool.pModel;
						if ( UpdateDaggerMatricesTown( pcModelWeapon, pcUnitData->pcaWeaponDaggerBone[0], 0 ) )
						{
							ResetItemTextureColor();
							...
							bHandledRenderWeapons = true;
						}


						//Assassin & Dagger? ( Town )
						pcModelWeapon = pcUnitData->sLeftHandTool.pModel;
						if ( UpdateDaggerMatricesTown( pcModelWeapon, pcUnitData->pcaWeaponDaggerBone[1], 1 ) )
						{
							ResetItemTextureColor();
						...
						pcModelWeapon = pcModelWeaponOld;
					}

					if ( bHandledRenderWeapons == false )
					{
						if ( pcUnitData->sLeftHandTool.eItemID == pcUnitData->eWeaponItemID )
						{
			...
			if ( bHandledRenderWeapons == false && pcUnitData->sLeftHandTool.pModel && pcUnitData->eWeaponItemID != pcUnitData->sLeftHandTool.eItemID )
			{
				pcModelWeapon = pcUnitData->sLeftHandTool.pModel;
				...
				if ( UpdateDaggerMatricesTown( pcModelWeapon, pcUnitData->pcaWeaponDaggerBone[1], 1 ) == false )
				{
					if ( pcUnitData->sLeftHandTool.pBone )
...
BOOL ClientUnitDataUpdate( UnitData * pcUnitData )
{
	...
	if ( pcUnitData->iMoveSequenceCounter == pcUnitData->iMoveSequenceEnd )
	{
			...
			(pcUnitData->GetAnimation() != ANIMATIONTYPE_Attack) && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Skill) && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Revive) )
		{
			if ( pcUnitData->IsPlayer() )
			{
					...
					(pcUnitData->GetAnimation() == ANIMATIONTYPE_Flinch) )
				{
					...
					{
						int iMovementSpeed = GetValueFromGameBaseTime( 250 + 10 * pcUnitData->sCharacterData.iMovementSpeed );
		...
		pcUnitData->iFrame += GetValueFromGameBaseTime( 80 );
	...
	else
	{
		...
		if ( psMoveSequence->iFrame < 0xFFFF0000 )
		{
			...
			//pcUnitData->iFrame += GetValueFromGameBaseTime( pcUnitData->iFPS );
		...
		pcUnitData->iFPS = GetValueFromGameBaseTime( 80 );
		...
		UpdateEquippedItems( pcUnitData, psMoveSequence->saEquippedItems );

		pcUnitData->iMoveSequenceCounter += GetValueFromGameBaseTime( 1 );
...
BOOL SendSingleDamageUnit( UnitData * pcUnitData, int iDamageMin, int iDamageMax, int iState, int iResistance, int iSkillID, BOOL bChance, SocketData * pcSocketData )
{
		...
		switch ( pcUnitData->sCharacterData.iType )
		{
			case CHARACTERTYPE_Monster:
				...
				if ( pcUnitData->IsPlayer() == FALSE )
				{
					...
					{
						//DAMAGEINFOHANDLER->SetDefenseInfo()
						return FALSE;
					}
				}
				break;
	...
	//Scratch Attack Rating
	if ( UNITDATA->sCharacterData.iClass == CHARACTERCLASS_Shaman )
	{
		if ( iSkillID == SKILLID_Scratch )
		{
			sPacket.iAttackRating += (sPacket.iAttackRating * iaShaT34AttackRatingBoost[UNITDATA->sActiveSkill.cLevel - 1]) / 100;
		}
	}
...
void UpdateEquippedItems( UnitData * pcUnitData, short saNewEquippedItems[4] )
{
	...
	{
		//Left Hand
		if ( saNewEquippedItems[1] != pcUnitData->saEquippedItems[1] )
		{
			if ( saNewEquippedItems[1] >= 0 )
			{
				ITEMHANDLER->EquipItem( pcUnitData, ITEMHANDLER->FindItemTable( saNewEquippedItems[1] )->sBaseItemID.ToItemID(), 2 );
		...
		//Right Hand
		if ( saNewEquippedItems[2] != pcUnitData->saEquippedItems[2] )
		{
			if ( saNewEquippedItems[2] >= 0 )
			{
				ITEMHANDLER->EquipItem( pcUnitData, ITEMHANDLER->FindItemTable( saNewEquippedItems[2] )->sBaseItemID.ToItemID(), 1 );
		...
		//Body
		if ( saNewEquippedItems[0] != pcUnitData->saEquippedItems[0] )
		{
			ITEMHANDLER->EquipBody( pcUnitData, saNewEquippedItems[0] );
			pcUnitData->saEquippedItems[0] = saNewEquippedItems[0];
		}
}
}
...
void RenderLightStage( LPDIRECT3DTEXTURE9 psTexture, Point3D sPosition )
{
...
{
	for ( int i = 0; i < pcStage->iLightCount; i++ )
	{
		StageLight * psLight = pcStage->pLight + i;
		if ( (psLight->iType & 0x00002) && (!(psLight->iType & 0x00001) || NIGHT_MODE) )
		{
			EXERender::Billboard bb;
			
			bb.x = UNITDATA->sPosition.iX;
			bb.y = UNITDATA->sPosition.iY;
			bb.z = UNITDATA->sPosition.iZ;
			...
			RENDERER->AddBillboardHorizontalRotation( &bb, 1024.0f, 0.0f, 0.0f );
		}
	}
}
...
BOOL __stdcall SetBoneMeshes( EXEModel * pcModel )
{
	...
	pcUnitData->pcBodyMeshContainer = pcModel;

	EXEModel * pcBone = pcModel->pcBoneContainer ? pcModel->pcBoneContainer : pcModel;

	pcUnitData->pcBoneShamanBackWeapon = pcBone->GetMesh( "Bip P-F01" );
	pcUnitData->sRightHandTool.pBone = pcBone->GetMesh( "Bip weapon01" );
	
	if ( ItemID( pcUnitData->sLeftHandTool.eItemID ).ToItemType() == ITEMTYPE_Shield )
	{
		pcUnitData->sLeftHandTool.pBone = pcBone->GetMesh( "Bip01 L Forearm" );
	}
	else if ( (pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Assassin) && 
		(ItemID( pcUnitData->sLeftHandTool.eItemID ).ToItemType() == ITEMTYPE_Dagger) )
	{
		pcUnitData->sLeftHandTool.pBone = pcBone->GetMesh( "Bip weapon05" );
	}
	else
	{
		pcUnitData->sLeftHandTool.pBone = pcBone->GetMesh( "Bip01 L Hand" );
	}

	pcUnitData->pcaBackBone[0] = pcBone->GetMesh( "Bip in01" );
	pcUnitData->pcaBackBone[1] = pcBone->GetMesh( "Bip in-cro" );
	pcUnitData->pcaBackBone[2] = pcBone->GetMesh( "Bip in-bow" );
	
	pcUnitData->pcShieldBone = pcBone->GetMesh( "Bip-shield" );

	pcUnitData->pcTargetBoneFighterT5 = pcBone->GetMesh( "Bip01 TargetBone" );

	pcUnitData->pcAttackBone = nullptr;

	if( (pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Archer) || (pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Atalanta) )
		pcUnitData->pcAttackBone = pcBone->GetMesh( "Bip01 L Foot" );

	if ( pcUnitData->sCharacterData.iClass == CHARACTERCLASS_Magician )
		pcUnitData->pcAttackBone = pcBone->GetMesh( "Bip01 Head" );

	pcUnitData->pcaWeaponDaggerBone[0] = pcBone->GetMesh( "Bip in07" );
	pcUnitData->pcaWeaponDaggerBone[1] = pcBone->GetMesh( "Bip in08" );
	
	pcUnitData->pcBoneContainer = pcBone;
	
	pcUnitData->iModelHeight = pcModel->uMeshHeight;
	pcUnitData->iModelWidth = (pcModel->uMeshWidth * 160) >> 8;
...
NAKED void __cdecl StartSkillLoop( int iSkillID, UnitData * pcUnitData )
{
	JMP( 0x0040F460 );
}
...
NAKED void __cdecl AddFootstepEffect( UnitData * pcUnitData )
{
	JMP( 0x00543E60 );
}

void SetUnitDataTerrainEffects( UnitData * pcUnitData )
{
	if ( pcUnitData->bWalking )
	{
		bool bIsIceFloor = false;

		if ( (pcUnitData->iLoadedMapIndex >= 0) && FIELDGAME[pcUnitData->iLoadedMapIndex] )
		{
			int iMapID = FIELDGAME[pcUnitData->iLoadedMapIndex]->iMapID;
			if ( (iMapID != MAPID_AcasiaForest) && (iMapID != MAPID_BambooForest) &&
				(iMapID != MAPID_ValleyOfTranquility) && (iMapID != MAPID_LandOfDusk) &&
				(iMapID != MAPID_MushroomCave) && (iMapID != MAPID_DarkSanctuary) &&
				(iMapID != MAPID_Bellatra) && (iMapID != MAPID_BlessCastle) &&
				(iMapID != MAPID_GreedyLake) )
				pcUnitData->InvalidateWaterHeight();

			if ( (UNITDATA == pcUnitData) && (FIELDGAME[pcUnitData->iLoadedMapIndex]->iMapType == MAPTYPE_Ice) )
				bIsIceFloor = true;
		}
		auto UpdateTerrainEffects = [pcUnitData, bIsIceFloor]( int iEventID )-> void
		{
			if ( pcUnitData->GetFrameEvent( iEventID ) )
			{
				int iEventFrame = pcUnitData->GetFrameEvent( iEventID );
				int iBeginFrame = pcUnitData->GetAnimationBeginFrame();
				int iEndFrame	= pcUnitData->GetAnimationEndFrame();

				int iCompareFrame = pcUnitData->iFrame - (iBeginFrame * 160);
				if ( (iEventFrame > (iCompareFrame - pcUnitData->iFPS)) && (iEventFrame <= iCompareFrame) )
				{
					AddFootstepEffect( pcUnitData );

					if ( pcUnitData->HasWaterHeight() && (pcUnitData->GetWaterHeight() > pcUnitData->sPosition.iY) &&
						(pcUnitData->GetWaterHeight() < (pcUnitData->sPosition.iY + 32 * 256)) && (pcUnitData->iLoadedMapIndex >= 0) )
					{
						GetMoveLocation( 0, 0, 8 << 8, 0, pcUnitData->sAngle.iY, 0 );

						SetTerrainEffect( pcUnitData->sPosition.iX + RADIAN3D_X, pcUnitData->GetWaterHeight() + 128, pcUnitData->sPosition.iZ + RADIAN3D_Z, 60000 );
					}
					else if ( bIsIceFloor )
					{
						GetMoveLocation( 3 << 8, 0, 5 << 8, 0, pcUnitData->sAngle.iY, 0 );

						pcUnitData->SetIceFootstep( 1, TICKCOUNT, RADIAN3D_X, RADIAN3D_Z );
					}
				}
			}
		};

		UpdateTerrainEffects( 0 );
		UpdateTerrainEffects( 1 );
	}
}

BOOL __cdecl OnRespawnUnitSelf( UnitData * pcUnitData )
{
	bool bRet = false;
...
void CheckForYahooClan( UnitData * pcUnitData )
{
	if ( pcUnitData->GetAnimation() == ANIMATIONTYPE_Yahoo )
	{
		if ( pcUnitData->sCharacterData.iClanID )
		{
			PacketSimple sPacket;
			sPacket.iLength = sizeof( PacketSimple );
			sPacket.iHeader = PKTHDR_YahooClan;
			sPacket.iUnk2 = TICKCOUNT;
			sPacket.iUnk = 0;
			sPacket.iUnk3 = 0;
			sPacket.iUnk4 = 0;
			SENDPACKETG( &sPacket );
...
{
	int iUserSlowSpeed = 128;

	if ( CanHandleMovementFrames( pcUnitData ) )
	{
		if ( CGameInventory::GetInventorySideIndex() )
		{
			CGameInventory::GetInstance()->ChangeInventorySide( CGameInventory::GetInventorySideIndex() );

			CGameInventory::GetInstance()->SetInventorySideIndex( 0 );
		}

		if ( CGameInventory::GetIsChangeCostumeSet() )
		{
			BOOL bCostumeInventoryOpen = CGameInventory::GetIsCostumeInventoryOpen() ? FALSE : TRUE;

			HUDHANDLER->GetInventoryWindow()->SetCostumeInventoryOpened( bCostumeInventoryOpen );
			CGameInventory::SetCostumeInventoryOpen( bCostumeInventoryOpen );

			CGameInventory::SetIsChangeCostumeSet( FALSE );

			int iArmorItemIndex = INVENTORYITEMSLOT[ITEMSLOT_Armor - 1].iItemIndex;
			int iCostumeItemIndex = INVENTORYITEMSLOT[ITEMSLOT_Costume - 1].iItemIndex;

			if ( iCostumeItemIndex > 0 )
			{
				//ITEMHANDLER->SetCharacterItem( ITEMID_None, ITEMSLOT_Armor, FALSE );
				ITEMHANDLER->SetCharacterItem( iCostumeItemIndex > 0 ? INVENTORYITEMS[iCostumeItemIndex - 1].sItem.sItemID.ToItemID() : ITEMID_None, ITEMSLOTFLAG_Costume, iCostumeItemIndex > 0 ? TRUE : FALSE );
			}
			else
			{
				ITEMHANDLER->SetCharacterItem( iArmorItemIndex > 0 ? INVENTORYITEMS[iArmorItemIndex - 1].sItem.sItemID.ToItemID() : ITEMID_None, ITEMSLOTFLAG_Armor, iArmorItemIndex > 0 ? TRUE : FALSE );
			}
		}

		if ( CharacterGame::GetCurrentHP() == 0 )
		{
			CharacterGame::SetCurrentHP( pcUnitData->sCharacterData.sHP.sMax / 2 );
		}

		if ( pcUnitData->iFreezeTimeLeft > 0 )
		{
			iUserSlowSpeed = pcUnitData->iUpdateMoveReduce ? pcUnitData->iUpdateMoveReduce : 156;

			if ( (pcUnitData->iUpdateMoveReduce > 200) || (pcUnitData->iUpdateMoveReduce < 0) )
				iUserSlowSpeed = 156;
		}

		if ( pcUnitData->bWalking )
		{
			int iMovementSpeed = pcUnitData->sCharacterData.iMovementSpeed;

			EXECharacter::UpdateSelfCharacterMovement( pcUnitData, iUserSlowSpeed );
		}
		else
		{
			CanHandleStopMove( pcUnitData );

			if ( pcUnitData->iFloorHeight < pcUnitData->sPosition.iY )
				pcUnitData->Move( 0 );

			pcUnitData->iFPS = GetValueFromGameBaseTime( 70 );

			if ( pcUnitData->iFrameTimer == 0 )
			{
				if ( EXECharacter::GetUnitDataColision( pcUnitData->sPosition.iX, pcUnitData->sPosition.iY, pcUnitData->sPosition.iZ ) )
				{
			...
			if ( (pcUnitData->GetAnimation() != ANIMATIONTYPE_Attack) && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Skill) && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Potion) )
			{
				extern BOOL __cdecl OpenPlaySkill( struct Skill * psSkill );

				if ( ENABLE_SKILL_LOOP && SELECTED_SKILL_MOUSE_INDEX )
				{
					if ( SELECTED_SKILL_MOUSE_INDEX == 1 )
					{
						if( SKILL_LEFTSIDE )
							OpenPlaySkill( SKILL_LEFTSIDE );
					}
	...
	if ( (pcUnitData->GetAnimation() == ANIMATIONTYPE_Attack) || (pcUnitData->GetAnimation() == ANIMATIONTYPE_Skill) )
	{
		if ( pcUnitData->bOverrideFPS == FALSE )
			pcUnitData->iFPS = (GetValueFromGameBaseTime( 80 ) * pcUnitData->iWeaponSpeed) >> 8;
		else
			pcUnitData->iFPS = pcUnitData->iNewFPS;
	}
	else if ( (pcUnitData->GetAnimation() == ANIMATIONTYPE_Flinch) || (pcUnitData->GetAnimation() == ANIMATIONTYPE_Potion) )
		pcUnitData->iFPS = GetValueFromGameBaseTime( 92 );

	if ( pcUnitData->iFreezeTimeLeft )
		pcUnitData->iFPS = (pcUnitData->iFPS * iUserSlowSpeed) >> 8;

	if ( pcUnitData->iFreezeTimeLeft > 0 )
	{
		pcUnitData->iFreezeTimeLeft -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->iFreezeTimeLeft < 0 )
			pcUnitData->iFreezeTimeLeft = 0;

		if ( (pcUnitData->iFreezeTimeLeft == 0) && pcUnitData->iIceOverlay )
			pcUnitData->iIceOverlay = (-pcUnitData->iIceOverlay);
	}

	if ( pcUnitData->bDistortion > 0 )
	{
		pcUnitData->bDistortion -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->bDistortion < 0 )
	...
	if ( pcUnitData->iVanishTimeLeft > 0 )
	{
		pcUnitData->iVanishTimeLeft -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->iVanishTimeLeft < 0 )
			pcUnitData->iVanishTimeLeft = 0;

		if ( (pcUnitData->iVanishTimeLeft == 0) && BLURTIME )
			BLURTIME = TICKCOUNT;
	}

	if ( pcUnitData->iStunTimeLeft > 0 )
	{
		pcUnitData->iStunTimeLeft -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->iStunTimeLeft < 0 )
			pcUnitData->iStunTimeLeft = 0;
	}

	if ( pcUnitData->iIceOverlay )
	{
		if ( (pcUnitData->iIceOverlay > 0) && (pcUnitData->iIceOverlay < 160) )
		{
			pcUnitData->iIceOverlay += GetValueFromGameBaseTime( 1 );
			if ( pcUnitData->iIceOverlay > 160 )
				pcUnitData->iIceOverlay = 160;
		}
		else if ( pcUnitData->iIceOverlay < 0 )
		{
			pcUnitData->iIceOverlay += GetValueFromGameBaseTime( 1 );
			if ( pcUnitData->iIceOverlay > 0 )
				pcUnitData->iIceOverlay = 0;
	...
	if ( pcUnitData->bNoDeath > 0 )
	{
		pcUnitData->bNoDeath -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->bNoDeath < 0 )
			pcUnitData->bNoDeath = 0;
	}

	if ( pcUnitData->sPoison.sMax > 0 )
	{
		if ( pcUnitData->iPoison <= 0 )
			pcUnitData->iPoison = 1;
		
		pcUnitData->sPoison.sMax -= (short)GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->sPoison.sMax < 0 )
			pcUnitData->sPoison.sMax = 0;

		if ( pcUnitData->sPoison.sMin && ((pcUnitData->sPoison.sMax & 1) == 0) && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Die) )
		{
			CharacterGame::SetCurrentHP( CharacterGame::GetCurrentHP() - pcUnitData->sPoison.sMin );
			if ( CharacterGame::GetCurrentHP() <= 0 )
				CharacterGame::SetCurrentHP( 1 );
		}
	}
	else
	{
		if ( pcUnitData->iPoison > 0 )
			pcUnitData->iPoison = (-pcUnitData->iPoison);
	}

	if ( pcUnitData->iPoison )
	{
		if ( (pcUnitData->iPoison > 0) && (pcUnitData->iPoison < 100) )
		{
			pcUnitData->iPoison += GetValueFromGameBaseTime( 1 );
			if ( pcUnitData->iPoison > 100 )
				pcUnitData->iPoison = 100;
		}
		else if ( pcUnitData->iPoison < 0 )
		{
			pcUnitData->iPoison += GetValueFromGameBaseTime( 1 );
			if ( pcUnitData->iPoison > 0 )
				pcUnitData->iPoison = 0;
	...
	if ( pcUnitData->iTalkFrame && pcUnitData->psTalkModelAnimation )
	{
		if ( pcUnitData->iTalkFrame >= (pcUnitData->psTalkModelAnimation->iEndFrame * 160) )
			pcUnitData->iTalkFrame = 0;
		else
			pcUnitData->iTalkFrame = GetValueFromGameBaseTime( 80 );
	}

	pcUnitData->iFrame += pcUnitData->iFPS;

	pcUnitData->HandleAttackEvent();

	SetUnitDataTerrainEffects( pcUnitData );

	if ( pcUnitData->GetAnimation() == ANIMATIONTYPE_Potion )
	{
		if ( pcUnitData->GetFrameEvent( 0 ) )
		{
			pcUnitData->DecreaseFrame( 95 );

			int iCompareFrame = pcUnitData->iFrame - pcUnitData->GetAnimationBeginFrame();
			if ( (pcUnitData->GetFrameEvent( 0 ) > (iCompareFrame - pcUnitData->iFPS)) && (pcUnitData->GetFrameEvent( 0 ) <= iCompareFrame) )
			{
				if ( ETHERCORE_USE_ITEMID )
					pcUnitData->iPotionStatus = POTIONSTATUS_EtherCore;
				else
				{
					if ( ITEMDATA_USED_ITEM && ITEMDATA_USED_ITEM->sItem.sItemID.ToInt() )
					{
						ItemHandler::UseItem( &ITEMDATA_USED_ITEM->sItem );
					}

					int iHeightEffect = pcUnitData->sPosition.iY + (48 << 8);

					switch ( ItemHandler::UsedPotion() )
					{
						case POTIONSTATUS_HP:
							SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, iHeightEffect, pcUnitData->sPosition.iZ, 600 );
							pcUnitData->iPotionStatus = POTIONSTATUS_HP;
							CharacterGame::ResetPotionGraph( POTIONSTATUS_HP - 1 );
							break;
						case POTIONSTATUS_MP:
							SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, iHeightEffect, pcUnitData->sPosition.iZ, 700 );
							pcUnitData->iPotionStatus = POTIONSTATUS_MP;
							CharacterGame::ResetPotionGraph( POTIONSTATUS_MP - 1 );
							break;
						case POTIONSTATUS_SP:
							SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, iHeightEffect, pcUnitData->sPosition.iZ, 800 );
							pcUnitData->iPotionStatus = POTIONSTATUS_SP;
							CharacterGame::ResetPotionGraph( POTIONSTATUS_SP - 1 );
							break;
						case POTIONSTATUS_HPMPSP:
							SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, iHeightEffect, pcUnitData->sPosition.iZ, 800 );
							SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, iHeightEffect, pcUnitData->sPosition.iZ, 700 );
							...
							pcUnitData->iPotionStatus = POTIONSTATUS_HPMPSP;
							CharacterGame::ResetPotionGraph( POTIONSTATUS_HPMPSP - 1 );
							
							//TODO: ADD EFFECT TO EVERYONE
							break;
						case POTIONSTATUS_Flinch:
							pcUnitData->Animate( ANIMATIONTYPE_Flinch );
							PlayUnitSound( pcUnitData );
							break;

						case POTIONSTATUS_Yahoo:
							pcUnitData->Animate( ANIMATIONTYPE_Yahoo );
							break;
						default:
							break;
					}
				...
				SetDynamicLight( pcUnitData->sPosition.iX, pcUnitData->sPosition.iY + (48 << 8), pcUnitData->sPosition.iZ, 50, 100, 70, 0, 250, 1 );
			}
	...
	if ( pcUnitData->iFrame >= pcUnitData->GetAnimationEndFrame() )
	{
		if ( pcUnitData->GetAnimation() == ANIMATIONTYPE_Die )
		{
			if ( RESTART_BUTTON_ID )
			{
				if ( RESTART_BUTTON_ID == RestartButtonID::RESTARTBUTTONID_Field )
				{
					if ( MAP_ID == MAPID_Bellatra )
						MapGame::TeleportMapPositionNear( MAPID_NaviskoTown, pcUnitData->sPosition.iX, pcUnitData->sPosition.iZ, &iBackX, &iBackZ );
					else
						MapGame::TeleportMapPositionNear( MAP_ID, pcUnitData->sPosition.iX, pcUnitData->sPosition.iZ, &iBackX, &iBackZ );


					//TODO Check EXP Lose
					if ( REVIVE_TIME == FALSE )
					{
				...
				if ( RESTART_BUTTON_ID == RestartButtonID::RESTARTBUTTONID_Town )
				{
					if ( MAP_ID == MAPID_BlessCastle )
					{
						if ( pcUnitData->sCharacterData.iClanID && CBlessCastleHandler::GetCastleLeaderClanID() == CBlessCastleHandler::GetClanID( pcUnitData->sCharacterData.iClanID ) )
							BLESSCASTLEHANDLER->WarpCastle( TRUE, &iBackX, &iBackZ );
						else
							BLESSCASTLEHANDLER->WarpCastle( FALSE, &iBackX, &iBackZ );
					}
					else
						MapGame::TeleportStartField( &iBackX, &iBackZ );
				...
				if ( RESTART_BUTTON_ID == RestartButtonID::RESTARTBUTTONID_Exit )
				{
				...
				{
					pcUnitData->SetPosition( iBackX, 0, iBackZ );
					*TRACECAMERA = pcUnitData->sPosition;
					*TRACETARGET = pcUnitData->sPosition;
					...
					pcUnitData->iLoadedMapIndex = -1;
					int iFloorHeight = (-32767);
					if ( STAGE1 )
					{
						iFloorHeight = STAGE1->GetHighestPoint( pcUnitData->sPosition.iX, pcUnitData->sPosition.iY, pcUnitData->sPosition.iZ, pcUnitData->pcBodyMeshContainer ? pcUnitData->pcBodyMeshContainer->uMeshHeight : 0 );
						UNITDATA->iLoadedMapIndex = 0;
					}

					if ( (iFloorHeight == (-32767)) && STAGE2 )
				...
				RESTART_BUTTON_ID = RestartButtonID::RESTARTBUTTONID_None;

				WINDOW_GAMEDEATH = FALSE;
			}
			else
			{
				pcUnitData->iFrame = pcUnitData->GetAnimationEndFrame() - GetValueFromGameBaseTime( 80 );
			}

			if ( TRADEHANDLER->IsWindowTradeOpen() && !TRADEHANDLER->IsTradeMaskTime() )
			{
				//Send Cancel Trade
			...
			if ( (pcUnitData->iFrameTimer >= (70 * 10)) && (pcUnitData->iLoadedMapIndex >= 0) )
			{
				if ( REVIVE_TIME )
				{
					OnRespawnUnitSelf( pcUnitData );
		...
		{
			if ( pcUnitData->bOverrideFPS )
			{
				pcUnitData->bOverrideFPS -= GetValueFromGameBaseTime( 1 );
				if ( pcUnitData->bOverrideFPS < 0 )
					pcUnitData->bOverrideFPS = 0;
			}

			CheckForYahooClan( pcUnitData );

			if ( pcUnitData->GetAnimation() == ANIMATIONTYPE_Potion )
			{
				if ( ETHERCORE_USE_ITEMID )
				{
					EMapID eMapID = MAP_ID;
					if ( (eMapID != MAPID_Bellatra) && (eMapID != MAPID_T5QuestArena) && (eMapID != MAPID_QuestArena) )
					{
						switch ( ETHERCORE_USE_ITEMID )
						{
							...
							case ITEMID_AtlantisCore:
								...
								if ( ETHERCORE_USE_ITEMID == ITEMID_RicartenCore )
									MapGame::TeleportStartField( MAPID_RicartenTown, &pcUnitData->sPosition.iX, &pcUnitData->sPosition.iZ );
								else if ( ETHERCORE_USE_ITEMID == ITEMID_NaviskoCore )
									MapGame::TeleportStartField( MAPID_NaviskoTown, &pcUnitData->sPosition.iX, &pcUnitData->sPosition.iZ );
								else if ( ETHERCORE_USE_ITEMID == ITEMID_PillaiCore )
									MapGame::TeleportStartField( MAPID_PhillaiTown, &pcUnitData->sPosition.iX, &pcUnitData->sPosition.iZ );
								else
									MapGame::TeleportStartField( MAPID_Atlantis, &pcUnitData->sPosition.iX, &pcUnitData->sPosition.iZ );

								pcUnitData->SetPosition( pcUnitData->sPosition.iX, 0, pcUnitData->sPosition.iZ );
								break;
							default:
								break;
						}
						...
						*TRACETARGET = pcUnitData->sPosition;
						...
						SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, pcUnitData->sPosition.iY + (48 << 8), pcUnitData->sPosition.iZ, 500);
						SKILLMANAGERHANDLER->SkillPlaySound( 0x1000, pcUnitData->sPosition.iX, pcUnitData->sPosition.iY, pcUnitData->sPosition.iZ );
					...
					pcUnitData->bWalking = FALSE;
					ISMOUSELEFT_CLICKED = FALSE;
					pcUnitData->eNextAnimationType = ANIMATIONTYPE_None;
					SET_ETHERCORE_USE_ITEMID = ITEMID_None;
				}
			}

			if ( PACKET_VIRTUAL_LIFE )
			{
				int iHP = (pcUnitData->sCharacterData.sHP.sMax * 40) / 100;
				if ( PACKET_VIRTUAL_LIFE->iUnk2 < iHP )
					iHP = PACKET_VIRTUAL_LIFE->iUnk2;

				CharacterGame::SetCurrentHP( CharacterGame::GetCurrentHP() + iHP );
				CharacterGame::SetCurrentMP( CharacterGame::GetCurrentMP() + PACKET_VIRTUAL_LIFE->iUnk );
				CharacterGame::SetCurrentSP( CharacterGame::GetCurrentSP() + PACKET_VIRTUAL_LIFE->iUnk3 );

				DELET( PACKET_VIRTUAL_LIFE );
			}
			...
			if ( CGameInventory::GetInventorySideIndex() )
			{
				...
				if ( CGameInventory::GetIsChangeCostumeSet() )
				{
					...
					if ( bCostumeInventoryOpen )
					{
						int iCostumeItemIndex = INVENTORYITEMSLOT[ITEMSLOT_Costume - 1].iItemIndex;
						ITEMHANDLER->SetCharacterItem( iCostumeItemIndex > 0 ? INVENTORYITEMS[iCostumeItemIndex - 1].sItem.sItemID.ToItemID() : ITEMID_None, ITEMSLOT_Costume, iCostumeItemIndex > 0 ? TRUE : FALSE );
					}
					else
					{
						int iArmorItemIndex = INVENTORYITEMSLOT[ITEMSLOT_Armor - 1].iItemIndex;
						ITEMHANDLER->SetCharacterItem( iArmorItemIndex > 0 ? INVENTORYITEMS[iArmorItemIndex - 1].sItem.sItemID.ToItemID() : ITEMID_None, ITEMSLOT_Armor, iArmorItemIndex > 0 ? TRUE : FALSE );
					}
				}
			}

			if ( pcUnitData->eNextAnimationType && !pcUnitData->bOverrideFPS )
			{
				if ( pcUnitData->Animate( pcUnitData->eNextAnimationType ) == FALSE )
				{
					pcUnitData->Animate( ANIMATIONTYPE_Idle );
				}
				else if( ETHERCORE_USE_ITEMID && (pcUnitData->eNextAnimationType == ANIMATIONTYPE_Potion) )
				{
					SKILLMANAGERHANDLER->StartSkillEffectOld( pcUnitData->sPosition.iX, pcUnitData->sPosition.iY + (48 << 8), pcUnitData->sPosition.iZ, 500 );
					SKILLMANAGERHANDLER->SkillPlaySound( 0x1000, pcUnitData->sPosition.iX, pcUnitData->sPosition.iY, pcUnitData->sPosition.iZ );
				}
			...
			else
			{
				if ( (pcUnitData->GetAnimation() == ANIMATIONTYPE_Attack) || (pcUnitData->GetAnimation() == ANIMATIONTYPE_Skill) )
				{
					if ( (pcUnitData->GetAnimation() == ANIMATIONTYPE_Skill) && pcUnitData->sActiveSkill.iValue )
					{
						extern BOOL __cdecl EndSkill( class UnitData * pcThis );
						EndSkill( pcUnitData );
					}

					if ( pcUnitData->sActiveSkill.iValue && pcUnitData->bOverrideFPS )
					{
						pcUnitData->iFrame = pcUnitData->GetAnimationBeginFrame();

						StartSkillLoop( pcUnitData->sActiveSkill.iValue, pcUnitData );
					}
					else
					{
						if ( (ENABLE_SKILL_LOOP == FALSE) && (ISMOUSELEFT_CLICKED == FALSE) && (ISMOUSERIGHT_CLICKED == FALSE) &&
							UNITDATA_FOCUS && (UNITDATA_FOCUS == pcUnitData->pcTarget) )
						{
							if ( CharacterGame::CancelAttack() )
								pcUnitData->bWalking = FALSE;
						}

						if ( pcUnitData->bWalking && (FOLLOWMODE_ACTION == FALSE) )
						{
							pcUnitData->sAngle.iY = GetAngleByMousePosition();
						}
					}
				}
				else if ( pcUnitData->GetAnimation() == ANIMATIONTYPE_Potion )
				{
					if ( (ENABLE_SKILL_LOOP == FALSE) && (ISMOUSELEFT_CLICKED == FALSE) && (ISMOUSERIGHT_CLICKED == FALSE) &&
						UNITDATA_FOCUS && (UNITDATA_FOCUS == pcUnitData->pcTarget) )
					{
				...
				if ( pcUnitData->IsRepeatAnimation() )
				{
					pcUnitData->iFrame = pcUnitData->GetAnimationBeginFrame() + (pcUnitData->iFrame - pcUnitData->GetAnimationEndFrame());
				}
				else
				{
					if ( pcUnitData->bOverrideFPS == 0 )
		...
		int iAttackSpeed = pcUnitData->sCharacterData.iAttackSpeed;
		if ( iAttackSpeed == 0 )
			iAttackSpeed = 7;
		else
			iAttackSpeed += 2;

		pcUnitData->iWeaponSpeed = iAttackSpeed * 40;

		if ( pcUnitData->bIsWeaponTwoHanded )
			pcUnitData->iWeaponSpeed += 16;
	...
	if ( pcUnitData->iFireOverlay > 0 )
	{
		pcUnitData->iFireOverlay -= GetValueFromGameBaseTime( 1 );
		if ( pcUnitData->iFireOverlay < 0 )
			pcUnitData->iFireOverlay = 0;
	}
	...
	pcUnitData->saEquippedItems[3] = pcUnitData->sActiveSkill.iValue;

	if ( pcUnitData->HasWaterHeight() && (pcUnitData->GetWaterHeight() > pcUnitData->sPosition.iY) && (pcUnitData->GetWaterHeight() < (pcUnitData->sPosition.iY + (32 << 8))) &&
		(pcUnitData->iLoadedMapIndex >= 0) )
	{
		int iCount = 0x3F;
		int iDistance = 0;
		...
		if ( (pcUnitData->iLoopTimer & iCount) == 0 )
		{
			GetMoveLocation( 0, 0, iDistance << 8, 0, pcUnitData->sAngle.iY, 0 );
			
			SetTerrainEffect( pcUnitData->sPosition.iX + RADIAN3D_X, pcUnitData->GetWaterHeight() + 128, pcUnitData->sPosition.iZ + RADIAN3D_Z, 60000 );
		}
	}

	SaveCharacterFrames( pcUnitData );
}

DWORD TIME_SKILL_LOOP_TIMEOUT = 0;

BOOL __cdecl TraceAttackPlayGate()
{
	if ( KEYBOARDHANDLER->IsKeyDown( VK_SHIFT ) )
	{
		if ( TIME_SKILL_LOOP_TIMEOUT < TICKCOUNT )
		{
	...
	if ( UNITDATA_SELECTED )
	{
		FOLLOWMODE_ACTION = TRUE;

		UNITDATA_FOCUS = UNITDATA_SELECTED;

		AUTOMATIC_SKILL_ATTACK = TRUE;
	...
	if ( ITEMVIEW_SELECTED )
	{
		ITEMVIEW_AUTO_FOLLOW = ITEMVIEW_SELECTED;
