	...
	if( pListPartyMembers )
	{
		...
		for( size_t i = 0; i < MAX_PARTY_MEMBERS; i++ )
		{
			...
			if( sPartyMember.sData.iID )
			{
				...
				std::string strClass = GetCharacterClassByFlag( CharacterClassToClassFlag( (ECharacterClass)sPartyMember.cCharacterClass ) );
				...
				pImageCharacterClass->SetActiveImage( UI::ImageLoader::LoadActiveImage( std::string( "game\\images\\UI\\hud\\party\\" + strClass + ".png" ).c_str(), std::string( "game\\images\\UI\\hud\\party\\" + strClass + ".png" ).c_str() ) );
...
void CPartyWindow::ShowMessageBox( int iType, int iIndex )
{
	...
	{ 
		MESSAGEBOX->SetDescription( FormatString(AY_OBFUSCATE("Do you want to change Party to %s?"), GetPartyMode((EPartyMode)((PARTYHANDLER->GetPartyData().eMode+1)%3))) );
