...
NAKED void CGameInventory::OnMouseRDownClick( int iX, int iY )
{
	JMP( pfnOnMouseRClickDown );
}
...
BOOL CGameInventory::ChangeInventorySide( int iType )
{
			...
			if ( pc->bValid )
			{
				for ( int j = 0; j < INVENTORY_DATACOUNT; j++ )
				{
					...
					if ( pc2->bValid == FALSE )
					{
						...
						if ( UNITDATA->sCharacterData.iClass == CHARACTERCLASS_Assassin && pc->sBaseItemID.ToItemType() == ITEMTYPE_Dagger )
						{
							ITEMHANDLER->SetCharacterItem( pc->sBaseItemID.ToItemID(), pc->iItemSlot == ITEMSLOT_LeftHand ? ITEMSLOTFLAG_LeftHand : ITEMSLOTFLAG_RightHand, TRUE );
						}
						else
	...
	if ( (CGameInventory::GetIsCostumeInventoryOpen() == TRUE) && ((iType == 3) || (iType == 4)) )
	{
		HUDHANDLER->GetInventoryWindow()->SetCostumeInventoryOpened( TRUE );
	}
...
BOOL CGameInventory::GetInventoryItem( int iX, int iY, int iPickup )
{
	...
	for( size_t i = 0; i < 100; i++ )
	{
		...
		if( pcItemData )
		{
			...
			{
				if ( CGameInventory::GetIsCostumeInventoryOpen() == TRUE )
				{
				...
				if( sCurMouse.Inside( Rectangle2D( pcItemData->sPosition.iX, pcItemData->sPosition.iY, pcItemData->iWidth, pcItemData->iHeight ) ) )
				{
					...
					if( iPickup == 1 )
					{
						...
						if( pcItemData->iItemSlot )
						{
							...
							{
								ITEMHANDLER->SetCharacterItem( pcItemData->sBaseItemID.ToItemID(), pcItemData->iItemSlot == ITEMSLOT_LeftHand ? ITEMSLOTFLAG_LeftHand : ITEMSLOTFLAG_RightHand, FALSE );
							}
							else
							{
								...
								if ( (pcItemData->iItemSlot == ITEMSLOT_Costume) && (iArmorItemIndex > 0) )
									ITEMHANDLER->SetCharacterItem( INVENTORYITEMS[iArmorItemIndex - 1].sItem.sItemID.ToItemID(), ITEMSLOTFLAG_Armor, TRUE );
								else if ( (pcItemData->iItemSlot == ITEMSLOT_Armor) && (iCostumeItemIndex > 0) )
									ITEMHANDLER->SetCharacterItem( INVENTORYITEMS[iCostumeItemIndex - 1].sItem.sItemID.ToItemID(), ITEMSLOTFLAG_Costume, TRUE );
							}
