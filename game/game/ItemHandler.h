...
public:
	...
	bool										GetItemDataTableByItemID( ItemData & cItemDataOut, EItemID iItemID );
	...
	class ItemData *							FindItemTableByCode( const std::string strItemCode );
	...
	int											GetItemInventoryIndexByCode( EItemID eID );
	ItemData *									GetItemInventoryByCode( EItemID eID );
	ItemData *									GetItemInventoryByIndex( int iIndex );
	...
	const char *								GetArmorPatternName( class UnitData * pcUnit, DWORD dwCode );
	...
	static BOOL									RightClickSimulate( EItemID eItemID );

	static int									UsedPotion();
	...
	static BOOL									IsSameItem( Item * psItemOne, Item * psItemTwo );
