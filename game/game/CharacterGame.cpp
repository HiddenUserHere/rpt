...
CCharacterSaveHandler * CharacterGame::pcCharacterSaveHandlerReceive = nullptr;
CCharacterSaveHandler * CharacterGame::pcCharacterSaveHandlerSend = nullptr;
...
CharacterGame::CharacterGame()
{
	...
	pcCharacterSaveHandlerReceive = new CCharacterSaveHandler();
	pcCharacterSaveHandlerSend = new CCharacterSaveHandler();
}

CharacterGame::~CharacterGame()
{
	DELET( pcCharacterSaveHandlerSend );
	DELET( pcCharacterSaveHandlerReceive );
}
...
void CharacterGame::OnCharacterUpdateData()
{
	...
	if ( CQUESTGAME->GetQuestInstance()->IsDoneQuest( QUESTID_T5QuestArena ) )
	{
		...
		iPoints += (UNITDATA->sCharacterData.iLevel - 80) / 3;
	...
	if ( Game::GetGameMode() == GAMEMODE_InGame && UNITDATA->bActive )
	{
		...
		int iSlot = CGameInventory::GetIsCostumeInventoryOpen() ? ITEMSLOT_Costume : ITEMSLOT_Armor;
	...
	UNITDATA->sCharacterData.iMovementSpeed += MOUNTHANDLER->UpdateMovementSpeed();
...
void CharacterGame::OnCharacterSetCharInfoEx()
{
	...
	*/
	...
	{
		int iLevelSkill = SKILLMANAGERHANDLER->GetLevelSkill( SKILLID_WizardTrance );
		if ( INVENTORYITEMSLOT[ITEMSLOT_LeftHand - 1].iItemIndex > 0 )
		{
			...
			if ( psItemData && psItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Orb )
			{
				if ( iLevelSkill )
				{
					...
					*piBlock += (int)((psItemData->sItem.fBlockRating * (float)iPercentSkill) / 100.0f);
				}
			}
		}
	}
	...
	*piCritical += (*(int*)0x0362A2D0);
	...
	iEvade += MOUNTHANDLER->UpdateEvade();

	*(int *)0x04B0C210 = iEvade;
}
...
void CharacterGame::SetDecreaseAutomation()
{
	int iLevelSkill = SKILLMANAGERHANDLER->GetLevelSkill( SKILLID_Automation );
	if ( SKILLMANAGERHANDLER->IsActiveBuff( SKILLID_Automation ) )
	{
		if ( iLevelSkill )
		{
			...
			if ( psItemData && (psItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Bow || psItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Javelin) )
			{
				...
				CGameProtect::SetDecreaseMP( (CGameProtect::GetDecreaseMP() + faMechT23MPDrain[iLevelSkill - 1]) * 16.0f );
				CGameProtect::SetDecreaseSP( (CGameProtect::GetDecreaseSP() + faMechT23SPDrain[iLevelSkill - 1]) * 16.0f );
...
void CharacterGame::ResetPotionGraph( int iID )
{
	CALL_WITH_ARG1( 0x0045D910, (DWORD)iID );
}

NAKED BOOL CharacterGame::ResetInventoryGold()
{
	JMP( 0x0045DE30 );
}
...
BOOL CharacterGame::Save()
{
	...
	CharacterGame::pcCharacterSaveHandlerSend->ProcessPacket();
	...
	//CharacterGame::SendSavePacketData((PacketCharacterRecordData *)0x00CF48E0, SOCKETL);
...
BOOL CharacterGame::SendSavePacketData(PacketCharacterRecordData * psCharacterRecordData, SocketData * pcSocketData)
{
	PacketSaveData sPacket;

	BYTE * pbData = (BYTE*)psCharacterRecordData;
	...
	int iPartialSize = psCharacterRecordData->iLength / _countof(sPacket.szData);
	if ((psCharacterRecordData->iLength % _countof(sPacket.szData)) != 0)
	{
		if (psCharacterRecordData->iLength > _countof(sPacket.szData))
		{
			iPartialSize++;
		}
	}
	...
	while (true)
	{
		int iLength = psCharacterRecordData->iLength - iTotalLength;
		if (iLength > _countof(sPacket.szData))
		{
			iLength = _countof(sPacket.szData);
		}

		sPacket.iHeader = PKTHDR_SaveData;
		sPacket.iLength = iLength + 0x18;

		sPacket.iCount = iPacketCount;
		sPacket.iTotal = iPartialSize;

		sPacket.iDataSize = iLength;
		CopyMemory( &sPacket.szData, &pbData[iTotalLength], iLength);

		(*(int*)&sPacket.szDataWithChecksum[iLength]) = 1597826;

		//CheckSum
		for(int i = 0; i < iLength; i++)
		{
			(*(int*)&sPacket.szDataWithChecksum[iLength]) ^= (int)sPacket.szData[i];
		}
		...
		if (iTotalLength >= psCharacterRecordData->iLength)
		{
			break;
		}
	}
...
void CharacterGame::SaveEx()
{
	...
	//Get Right Hand ItemID
	s.sData.eWeaponID = UNITDATA->sRightHandTool.eItemID;
	s.sData.eShieldID = UNITDATA->sLeftHandTool.eItemID;

	ACTIONBARHANDLER->RecordSaveData( s.sActionBarData );
...
int CharacterGame::OnMotionChange( UnitData * pcUnitGame, int iMotion, int iAnimationID, EAnimationType eAnimationType, bool bRidingMount )
{
	...
	if ( pcUnitGame )
	{
		...
		{
			if ( (Game::GetGameMode() == GAMEMODE_InGame) && !IsStageVillage() && 
				((ItemID( pcUnitGame->sRightHandTool.eItemID ).ToItemType() == ITEMTYPE_Dagger) || (ItemID( pcUnitGame->sLeftHandTool.eItemID ).ToItemType() == ITEMTYPE_Dagger)) )
			{
		...
		if ( bRidingMount )
		{
			if ( ModelAnimation * psModelAnimation = pcUnitGame->GetModelAnimationByIndex( iMotion ) )
			{
				EAnimationType eAnimation = psModelAnimation->iType == ANIMATIONTYPE_Falling ? ANIMATIONTYPE_Idle : psModelAnimation->iType;

				if ( int iAnimationCharacterMountID = MOUNTHANDLER->GetAnimationID( pcUnitGame, eAnimation ); iAnimationCharacterMountID != -1 )
				{
					iMotion = iAnimationCharacterMountID;

					MOUNTHANDLER->UpdateAnimation( pcUnitGame, eAnimation );
				}
			}
		}
...
NAKED BOOL CharacterGame::CancelAttack()
{
	JMP( 0x004529B0 );
}
...
void CharacterGame::HandlePacket( PacketCharacterDataEx * psPacket )
{
	...
	ACTIONBARHANDLER->RestoreSaveData( psPacket->sActionBarData );
}
	...
	for( int i = 0; i < 15; i++ )
	{
		if( INVENTORYITEMSLOT[i].iItemIndex )
		{
			...
			if( psItemData->bValid && psItemData->sItem.sIntegrity.sMax > 0 )
			{
				...
				{
					if ( psItemData->sItem.sIntegrity.sCur <= (short)uIntegrityValueNotify )
					{
...
void CharacterGame::HandlePacket( PacketCharacterRecordDataSplit * psPacket )
{
	CharacterGame::pcCharacterSaveHandlerReceive->HandlePacket( psPacket );
...
NAKED BOOL CharacterGame::ChangeModelSkin( UnitData * pcUnitData, BOOL bIsComicArmor, int iComicHeadNumber, const char * pszBodyModel, const char * pszHeadModel )
{
	JMP( 0x00458B90 );
}

NAKED const char * CharacterGame::GetHeadModelSkin( UnitData * pcUnitData )
{
	JMP( 0x00458510 );
}

void CharacterGame::SendCharacterData( UnitData * pcUnitData )
{
	...
	sPacket.sAngle = pcUnitData->sAngle;
...
NAKED DWORD CharacterGame::GetCharacterDataChecksum( CharacterData * psCharacterData )
{
	JMP( 0x00462200 );
}
