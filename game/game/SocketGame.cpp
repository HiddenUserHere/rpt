...
SocketGame::SocketGame()
{
	...
	ZeroMemory( &sTCPWindowsData, sizeof( MIB_TCPSTATS ) );
	ZeroMemory( &sFirstTCPWindowsData, sizeof( MIB_TCPSTATS ) );

	UpdateTCPWindowsData( sFirstTCPWindowsData );
}
...
void SocketGame::Load( HWND hWnd )
{
	...
	int iMaxSendPacket = (IOCP_SERVER_TYPE || RAKNET_SERVER_TYPE) ? 1 : 500;
	int iMaxReceivePacket = (IOCP_SERVER_TYPE || RAKNET_SERVER_TYPE) ? 1 : 500;
	...
	for ( int i = 0; i < MAX_CONNECTIONS; i++ )
	{
		...
		if ( (IOCP_SERVER_TYPE == FALSE) && (RAKNET_SERVER_TYPE == FALSE) )
		{
...
void SocketGame::Loop()
{
	if ( Game::GetGameMode() == GAMEMODE_InGame )
	{
		...
		if ( SOCKETL )
			SOCKETL->UpdateTime( TICKCOUNT );
		if ( SOCKETG )
			SOCKETG->UpdateTime( TICKCOUNT );
	}
...
void SocketGame::Tick( ETickType iTickType )
{
	switch ( iTickType )
	{
		case TICKTYPE_100MilliSecond:
			PingConnections();
			UpdateTCPWindowsData( sTCPWindowsData );
			break;
	}
}
...
void SocketGame::PingConnections()
{
	...
	for ( int i = 0; i < MAX_CONNECTIONS; i++ )
	{
		...
		if ( (sd->bInUse) && (sd->bConnected) )
		{
			if ( (sd->bPing == false) )
			{
				if ( (sd->dwPingTime + 2500) < TICKCOUNT )
				{
			...
			{
				//Reset Pinging if taking longer than 5 seconds
				if ( sd->GetPing( TICKCOUNT ) > 5000 )
					sd->bPing = false;
			}
		}
	}
...
void SocketGame::SocketClose( SocketData * sd )
{
	if ( (sd) && (sd->bInUse) )
	{
		...
		else if ( RAKNET_SERVER_TYPE )
		{
			if ( auto pc = GetRakNetHandler( sd ) )
			{
				RemoveRakNetHandler( sd );
			}
		}
		else
		{
...
CFNetClientHandler * SocketGame::GetRakNetHandler( SocketData * sd )
{
	for( auto pc : vRakNetClients )
		if ( pc->GetInternalID() == sd->iConnectionID )
			return pc;

	return nullptr;
}

CFNetClientHandler * SocketGame::AddRakNetHandler()
{
	if( RAKNET_SERVER_TYPE == FALSE )
		return nullptr;

	CFNetClientHandler * pClientHandler = new CFNetClientHandler();

	vRakNetClients.push_back( pClientHandler );

	return pClientHandler;
}

void SocketGame::RemoveRakNetHandler( SocketData * sd )
{
	for( auto it = vRakNetClients.begin(); it != vRakNetClients.end(); )
	{
		auto pc = (*it);
		if ( pc->GetInternalID() == sd->iConnectionID )
		{
...
void SocketGame::CloseServerL()
{
	if ( SOCKETL != NULL )
	{
		...
		{
			if ( auto pc = GetRakNetHandler( SOCKETL ) )
			{
				RemoveRakNetHandler( SOCKETL );
			}
		}
		else
		{
...
void SocketGame::CloseServerG()
{
	if ( SOCKETG != NULL )
	{
		...
		{
			if ( auto pc = GetRakNetHandler( SOCKETG ) )
			{
				RemoveRakNetHandler( SOCKETG );
			}
		}
		else
		{
...
bool SocketGame::ConnectSocket( SocketData * sd, const char * pszIP, int iPort )
{
	...
	{
		auto pcClient = SOCKETGAME->AddRakNetHandler();

		if ( pcClient->Init( pszIP, iPort ) )
		{
			sockaddr_in sSockAddr;
			ZeroMemory( &sSockAddr, sizeof( sockaddr_in ) );
			sSockAddr.sin_family = AF_INET;
			...
			sd->iConnectionID = pcClient->GetInternalID();
...
BOOL SocketGame::SendPacketLogin( Packet * psPacket, BOOL bEncrypted )
{
	...
	{
		if( auto pc = SOCKETGAME->GetRakNetHandler( SOCKETL ) )
			pc->Send( SOCKETL, psPacket, bEncrypted );
	}
	else
	{
...
BOOL SocketGame::SendPacketGame( Packet * psPacket, BOOL bEncrypted )
{
	...
	{
		if ( auto pc = SOCKETGAME->GetRakNetHandler( SOCKETG ) )
			pc->Send( SOCKETG, psPacket, bEncrypted );
	}
	else
	{
...
std::string SocketGame::GetTCPWindowsDataString()
{
	UINT uTotalSent = (sTCPWindowsData.dwOutSegs - sFirstTCPWindowsData.dwOutSegs);
	double dTotalSent = (double)uTotalSent;
	double dRetransSegs = (double)(sTCPWindowsData.dwRetransSegs - sFirstTCPWindowsData.dwRetransSegs);
	double dLoss = (dRetransSegs * 100.0);
	double dLossPercent = dLoss / (dTotalSent > 0 ? dTotalSent : 1);

	std::string strTotalSent = FormatNumber( uTotalSent );
	std::string strRetransSegs = FormatNumber( (INT64)dRetransSegs );


	return std::string( FormatString( "System Data Avg (Total: %s) (Loss: %s) (Loss Avg: %.2llf%%)", strTotalSent.c_str(), strRetransSegs.c_str(), dLossPercent));
}

void SocketGame::UpdateTCPWindowsData( MIB_TCPSTATS & sData )
{
	GetTcpStatisticsEx( &sData, AF_INET );
}
