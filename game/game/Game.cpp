...
EXEModelData * Game::ReadInx( const char * pszFile )
{
...
void Game::LoadDataFromLoader( EXEDataLoader * psLoader )
{
	...
	if ( psLoader->pcUnitData )
	{
		...
		{
			static DWORD TIME_MODEL_NOT_FOUND = 0;
			if ( (GM_MODE == FALSE) && (TIME_MODEL_NOT_FOUND < TICKCOUNT) )
			{
				PacketLogCheatEx sPacket;
				sPacket.iLength = sizeof( PacketLogCheatEx );
				sPacket.iHeader = PKTHDR_LogCheat;
				sPacket.iCheatID = CHEATLOGID_ModelFileNotFound;
				STRINGCOPY( sPacket.szParam, psLoader->szName );
				SENDPACKETL( &sPacket );

				TIME_MODEL_NOT_FOUND = TICKCOUNT + 1000;
			}

			if ( DEBUG_GM )
			{
				ChatGame::AddChatBoxTextEx( CHATCOLOR_Error, AY_OBFUSCATE( "> Model %s not found!" ), psLoader->szName );
			}
		}
...
void Game::AddStageLoader( Stage * pcStage, const std::string strStageName )
{
	int i = READINT( 0x00A18094 ) & 0x3F;
	...
	ps->pcStage = pcStage;
	ps->pcUnitData = nullptr;
	ps->pcItemDrop = nullptr;
	ps->bCancel = FALSE;
	ps->iResult = 0;
	STRINGCOPY( ps->szName, strStageName.c_str() );
...
void Game::HandlePacket( PacketUnitInfo * psPacket )
{
	...
	else
	{
		if ( auto pcUnitData = UNITGAME->GetFreeUnitDataSlot() )
		{
			...
			pcUnitData->sAngle = psPacket->sAngle;
		}
	}
}
...
void Game::SetNewHardwareID( const char * pszNewHardwareID )
{
	STRINGCOPY( szNewHardwareID, pszNewHardwareID );
}
