...
void UnitMesh::EquipWeapon( EItemID eItemID )
{
	if ( pcUnitDataMesh )
	{
		if ( pcUnitDataMesh->sCharacterData.iClass == CHARACTERCLASS_Assassin )
		{
			ITEMHANDLER->EquipItem( pcUnitDataMesh, eItemID, 1 );
			ITEMHANDLER->EquipItem( pcUnitDataMesh, eItemID, 2 );
		}
		else
			ITEMHANDLER->EquipItem( pcUnitDataMesh, eItemID, 1 );
	}
...
void UnitMesh::EquipShield( EItemID eShieldID )
{
	if ( pcUnitDataMesh )
	{
		ITEMHANDLER->EquipItem( pcUnitDataMesh, eShieldID, 2 );
	}
}
	...
	if( renderTarget && GRAPHICENGINE->GetFrameCamera() )
	{
		if( Graphics::Graphics::GetInstance()->SetRenderTarget( renderTarget ) )
		{
			...
			if( pcUnitDataMesh->pcHeadMeshContainer && pcUnitDataMesh->pcHeadMeshContainer->pcNewModel && bRenderHead )
			{
				...
				if ( pcUnitDataMesh->pcHead->m_ModelsGroup )
					pModelsGroup = &pcUnitDataMesh->pcHead->m_ModelsGroup->DefaultModel;
				else
					pModelsGroup = nullptr;

				pcUnitDataMesh->pcHeadMeshContainer->pcNewModel->Render( (IO::SMD::FrameInfo *)&pcUnitDataMesh->iStartFrame, pModelsGroup ? pModelsGroup->meshes.size() ? pModelsGroup : nullptr : nullptr );
			}
			...
			if( pcUnitDataMesh->pcBodyMeshContainer && pcUnitDataMesh->pcBodyMeshContainer->pcNewModel && bRenderBody )
			{
				...
				if ( pcUnitDataMesh->pcBody->m_ModelsGroup )
					pModelsGroup = &pcUnitDataMesh->pcBody->m_ModelsGroup->DefaultModel;
				else
					pModelsGroup = nullptr;

				pcUnitDataMesh->pcBodyMeshContainer->pcNewModel->Render( (IO::SMD::FrameInfo*)&pcUnitDataMesh->iStartFrame, pModelsGroup ? pModelsGroup->meshes.size() ? pModelsGroup : nullptr : nullptr );
			}

			//pcUnitDataMesh->Render();
			...
			{
				Graphics::Sprite::Default->PushTranslation( Math::Vector2( (float)iRenderX, (float)iRenderY ) );
