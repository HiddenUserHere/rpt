...
BOOL RecvPacket::AnalyzePacket( SocketData * pcSocketData, Packet * psPacket )
{
	switch ( psPacket->iHeader )
	{
		...
		case PKTHDR_GetItemData:
			if ( GM_MODE )
			{
				...
				if ( ITEMINDEX_INVENTORY > 0 )
				{
					...
					sPacket.iItemIndex = ITEMINDEX_INVENTORY - 1;
					CopyMemory( &sPacket.sItem, &INVENTORYITEMS[ITEMINDEX_INVENTORY - 1], sizeof( ItemData ) );
					SENDPACKETG( &sPacket );
				}
			}
			break;
		...
		case PKTHDR_RetryWarehouse:
			WAREHOUSEHANDLER->OnWareHouseRetry();
			break;
		...
		case PKTHDR_SaveData:
			...
			CHARACTERGAME->HandlePacket( (PacketCharacterRecordDataSplit *)psPacket );
			break;

		case PKTHDR_WarehouseSaveSuccess:

			WAREHOUSEHANDLER->OnWareHouseSaved();
		...
		case PKTHDR_CreateCharacter:
			...
			{
				((CCharacterScreen *)GAMESCREEN->GetActiveScreen())->HandlePacket( (PacketCreateCharacter *)psPacket );
			}
			break;
...
*/
		...
		case PKTHDR_PersonalShopMessage:
			PERSONALSHOPHANDLER->HandlePacket( (PacketChatBoxMessage *)psPacket );
			break;
		...
		case PKTHDR_YurinaSkipQuests:
			...
			CQUESTGAME->SkipNewPlayerQuests(TRUE);
		
			UNITDATA->sCharacterData.iRank = ECharacterRank::CHARACTERRANK_Rank4;
