...
BOOL CSettingsModel::ReadSettings()
{
	...
	//Personalization
	{
		...
		sSettingsData.bShowConnectionData = cReader.ReadBool( "Personalization", "ShowConnectionData" );
	}
...
void CSettingsModel::WriteSettings()
{
	...
	//Personalization
	{
		...
		cReader.WriteBool( "Personalization", "ShowConnectionData", sSettingsData.bShowConnectionData );
	}
...
void CSettingsModel::OnWindowSizeChange( EWindowSizeStateChange eState, UINT uWidth, UINT uHeight )
{
	...
	switch ( eState )
	{
		case WINDOWSIZESTATECHANGE_Maximized:
		{
			RECT rc;
			rc.left = 0;
			rc.top = 0;
			...
			GRAPHICENGINE->Reset( SETTINGSHANDLER->GetModel()->GetSettings().bWindowed, uWidth, uHeight, TRUE );
			bMaximizedWindow = TRUE;
		}
		break;

		case WINDOWSIZESTATECHANGE_Restored:
			if ( GRAPHICENGINE->IsInitialized() )
			{
				GRAPHICENGINE->ResolutionChange( FALSE, uWidth, uHeight );
		...
		case WINDOWSIZESTATECHANGE_Minimized:
			break;
