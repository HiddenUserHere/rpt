...
namespace UI
{
	...
	struct OptionData
	{
		...
		OptionData( bool b, Text_ptr p ) : bActive( b ), pText( p ) {};
		~OptionData() {};
	};
...
public:
	...
	void										SetSelectedID( int iID );

	bool										HideOption( int iID );
	bool										ShowOption( int iID );
	...
	OptionData									* GetTextOption( int iIDObject )
	{
		// Find object and return pointer to this
		for ( UINT u = 0; u < vTextOptions.size(); u++ )
		{
			if ( vTextOptions[u].pText->GetID() == iIDObject )
			{
				return &vTextOptions[ u ];
			}
		}
		return nullptr;
	};
...
private:
	...
	std::vector<OptionData>						vTextOptions;
