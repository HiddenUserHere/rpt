...
void CCoinShopWindow::OnBuyCredits( UIEventArgs eArgs )
{
	...
	{
		ShellExecute( 0, 0, "https://user.regnum.world/userpanel/", 0, 0, SW_SHOW );
...
void CCoinShopWindow::OnPutItemSwapper( UIEventArgs eArgs )
{
	...
	if ( pWindowItem )
	{
		...
		if ( pItemBox && pItemBox->GetItems().size() > 0 && pText )
		{
			...
			iSelectedSwap = pcItemData->sItem.sItemID.ToItemType();

			//Set Price
			iCoins = Base::Coin::GetSwapperPrice( &pcItemData->sItem, iSelectedSwap );

			auto pComboBoxSwap = pWindowItem->GetElement<UI::ComboBox>( COMBOBOX_Swap );
			if ( (pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Boots) ||
				(pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Bracelets) ||
				(pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Gauntlets) )
			{
				pComboBoxSwap->HideAll();

				pComboBoxSwap->ShowOption( ITEMTYPE_Boots );
				pComboBoxSwap->ShowOption( ITEMTYPE_Bracelets );
				pComboBoxSwap->ShowOption( ITEMTYPE_Gauntlets );
			}
			else
			{
				pComboBoxSwap->ShowAll();

				pComboBoxSwap->HideOption( ITEMTYPE_Boots );
				pComboBoxSwap->HideOption( ITEMTYPE_Bracelets );
				pComboBoxSwap->HideOption( ITEMTYPE_Gauntlets );
			}

			pComboBoxSwap->SetSelectedID( iSelectedSwap );
...
void CCoinShopWindow::OnParentTabButtonClick( class CCoinShopTab * pcTab, UIEventArgs eArgs )
{
	...
	if ( pcTab )
	{
		...
		const std::vector<CCoinShopTab*> & vTabs = pcTab->GetChildTabs();
		for ( auto it = vTabs.begin(); it != vTabs.end(); it++ )
		{
...
void CCoinShopWindow::Load( class CCoinShopTab * pcTab )
{
	...
	if ( pListItems )
	{
		...
		// Get Item List
		const std::vector<CCoinShopItem*> & vItems = pcTab->GetItems();
	...
	if ( pcItem )
	{
		...
		if ( pListItems )
		{
			...
			if ( pItemImage )
			{
				...
				if( (pcItem->GetTabID() == 25) || (pcItem->GetTabID() == 52) || (pcItem->GetTabID() == 68) )
					BuildPreviewHead( pWindowItem );

				//Preview of Costume Model
				if( (pcItem->GetTabID() == 49) || (pcItem->GetTabID() == 50) || (pcItem->GetTabID() == 61) )
				{
...
void CCoinShopWindow::BuildSwapper( BOOL bSpec )
{
	...
	if ( pListItems )
	{
		...
		if ( pWindowItem )
		{
			...
			pItemBox->AddRule( UI::ItemBox::ERule::RULE_Disallow, UI::ItemBox::ETypeRule::TYPERULE_ItemID, ITEMID_CuepyCostumeIce );
			pItemBox->AddRule( UI::ItemBox::ERule::RULE_Disallow, UI::ItemBox::ETypeRule::TYPERULE_ItemID, ITEMID_CandySheltom );
			pItemBox->AddRule( UI::ItemBox::ERule::RULE_Disallow, UI::ItemBox::ETypeRule::TYPERULE_Level, 110 );
	...
	if ( pListItems )
	{
		...
		if ( pWindowItem )
		{
...

			{
				...
				pCheckBoxPremiumChange->SetSelected( FALSE );
