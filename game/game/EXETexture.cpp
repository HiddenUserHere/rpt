...
bool EXETexture::DecryptBMP( char * pBuffer, int iSize )
{
	if ( iSize >= 14 )
	{
		if ( (pBuffer[0] == 'A') && (pBuffer[1] == 'N') )
		{
			for ( int i = 0; i < 14; i++ )
			{
				pBuffer[i] += (1 - (((i + 1) * i) * i));
		...
		if ( (pBuffer[0] == 'A') && ((pBuffer[1] == '8') || (pBuffer[1] == '6')) )
		{
			pBuffer[0] = 'B';
			pBuffer[1] = 'M';
...
bool EXETexture::DecryptTGA( char * pBuffer, int iSize )
{
	const bool USE_SANDURR_MODE = false;
	...
	if ( iSize >= 18 )
	{
		if ( USE_SANDURR_MODE )
		{
			...
			{
				int iByte1 = pBuffer[13];
				int iByte2 = pBuffer[12];
				int iByte3 = pBuffer[14];
				int iByte4 = pBuffer[15];
				...
				iByte4 ^= iByte3;
				(*(int *)&pBuffer[0]) = 0x20000;
				(*(int *)&pBuffer[4]) = 0;
				(*(short *)&pBuffer[14]) = iByte4;
