...
BOOL CAnimationHandler::InjectDynamicAnimation( struct EXEModelData * psModelData, PTModel * pcModelAnimation, int iID, BOOL bLoop, int iClassFlag, int iAreaType, int iaEvent[4], int iItemType, int iTypeAnim, EMountType eMountType )
{
	...
	{
		CMountHandler::SetCharacterAnimation( GetCharacterClassByAnimationClassFlag( (EAnimationClassFlag)iClassFlag ), eMountType, (EAnimationType)iTypeAnim, psModelINX->iNumModelAnimation );
	}
...
void CAnimationHandler::LoadDynamicAnimation()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		eMountType	= (EMountType)cReader.ReadInt( szSkillName, "MountType", (int)EMountType::MOUNTTYPE_None );
		...
		InjectDynamicAnimation( psModelData, pcModel, iID, bLoop, iClassFlag, iAreaType, iaEvent, iItemType, iTypeAnim, eMountType );
