...
BOOL Button::OnMouseClick( CMouse * pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	...
	if ( (psPosition->iX >= iRenderX) && (psPosition->iX <= (iRenderX + iRenderWidth - iSourceX)) )
	{
		...
		if ( (psPosition->iY >= iRenderY) && (psPosition->iY <= (iRenderY + iRenderHeight - iSourceY)) )
		{
			...
			if ( iAdditionalEventID && eAdditionalMouseEvent == pcMouse->GetEvent() || bHolding )
			{
				if ( bCanHold )
					bHolding = TRUE;

				UI::Event::Invoke( iAdditionalEventID );
				return TRUE;
			}

			if ( pImageHold && ((pcMouse->GetEvent() == ClickDownR) && (eAdditionalMouseEvent == ClickUpR)) )
				bHoldingClick = TRUE;
		}
	}
...
void Button::OnMouseMove( CMouse * pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	...
	if ( (psPosition->iX >= iRenderX) && (psPosition->iX <= (iRenderX + iRenderWidth - iSourceX)) )
	{
		...
		if ( (psPosition->iY >= iRenderY) && (psPosition->iY <= (iRenderY + iRenderHeight - iSourceY)) )
		{
			...
			if ( iOnHoverEventID != -1 )
				Event::Invoke( iOnHoverEventID );
		}
	}

	if ( bHover == FALSE )
	{
		if ( iOnHoverOutEventID != -1 )
			Event::Invoke( iOnHoverOutEventID );
	}
}
}