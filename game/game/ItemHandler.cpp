...
bool ItemHandler::GetItemDataTableByItemID( ItemData & cItemDataOut, EItemID iItemID )
{
	for ( UINT i = 0; i < ITEMTABLE_ITEM_COUNT; i++ )
	{
		...
		{
			CopyMemory( &cItemDataOut, psItemData, sizeof( ItemData ) );
...
ItemData * ItemHandler::FindItemTableByCode( const std::string strItemCode )
{
	for ( UINT i = 0; i < ITEMTABLE_ITEM_COUNT; i++ )
	{
		ItemData * psItemData = (ItemData *)((DWORD)allocIT + (i * 0x314));
		if ( STRINGCOMPAREI( psItemData->szInventoryName, strItemCode.c_str() ) )
			return psItemData;
	}
...
ItemData * ItemHandler::GetMouseItemHover()
{
	if ( ITEMINDEX_INVENTORY > 0 )
		return &INVENTORYITEMS[ITEMINDEX_INVENTORY - 1];
...
int ItemHandler::GetItemInventoryIndexByCode( EItemID eID )
{
	for ( int i = 0; i < 100; i++ )
	{
...
{
	int iIndex = GetItemInventoryIndexByCode( eID );
	...
	if ( iIndex >= 0 )
	{
		if ( iIndex < 100 )
			...
			return TEMPINVENTORYITEMS + (iIndex - 100);
	}
	
	return NULL;
}

ItemData * ItemHandler::GetItemInventoryByIndex( int iIndex )
{
	if ( iIndex >= 0 )
	{
		if ( iIndex < 100 )
		{
			ItemData * ps = INVENTORYITEMS + iIndex;
			if ( ps->bValid )
				return ps;
		}
		else
		{
			ItemData * ps = TEMPINVENTORYITEMS + (iIndex - 100);
			if ( ps->bValid )
				return ps;
		}
	}
...
ItemData * ItemHandler::GetItemInventory( EItemID eID, int iChk1, int iChk2 )
{
	...
	if ( ItemData * pcItemData = GetMouseItem() )
	{
		if ( pcItemData->bValid )
		{
			if ( (pcItemData->sItem.sItemID.ToItemID() == eID) && (pcItemData->sItem.iChk1 == iChk1) && (pcItemData->sItem.iChk2 == iChk2) )
				return pcItemData;
		}
	}
...
const char * ItemHandler::GetArmorPatternName( class UnitData * pcUnit, DWORD dwCode )
{
	const char * pszArmorName = NULL;
...
BOOL ItemHandler::RightClickSimulate( EItemID eItemID )
{
	if ( int iIndex = ITEMHANDLER->GetItemInventoryIndexByCode( eItemID ); iIndex >= 0 )
	{
		auto pcItemData = ITEMHANDLER->GetItemInventoryByIndex( iIndex );

		ITEMINDEX_INVENTORY = iIndex + 1;
		
		CGameInventory::GetInstance()->OnMouseRDownClick( pcItemData->sPosition.iX, pcItemData->sPosition.iY );
...
NAKED int ItemHandler::UsedPotion()
{
	JMP( 0x0050A5C0 );
}
...
BOOL ItemHandler::SetCharacterItem( EItemID iItemID, int iPosition, BOOL bActive )
{
	...
	switch ( iPosition )
	{
		...
		case ITEMSLOTFLAG_Costume:
		{
			...
			//		CGameInventory::SetCostumeInventoryOpen( TRUE );
			}
...
BOOL ItemHandler::OnSetArmorUnit( UnitData * psUnit, UINT uItemCode )
{
	...
	const char * pszArmorName = GetArmorPatternName( psUnit, uItemCode );
...
void ItemHandler::DrawColorBox( DWORD Color, int iX, int iY, int iW, int iH )
{
	...
	GRAPHICENGINE->DrawRectangle2D( Rectangle2D( iX, iY, iW, iH ), Color, Color );
}
...
void ItemHandler::DrawSprite( int winX, int winY, int lpdds, int x, int y, int width, int height )
{
	...
	EXESurfaceOld * psSurface = (EXESurfaceOld *)lpdds;

	if( psSurface )
		UI::ImageRender::Render( psSurface->sTexture.pcD3DTexture, x, y, width, height, winX, winY, -1, FALSE );
}
...
{ //Return 0: Deny Use 1: Allow Use 2: Used
	...
	{
		if ( iItemID == ITEMID_AtlantisCore )
		{
			TITLEBOX( "You can't use this Ether Core!" );
			return 0;
		}
	...
	if (iItemID == ITEMID_StarterCore)
	{
		...
		STARTERCOREHANDLER->Open(pcItemData);
...
NAKED BOOL ItemHandler::IsSameItem( Item * psItemOne, Item * psItemTwo )
{
	JMP( 0x0045BBC0 );
}
