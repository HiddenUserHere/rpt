...
void __cdecl AssaParticle_DivineInhalationStart( UnitData * pcUnitDataEffect, int iTimeEffect )
{
	...
	if ( pcUnitDataEffect->pcBoneContainer )
	{
		...
		sCharacterPosition.iX = (int)(pcUnitDataEffect->sPosition.iX + sMeshPosition.iX);
		sCharacterPosition.iY = (int)(pcUnitDataEffect->sPosition.iY + sMeshPosition.iY);
		sCharacterPosition.iZ = (int)(pcUnitDataEffect->sPosition.iZ + sMeshPosition.iZ);
	}
	else
	{
...
BOOL __cdecl AssaParticleClanMonsterHit1( UnitData * pcUnitDataEffect )
{
	...
	Point3D sCharacterPosition;
	sCharacterPosition.iX = (int)(pcUnitDataEffect->sPosition.iX + sMeshPosition.iX);
	sCharacterPosition.iY = (int)(pcUnitDataEffect->sPosition.iY);
	sCharacterPosition.iZ = (int)(pcUnitDataEffect->sPosition.iZ + sMeshPosition.iZ);
	...
	if ( HANDLE hFile = CreateFile( pszFileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL ); hFile != INVALID_HANDLE_VALUE )
	{
		...
		if ( sVersion.IsValid() )
		{
			...
			if ( pcStage->iLightCount > 0 )
			{
				...
				pcStage->pLight = new StageLight[pcStage->iLightCount];
				ReadFile( hFile, pcStage->pLight, sizeof( StageLight ) * pcStage->iLightCount, &dwFileSize, NULL );

				if ( bEncrypted )
					pcEncrypter->EncDecrypt( (BYTE *)pcStage->pLight, sizeof( StageLight ) * pcStage->iLightCount );
			}
		...
		else if ( strCommand.compare( "*PHYSIQUE_VERTEXASSIGNMENT_NONBLENDED_RIGIDTYPE" ) == 0 )
		{
			if ( vArguments.size() == 2 )
			{
				...
				if ( EXEModel::GetBone() )
				{
					...
					//Set Weight for non-blended skin
