...
extern BOOL __cdecl CanHandleStopMove( UnitData * pcUnitData );
extern BOOL __cdecl CanHandleMovementFrames( UnitData * pcUnitData );
extern void __cdecl FrameUnitUpdate( UnitData * pcUnitData );

extern BOOL __cdecl OnRespawnUnitSelf( UnitData * pcUnitData );
...
int __stdcall ChangeMotion( int iMotion, int iAnimation )
{
	...
	return CHARACTERGAME->OnMotionChange( (UnitData*)pptr, iMotion, iAnimation, ANIMATIONTYPE_None, MOUNTHANDLER->IsRiding( pptr ) );
}
...
BOOL __stdcall PrepareSkillInfoSpecial( Skill * psSkill, SkillBox * psSkillBox, int iIcon, BOOL bSkillTimer )
{
	if ( SKILLINFOBOX->CanPrepareShowSkill( psSkill, Rectangle2D( psSkillBox->BoxRect.left, psSkillBox->BoxRect.top, psSkillBox->BoxRect.right, psSkillBox->BoxRect.bottom ) ) )
	{
		...
		SKILLINFOBOX->PrepareShowSkill( psSkill, bSkillTimer, Rectangle2D( psSkillBox->BoxRect.left, psSkillBox->BoxRect.top, psSkillBox->BoxRect.right, psSkillBox->BoxRect.bottom ) );
	}
...
{
	PrepareSkillInfoSpecial( psSkill, psSkillBox, iIcon, FALSE );
...
BOOL __cdecl CanUseItemSlotGate( ItemData * pcItemData, ItemInventoryData * psSlot, BOOL bNegative )
{
	if ( bNegative )
	{
		...
		if ( pcItemData->sBaseItemID.ToItemType() == ITEMTYPE_Dagger )
		{
			//Can Use
			if ( (psSlot->iItemSlot & ITEMSLOTFLAG_LeftHand) || (psSlot->iItemSlot & ITEMSLOTFLAG_RightHand) )
				return FALSE;
		}
...
BOOL __stdcall LoadModelOldGate( const char * pszFileName, const char * pszNodeName )
{
	...
	auto CheckAllowedModels = []( const std::string _strFileName ) -> bool
	{
		static const std::vector<std::string> vAllowedModelsFolder =
		{
			"weapons\\",
			"field\\",
		};

		for ( auto strFolder : vAllowedModelsFolder )
		{
			if ( _strFileName.length() > strFolder.length() )
			{
				auto strFolderName = _strFileName.substr( 0, strFolder.length() );

				if ( std::equal( strFolderName.begin(), strFolderName.end(),
					strFolder.begin(), strFolder.end(),
					[]( char a, char b )
				{
	...
	if ( (strFileName.length() > 9) && (((strFileName.substr( 0, 8 ).compare( "weapons\\" ) == 0) && (strFileName[8] != 'T')) || CheckAllowedModels( strFileName )) )
		bNewRender = true;
...
void __cdecl OnLightImageDrawGate( Stage * pcStage, int iX, int iY, int iZ )
{
	OnDrawLightImage( pcStage, iX, iY, iZ );
}

bool __cdecl DecryptBMP( char * pBuffer, unsigned int uBufferSize )
{
	return EXETexture::DecryptBMP( pBuffer, uBufferSize );
}

bool __cdecl DecryptTGA( char * pBuffer, unsigned int uBufferSize )
{
	return EXETexture::DecryptTGA( pBuffer, uBufferSize );
}

BOOL __cdecl OnLevelAgeCheck( ItemData * pcItemData )
{
	return pcItemData->sItem.sAgeLevel < MAX_AGING;
}

BOOL __cdecl CanRenderPhoenixPet( UnitData * pcUnitData )
{
	if ( pcUnitData != UNITDATA )
	{
		if ( (pcUnitData->iVanishTimeLeft > 0) || (pcUnitData->iBlindSkill > 0) )
		{
			if( pcUnitData->sRenderPoint.iZ >= 0x30000 )
				return FALSE;
		}
	}
...
BOOL __cdecl OnWarehouseSend( void * pUnknownData )
{
	WAREHOUSEHANDLER->UpdateWarehouseTime();

	CALL_WITH_ARG3( 0x00622150, (DWORD)pUnknownData, 0, 0 );
	return TRUE;
}

void __cdecl OnItemSetCharacterSlotGate( ItemData * pcItemData )
{
	bool bRet = false;

	//Armor Slot?
	if ( pcItemData->iVisibleItemType == ITEMSLOTFLAG_Armor )
	{
		//Check for Costume
		int iCostumeItemIndex = INVENTORYITEMSLOT[ITEMSLOT_Costume - 1].iItemIndex;
		if ( iCostumeItemIndex > 0 )
		{
			//ITEMHANDLER->SetCharacterItem( pcItemData->sBaseItemID.ToItemID(), pcItemData->iVisibleItemType, TRUE);
			ITEMHANDLER->SetCharacterItem( iCostumeItemIndex > 0 ? INVENTORYITEMS[iCostumeItemIndex - 1].sItem.sItemID.ToItemID() : ITEMID_None, ITEMSLOTFLAG_Costume, TRUE );

			bRet = true;
		}
	}
	else if ( pcItemData->sBaseItemID.ToItemType() == ITEMTYPE_Dagger )
	{
		if ( pcItemData->iItemSlot == ITEMSLOT_LeftHand )
		{
			ITEMHANDLER->SetCharacterItem( pcItemData->sBaseItemID.ToItemID(), ITEMSLOTFLAG_LeftHand, TRUE );
...
void __stdcall SendCharacterDataGate( void * pUnused )
{
	UnitData * pcThis;

	READTHIS( pcThis );

	CHARACTERGAME->SendCharacterData( pcThis );
}
...
BOOL APIENTRY DllMain( HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved )
{
	if ( ul_reason_for_call == DLL_PROCESS_ATTACH )
	{
		...
		if ( hFile = FindFirstFileA( szFileName, &sFindData ) )
		{
			do
			{
				if ( (std::string( sFindData.cFileName ).compare( AY_OBFUSCATE( "game.dll" ) ) != 0) )
				{
		...
		*/
		...
		float discount = 1.0f; //reduce skill price for now while skills are broken
		...
		(*(DWORD *)0x04AF8270) = (DWORD)&MainUnitDataSelf;
	...
	(*(DWORD *)0x04AF8398) = (DWORD)&OnLightImageDrawGate;
	(*(DWORD *)0x04AF839C) = (DWORD)&DecryptBMP;
	(*(DWORD *)0x04AF83A0) = (DWORD)&DecryptTGA;
	(*(DWORD *)0x04AF83A4) = (DWORD)&OnLevelAgeCheck;
	(*(DWORD *)0x04AF83A8) = (DWORD)&CanRenderPhoenixPet;
	(*(DWORD *)0x04AF83AC) = (DWORD)&OnItemSetCharacterSlotGate;
	(*(DWORD *)0x04AF83B0) = (DWORD)&OnWarehouseSend;
	(*(DWORD *)0x04AF83B4) = (DWORD)&SendCharacterDataGate;
