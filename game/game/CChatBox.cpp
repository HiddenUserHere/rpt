	...
	if( vListToAdd.size() > 0 )
	{
		for( auto &v : vListToAdd )
		{
			if( v )
			{
				...
				pTextMessage->SetID( TEXT_MessageGame );
				...
				pBaseWindow->SetEvent( UI::Event::Build( std::bind( &CChatBox::OnMessageClick, this, pTextMessage, eChatColor, std::placeholders::_1 ) ), EMouseEvent::DoubleClickL );
...
void CChatBox::OnMessageClick( UI::Text_ptr pTextMessage, EChatColor eChatColor, UIEventArgs eArgs )
{
	std::string strName = GetPlayerNameReceived( pTextMessage->GetHighlightText().length() == 0 ? pTextMessage->GetText() : pTextMessage->GetHighlightText(), eChatColor );
	if ( strName.length() > 0 )
	{
		OnEnableChatBox();
		...
		SetInputFieldText( FormatString( "/:%s ", strName.c_str() ) );
	}
}
...
std::string CChatBox::GetPlayerNameReceived( std::string strMessage, EChatColor eChatColor )
{
	...
	else
	{
		if ( strMessage.length() > 2 )
		{
			//Get All the text before the : and first space
			size_t iFind = strMessage.find( ':' );
			if ( iFind != std::string::npos )
			{
				strName = strMessage.substr( 0, iFind );

				//Player name?
				if ( (eChatColor == CHATCOLOR_Trade) || (eChatColor == CHATCOLOR_Clan) || (eChatColor == CHATCOLOR_ClanLeader) )
				{
					//Remove the [%c] from the name
					if ( strName.length() > 3 )
					{
						if ( strName[0] == '[' && strName[2] == ']' )
