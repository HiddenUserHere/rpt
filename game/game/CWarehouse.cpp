...
void CWarehouse::Update()
{
	if ( Game::GetGameMode() == GAMEMODE_InGame )
	{
		if ( dwTimeSending && dwTimeSending < TICKCOUNT )
		{
			OnWareHouseRetry();
		}
	}
...
void CWarehouse::OnWareHouseRetry()
{
	if ( !UpdateWarehouseTime() )
		return;

	if ( DEBUG_GM )
	{
		CHATGAMEHANDLER->AddChatBoxText( AY_OBFUSCATE( "> Retrying to save warehouse..." ) );
	}

	CALL( 0x0062A2CB );
}

BOOL CWarehouse::UpdateWarehouseTime()
{
	if ( iSending >= MAX_RETRY_WAREHOUSE )
	{
	...
	dwTimeSending = TICKCOUNT + 5000;

	return TRUE;
}

void CWarehouse::OnWareHouseSaved()
{
	iSending = 0;
	dwTimeSending = 0;
}
