...
void UnitGame::SetCharacterFormula( ItemData * pItem, ItemInventoryData * pInventory, CharacterDataLocal & sCharacterDataLocal )
{
	...
	switch( PLAYERDATA->iClass )
	{
		...
		{
			int iItemLeftIndex = pInventory[ITEMSLOT_LeftHand - 1].iItemIndex - 1;
			int iItemLeftSpec = 0;
			if ( iItemLeftIndex >= 0 )
				iItemLeftSpec = pItem[iItemLeftIndex].sItem.sItemID.ToItemType() == ITEMTYPE_Dagger ? pItem[iItemLeftIndex].sItem.eSpecialization : 0;

			int iItemLeftClass = 0;
			if ( iItemLeftIndex >= 0 )
				iItemLeftClass = pItem[iItemLeftIndex].sItem.sItemID.ToItemType() == ITEMTYPE_Dagger ? pItem[iItemLeftIndex].iItemWpnDmg : 0;

			if ( (iItemClass == WEAPONCLASS_Melee) || (iItemLeftClass == WEAPONCLASS_Melee) )
			{
				PLAYERDATA->iMinDamage = 1 + ((sCharacterDataLocal.iMinDamage * (iStrength + 240) / 240)) + ((sCharacterDataLocal.iMinDamage * (iSpirit + 480) / 480)) + ((sCharacterDataLocal.iMinDamage * (iTalent + 420) / 420)) + ((sCharacterDataLocal.iMinDamage * (iAgility + 400) / 400)) - ((sCharacterDataLocal.iMinDamage * (iHealth + 500) / 400));
				PLAYERDATA->iMaxDamage = 3 + ((sCharacterDataLocal.iMaxDamage * (iStrength + 240) / 240)) + ((sCharacterDataLocal.iMaxDamage * (iSpirit + 480) / 480)) + ((sCharacterDataLocal.iMaxDamage * (iTalent + 420) / 420)) + ((sCharacterDataLocal.iMaxDamage * (iAgility + 400) / 400)) - ((sCharacterDataLocal.iMaxDamage * (iHealth + 500) / 400));
				if ( (iItemSpec == CLASSFLAG_Assassin) || (iItemLeftSpec == CLASSFLAG_Assassin) )
				{
					PLAYERDATA->iMinDamage += (sCharacterDataLocal.iMinDamage + sCharacterDataLocal.iMaxDamage) / 16;
					if ( pItem[iItemIndex].sItem.sSpecData.iSpecAttackPowerDiv > 0 )
						PLAYERDATA->iMaxDamage += PLAYERDATA->iLevel / pItem[iItemIndex].sItem.sSpecData.iSpecAttackPowerDiv;
...
void UnitGame::RenderDebugText()
{
	...
	vUserInfo.push_back( GetConnectionDataText( true ) );
	vUserInfo.push_back( GetConnectionDataText( false ) );
	vUserInfo.push_back( SOCKETGAME->GetTCPWindowsDataString() );
	...
	vCharacterInfo.push_back( "Dark: " + to_string( DARKCOLOR_AMOUNT >> 1 ) );
	...
	extern int TOTAL_ITEM_STAGE_GAME;

	pFont->Draw( FormatString( "Total Items Floor: %d", TOTAL_ITEM_STAGE_GAME ), 350, 4 + (iLeft++ * 15), DT_WORDBREAK, DX::Color( 255, 255, 255, 255 ) );
...
void UnitGame::RenderConnectionData()
{
	if ( SETTINGSHANDLER->GetModel()->GetSettings().bShowConnectionData )
	{
		const int iXDebug = 8;
		int iPaddingBottom = 80;

		GetGraphicsDevice()->GetDefaultFont()->Draw( GetConnectionDataText( true ), iXDebug, iPaddingBottom, 0, DX::Color( 255, 255, 255 ) );
		iPaddingBottom += 15;

		GetGraphicsDevice()->GetDefaultFont()->Draw( GetConnectionDataText( false ), iXDebug, iPaddingBottom, 0, DX::Color( 255, 255, 255 ) );
		iPaddingBottom += 15;

		GetGraphicsDevice()->GetDefaultFont()->Draw( SOCKETGAME->GetTCPWindowsDataString(), iXDebug, iPaddingBottom, 0, DX::Color( 255, 255, 255 ) );
	}
}
...
void UnitGame::SendUnitDataEx()
{
	...
	extern UINT HANDLE_CPP_THREAD_EXPT;
	psPacketPlay->iMountTimer = HANDLE_CPP_THREAD_EXPT;
...
void UnitGame::HandlePacket( SocketData * pcSocketData, PacketUnitStatusContainer * psPacket )
{
	...
	for ( int i = 0; i < psPacket->iAmount; i++ )
	{
		...
		if ( auto pcUnitData = UNITDATABYID( ps->lID ) )
		{
			...
			if ( ps->bSelfUpdate && (ps->iHeader == PKTHDR_UnitStatusMove) )
			{
				pcUnitData->eUpdateMode = UPDATEMODE_ClientSelf;

				if ( ps->bOfflineShop && (pcUnitData->iPersonalShop == 0) )
				{
					pcUnitData->iPersonalShop = 1;

					PERSONALSHOPHANDLER->RequestPersonalShopMessage( pcUnitData->iID, 1 );

					UpdateEquippedItems( pcUnitData, ps->saEquippedItems );
				}
			}
		}
		else
...
std::string UnitGame::GetConnectionDataText( bool bLoginServer )
{
	SocketData * pcSocketData = bLoginServer ? SOCKETL : SOCKETG;
	if ( pcSocketData )
	{
		std::string strServer = bLoginServer ? "Login Server" : "Game Server";

		return strServer + " (PING: " + std::to_string( pcSocketData->iPing ) + " ms) (DL: " + FormatNumberString( pcSocketData->iAmountINB ) + " B/s " + FormatNumberString( pcSocketData->iCountINB ) + " P/s) " +
			"(UL: " + FormatNumberString( pcSocketData->iAmountOUTB ) + " B/s " + FormatNumberString( pcSocketData->iCountOUTB ) + " P/s)";
	}
