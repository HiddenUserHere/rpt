...
void CPersonalShopHandler::HandlePacket( PacketChatBoxMessage * psPacket )
{
	if ( UnitData * pcUnitData = UNITDATABYID( psPacket->lID ) )
	{
		if ( pcUnitData->iPersonalShop )
		{
			STRINGFORMAT( pcUnitData->szPersonalShop, "%s: %s", pcUnitData->sCharacterData.szName, psPacket->szChatBoxMessage );
		}
	}
...
void CPersonalShopHandler::RequestPersonalShopMessage( int iID, int iPersonalShopCode )
{
	ProcessPacket( &PacketPersonalShopRequestMessage( iID, iPersonalShopCode ) );
}
...
void CPersonalShopHandler::ProcessPacket( PacketPersonalShopRequestMessage * psPacket )
{
	psPacket->iLength = sizeof( PacketPersonalShopRequestMessage );
	psPacket->iHeader = PKTHDR_PersonalShopMessage;
	SENDPACKETG( psPacket );
}
