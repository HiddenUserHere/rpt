...
//	hHookWindowHandle = SetWindowsHookExA( WH_MOUSE_LL, HookCallbackMouseClick, NULL, 0 );
	...
	CAntiCheat::dwCheckSumDLL = CAntiCheat::GetModuleChecksum( std::string( AY_OBFUSCATE( "game.dll" ) ) );
	CAntiCheat::dwCheckSumEXE = CAntiCheat::GetModuleChecksum( std::string( AY_OBFUSCATE( "game.exe" ) ) );
...
*/
		...
		DWORD dwResChecksum = CAntiCheat::GetModuleChecksum( std::string(AY_OBFUSCATE("game.dll")) );
		...
		dwResChecksum = CAntiCheat::GetModuleChecksum( std::string( AY_OBFUSCATE( "game.exe" ) ) );
...
void CAntiCheat::UpdateHardwareID()
{
    ...
    HardwareIDData sDataOld;
	HardwareIDData sDataNew;

    char szTempPath[MAX_PATH] = {0};
    GetTempPathA( MAX_PATH, szTempPath );

	std::string strFilePath = szTempPath + std::string( AY_OBFUSCATE( "\\10_41198587.rep" ) );
	std::string strNewFilePath = szTempPath + std::string( AY_OBFUSCATE( "\\1518718581.cl" ) );

    CWMIReader * pcWMIReader = new CWMIReader();

	Game::SetHardwareID( pcWMIReader->GetHardwareID().c_str() );
	Game::SetNewHardwareID( pcWMIReader->GetHardwareNewID().c_str() );

    delete pcWMIReader;

	auto UpdateHardwareFile = []( HardwareIDData & sData, std::string strHardwareFile, bool bNewHardwareID, const char * pszHardwareID ) -> void
	{
		if ( FileExist( strHardwareFile ) == FALSE )
		{
			...
			STRINGCOPY( sData.szHardwareID, pszHardwareID );

			for ( int i = 0; i < _countof( sData.szHardwareID ); i++ )
				sData.szHardwareID[i] ^= 0x37;

			FILE * pFile = nullptr;
			fopen_s( &pFile, strHardwareFile.c_str(), AY_OBFUSCATE( "wb" ) );
			if ( pFile )
			{
		...
		{
			FILE * pFile = nullptr;
			fopen_s( &pFile, strHardwareFile.c_str(), AY_OBFUSCATE( "rb" ) );
			if ( pFile )
			{
				...
				if ( bNewHardwareID )
					Game::SetNewHardwareID( sData.szHardwareID );
				else
	...
	UpdateHardwareFile( sDataOld, strFilePath, false, Game::GetHardwareID() );
	UpdateHardwareFile( sDataNew, strNewFilePath, true, Game::GetNewHardwareID() );
}

void CAntiCheat::SelfChildProcess()
{
