...
void CMountWindowHandler::RenderMountIcon( int iMountID, Rectangle2D rRectMountBox )
{
    if ( iMountID == -1 )
        return;
}

UI::ImageBox_ptr CMountWindowHandler::GetMountImage( int iMountID )
{
    if ( iMountID == -1 )
        return nullptr;

    auto pList = pWindow1->GetElement<UI::List>( LIST_Mounts );
    if ( auto pMountItem = pList->GetWindow( iMountID ) )
    {
        if ( auto pImage = pMountItem->GetElement<UI::ImageBox>( IMAGEBOX_MountImage ) )
            return pImage;
    }
...
void CMountWindowHandler::UseMountByID( int iMountID )
{
    if ( iMountID == -1 )
        return;
    ...
    if ( auto pMountItem = pList->GetWindow( iMountID ) )
    {
        if ( auto pButton = pMountItem->GetElement<UI::Button>( BUTTON_UseMount ) )
            pButton->FireEvent();
    }
}

bool CMountWindowHandler::IsInCooldownMount()
{
    return uCooldownMount < TICKCOUNT;
}

UINT CMountWindowHandler::GetCooldownMount()
{
    return uCooldownMount > TICKCOUNT ? (uCooldownMount - TICKCOUNT) : 0;
}

UINT CMountWindowHandler::GetCooldownMountPercent()
{
    return (GetCooldownMount() * 100) / (uCooldownMountMax > 0 ? uCooldownMountMax : 1);
}

void CMountWindowHandler::SetCooldownMount( UINT uTimeMS )
{
    uCooldownMount = TICKCOUNT + uTimeMS;

    uCooldownMountMax = uTimeMS;
}
...
void CMountWindowHandler::AddMountWindow( int iID, const std::string strMountName, const std::string strMountImageFile, EMountType eMountType, EMountRareType eRareType, BOOL bIsSelected )
{
    ...
    pWindowMountList->SetHoverEvent( UI::Event::Build( std::bind( &CMountWindowHandler::OnHoverMountWindow, this, pWindowMountList->GetID(), std::placeholders::_1 ) ) );
    ...
    pButtonMount->SetID( BUTTON_UseMount );
...
{
    if ( IsInCooldownMount() )
    {
        if ( (UNITDATA->GetAnimation() != ANIMATIONTYPE_Die) && (UNITDATA->GetAnimation() != ANIMATIONTYPE_Attack) && (UNITDATA->GetAnimation() != ANIMATIONTYPE_Skill) )
        {
            OnButtonSelectMountWindow( iID, eMountType, eArgs );

            MOUNTHANDLER->Mount();

            SetCooldownMount( 3000 );
        }
    }
    else
    {
        float fSeconds = ((float)GetCooldownMount() / 1000.0f);
        if ( fSeconds < 0.1f )
            fSeconds = 0.1f;

        TITLEBOX( AY_OBFUSCATE("You must wait %.1f seconds"), fSeconds );
    }
}
...
void CMountWindowHandler::OnButtonSelectMountWindow( int iID, EMountType eMountType, UIEventArgs eArgs )
{
    ...
    for ( auto psMountItems : vMountItemsData )
    {
        if ( psMountItems->iMountID == (iCurrentMountWindowID - WINDOW_Mounts) )
        {
            for ( size_t i = 0; i < psMountItems->vItems.size(); i++ )
            {
            ...
            eCurrentMountRareType = psMountItems->eRareType;
...
void CMountWindowHandler::OnHoverMountWindow( int iID, UIEventArgs eArgs )
{
    iCurrentHoverMountWindowID = iID;
}
