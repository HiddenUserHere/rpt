...
void ItemBox::Render( int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	...
	if ( IsOpen() )
	{
		...
		{
			BOOL bIsPlace = IsAllowedItem( pcItemData );

			Rectangle2D sBox( iRenderX, iRenderY, iRenderWidth, iRenderHeight );

			if ( sBox.Inside( MOUSEHANDLER->GetPosition() ) )
			{
				int iPlaceX = -1;
				int iPlaceY = -1;
				int iItemX = -1;
				int iItemY = -1;
				int iCollideWidth = 0;
				int iCollideHeight = 0;
				if ( bIsPlace && (iMaxItems > 1) && !GetCollision( pcItemData, iRenderX, iRenderY, MOUSEHANDLER->GetPosition()->iX, MOUSEHANDLER->GetPosition()->iY, iPlaceX, iPlaceY, iItemX, iItemY, iCollideWidth, iCollideHeight ) )
				{
					bIsPlace = FALSE;
				}

				if ( (iItemX >= 0) && (iItemY >= 0) )
				{
					if ( bIsPlace == FALSE )
					{
						ITEMHANDLER->DrawColorBox( D3DCOLOR_RGBA( 255, 0, 0, 128 ), iRenderX + iItemX, iRenderY + iItemY, iCollideWidth, iCollideHeight );
					}
					else
					{
						ITEMHANDLER->DrawColorBox( D3DCOLOR_RGBA( 160, 235, 256, 128 ), iRenderX + iItemX, iRenderY + iItemY, iCollideWidth, iCollideHeight );
					}
				}

				if ( iMaxItems == 1 )
				{
					int iBoxWidth = bOverrideBoxSize ? iOverridedBoxWidth : iRenderWidth;
					int iBoxHeight = bOverrideBoxSize ? iOverridedBoxHeight : iRenderHeight;
					int iBoxX = bOverrideBoxSize ? (iRenderX + (iRenderWidth >> 1)) - (iBoxWidth >> 1) : iRenderX;
					int iBoxY = bOverrideBoxSize ? (iRenderY + (iRenderHeight >> 1)) - (iBoxHeight >> 1) : iRenderY;

					if ( bIsPlace )
					{
						ITEMHANDLER->DrawColorBox( vItems.size() == 1 ? D3DCOLOR_RGBA( 255, 255, 255, 128 ) : D3DCOLOR_RGBA( 160, 235, 256, 128 ), iBoxX, iBoxY, iBoxWidth, iBoxHeight );
					}
					else
					{
						ITEMHANDLER->DrawColorBox( D3DCOLOR_RGBA( 255, 0, 0, 128 ), iBoxX, iBoxY, iBoxWidth, iBoxHeight );
					}
				}
			}
		}
		...
		for ( std::vector<ItemData*>::iterator it = vItems.begin(); it != vItems.end(); it++ )
		{
			...
			if ( pc && pc->bValid )
			{
				bool bInsideItem = false;
				if ( (MOUSEHANDLER->GetPosition()->iX >= (pc->sPosition.iX + iRenderX)) && (MOUSEHANDLER->GetPosition()->iX <= (pc->sPosition.iX + iRenderX + pc->iWidth)) )
				{
					if ( (MOUSEHANDLER->GetPosition()->iY >= (pc->sPosition.iY + iRenderY)) && (MOUSEHANDLER->GetPosition()->iY <= (pc->sPosition.iY + iRenderY + pc->iHeight)) )
					{
						bInsideItem = true;
					}
				}
				...
				if ( bInsideItem )
				{
					if ( pcItemData->bValid == FALSE )
					{
						if ( iMaxItems == 1 )
						{
							...
							int iBoxX = bOverrideBoxSize ? (iRenderX + (iRenderWidth >> 1)) - (iBoxWidth >> 1) : iRenderX;
							int iBoxY = bOverrideBoxSize ? (iRenderY + (iRenderHeight >> 1)) - (iBoxHeight >> 1) : iRenderY;
							ITEMHANDLER->DrawColorBox( D3DCOLOR_RGBA( 255, 200, 0, 128 ), iBoxX, iBoxY, iBoxWidth, iBoxHeight );
						}
						else
						{
							ITEMHANDLER->DrawColorBox( D3DCOLOR_RGBA( 255, 200, 0, 128 ), (pc->sPosition.iX + iRenderX), (pc->sPosition.iY + iRenderY), pc->iWidth, pc->iHeight );
						}
					}
				}

				ITEMHANDLER->DrawSprite( iSourceX, iSourceY, (int)pc->pcInventoryTexture, (pc->sPosition.iX + iRenderX), (pc->sPosition.iY + iRenderY), pc->iWidth, pc->iHeight );
			...
			if ( ps )
			{
				if ( ps->eRule == eRule )
				{
					...
					if ( ps->eType == TYPERULE_LockItemGM )
					{
						...
						{
							if ( (pcItemData->sItem.bLockedItemType == ITEMLOCKEDTYPE_Forever) || ItemInfoBox::GetItemPlayTime( &pcItemData->sItem ) )
								iRet = i;
						}
					}
				}
...
BOOL ItemBox::GetCollision( ItemData * pcItemData, int iX, int iY, int iMouseX, int iMouseY, int & iPlaceX, int & iPlaceY, int & iItemX, int & iItemY, int & iItemWidth, int & iItemHeight )
{
	Rectangle2D sBox( iX, iY, iSlotCountX * 22, iSlotCountY * 22 );

	iItemWidth = pcItemData->iWidth;
	iItemHeight = pcItemData->iHeight;
		...
		for ( int j = 0; j < iSlotCountX; j++ )
		{
			...
			for ( std::vector<ItemData *>::iterator it = vItems.begin(); it != vItems.end(); it++ )
			{
				...
				if ( pc )
				{
					...
					{
						iCollideX = rItemRect.left + sBox.iX;
						iCollideY = rItemRect.top + sBox.iY;
						iItemWidth = pc->iWidth;
						iItemHeight = pc->iHeight;

						iItemX = rItemRect.left;
						iItemY = rItemRect.top;
...
BOOL ItemBox::AddItem(ItemData * pcItemData, int iX, int iY, int iMouseX, int iMouseY )
{
	...
	if ( iMaxItems > 1 )
	{
		...
		if ( GetCollision( pcItemData, iX, iY, iMouseX, iMouseY, iPlaceX, iPlaceY, iCollideX, iCollideY, iCollideWidth, iCollideHeight ) )
		{
