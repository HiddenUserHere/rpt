#include "stdafx.h"
#include "CStarterCoreHandler.h"


CStarterCoreHandler::CStarterCoreHandler()
{
...
void CStarterCoreHandler::Render()
{
	pWindow->Render( 0, 0, DX::Graphic::GetWidthBackBuffer(), DX::Graphic::GetHeightBackBuffer(), 0, 0 );
}

void CStarterCoreHandler::Update( float fTime )
{
	pWindow->Update( fTime );
}

void CStarterCoreHandler::Open( ItemData * pcItemData )
{
	if ( pWindow->IsOpen() == FALSE )
	{
		...
		for ( int i = 0; i < _countof( saStarterTeleportMapInfo ); i++ )
		{
			if ( saStarterTeleportMapInfo[i].bEnabled && (MAPLEVEL( saStarterTeleportMapInfo[i].iMapID ) < 200) )
			{
				auto pWindowMap = pWindow->GetElement<UI::List>( LISTID_Map )->GetWindow( WINDOWID_Map + i );

				if ( UNITDATA->sCharacterData.iLevel >= MAPLEVEL( saStarterTeleportMapInfo[i].iMapID ) )
				{
...
void CStarterCoreHandler::Close()
{
...
BOOL CStarterCoreHandler::IsOpen()
{
	return pWindow->IsOpen();
}

void CStarterCoreHandler::BuildWindow()
{
	...
	pButtonTeleport->SetEvent( UI::Event::Build( std::bind( &CStarterCoreHandler::OnClickTeleport, this, std::placeholders::_1 ) ) );
	...
	for ( int i = 0; i < _countof( saStarterTeleportMapInfo ); i++ )
	{
		if ( saStarterTeleportMapInfo[i].bEnabled && (MAPLEVEL( saStarterTeleportMapInfo[i].iMapID ) < 200) )
		{
			...
			pMapTypeImage->SetImage( UI::ImageLoader::LoadImage( GetMapTypeImageFile( saStarterTeleportMapInfo[i].iMapTypeFirst ) ) );
			...
			pTextMapName->SetMultiLine( TRUE );
			pTextMapName->SetWordWrap( TRUE );
			pTextMapName->SetTextEx( "%s", pszaMapsName[saStarterTeleportMapInfo[i].iMapID] );
			pTextMapName->SetColorHover( D3DCOLOR_XRGB( 255, 165, 0 ) );
			pTextMapName->SetEvent( UI::Event::Build( std::bind( &CStarterCoreHandler::OnClickMap, this, saStarterTeleportMapInfo[i].iMapID, i, std::placeholders::_1 ) ) );
			...
			pTextMapLevel->SetTextEx( "%d", MAPLEVEL( saStarterTeleportMapInfo[i].iMapID ) );
	...
	pButtonClose->SetEvent( UI::Event::Build( std::bind( &CStarterCoreHandler::OnButtonCloseClick, this, std::placeholders::_1 ) ) );
...
void CStarterCoreHandler::OnClickMap( int iMapID, int iIndex, UIEventArgs eArgs )
{
	...
	pWindow->GetElement<UI::ImageBox>( GetMapTypeImageID( saStarterTeleportMapInfo[iIndex].iMapTypeFirst ) )->SetPosition( 194, 260 );
	pWindow->GetElement<UI::ImageBox>( GetMapTypeImageID( saStarterTeleportMapInfo[iIndex].iMapTypeFirst ) )->Show();
...
void CStarterCoreHandler::OnClickTeleport( UIEventArgs eArgs )
{
	if ( UNITDATA->sCharacterData.iLevel >= MAPLEVEL( iMapID ) )
	{
		...
		MESSAGEBOX->SetEvent( std::bind( &CStarterCoreHandler::OnAcceptTeleport, this, iMapID, std::placeholders::_1 ) );
		MESSAGEBOX->Show();
	}
	else
...
void CStarterCoreHandler::OnAcceptTeleport( int iMapID, UIEventArgs eArgs )
{
...
void CStarterCoreHandler::OnButtonCloseClick( UIEventArgs eArgs )
{
	Close();
}

int CStarterCoreHandler::GetMapTypeImageID( ETeleportMapInfoType iType )
{
...
const char * CStarterCoreHandler::GetMapTypeImageFile( ETeleportMapInfoType iType )
{
	if ( iType == TELEPORTINFOTYPE_EXP )
