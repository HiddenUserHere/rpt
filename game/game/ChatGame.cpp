...
{ //0 = not send, 1 = ok...
	...
	//Connection Data
	if ( ChatCommand( "/data", pszBuff ) )
	{
		if ( SETTINGSHANDLER->GetModel()->GetSettings().bShowConnectionData )
		{
			AddChatBoxText( "> Connection Data is now hidden" );

			SETTINGSHANDLER->GetModel()->GetSettings().bShowConnectionData = FALSE;
		}
		else
		{
			AddChatBoxText( "> Connection Data is now visible" );

			SETTINGSHANDLER->GetModel()->GetSettings().bShowConnectionData = TRUE;
		}
	...
	if ( GM_MODE )
	{
		if ( ChatCommand( "/resetshaders", pszBuff ) )
		{
			GRAPHICS->GetShaderFactory()->Reload();
			AddChatBoxTextEx( CHATCOLOR_Error, "> Shaders Reloaded" );
		}
		...
		else if ( ChatCommand( "/TestParticle", pszBuff ) )
		{
			CParticleScript * pc = PARTICLEFACTORY->LoadScript( "game\\scripts\\particles\\LensLight.lua" );
			pc->SetOwner( new Engine::ObjectOwnerPosition( UNITDATA->sPosition ) );
		...
		{
			INVENTORYITEMS[ITEMINDEX_INVENTORY - 1].sItem.sMatureBar.sCur = 0;
...
void ChatGame::AddChatBoxText( const char * pszText, EChatColor iColor )
{
	CHATBOX->AddMessage( pszText, iColor );
}

void ChatGame::AddChatBoxTextEx( EChatColor iColor, const char * pszText, ... )
{
	va_list vl;
