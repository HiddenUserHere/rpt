...
void TitleBox::Render()
{
	...
	for( auto &v : vTitles )
	{
		...
		int iW = GETWIDTHTEXT( v.strText.c_str() );
		int iX = (RESOLUTION_WIDTH >> 1) - ((iW + 64) >> 1);
		int iY = v.sPosition.iY;
		...
		UI::ImageRender::Render( pImageLeft, iX, iY, pImageLeft->GetWidth(), pImageLeft->GetHeight(), DX::Color( (int)v.d3dcolor.a, 255, 255, 255 ).Get() );
		UI::ImageRender::Render( pImageCenter, iX + 32, iY, iW, pImageLeft->GetHeight(), DX::Color( (int)v.d3dcolor.a, 255, 255, 255 ).Get() );
		UI::ImageRender::Render( pImageRight, iX + 32 + iW, iY, pImageLeft->GetWidth(), pImageLeft->GetHeight(), DX::Color( (int)v.d3dcolor.a, 255, 255, 255 ).Get() );

		GetGraphicsDevice()->GetDefaultFont()->Draw( v.strText, iX + 32, iY + 13, 0, DX::Color( (int)v.d3dcolor.a, 255, 255, 255 ) );
	...
	for( auto &v : vTitles )
	{
		if( v.bMoveToDown )
		{
			...
			if( i == 0 )
				...
				v.sPosition.iY = (int)( v.iBasePositionY + ( 45 * ( 1.0f - f ) ) );
		}
