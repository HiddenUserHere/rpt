...
{
	pWindow = std::make_shared<UI::Window>( Rectangle2D( 574, 39, 216, 112 ) );
...
{
	UI::Window_ptr pWindowWeapon = std::make_shared<UI::Window>( Rectangle2D( 0, 0, 66, 112 ) );
	pWindowWeapon->SetID( WINDOW_Weapon );
	{
		UI::ItemBox_ptr pItemBoxWeapon = std::make_shared<UI::ItemBox>( 0, 6, 3, 4 );
		...
		pItemBoxWeapon->SetOverrideBoxSize( TRUE, 65, 110 );
		pWindowWeapon->AddElement( pItemBoxWeapon );

		UI::Window_ptr pWindowTwoHand = std::make_shared<UI::Window>( Rectangle2D( 148, 0, 67, 112 ) );
	...
	UI::Window_ptr pWindowShield = std::make_shared<UI::Window>( Rectangle2D( 148, 0, 66, 112 ) );
	pWindowShield->SetID( WINDOW_Shield );
	{
		UI::ItemBox_ptr pItemBoxShield = std::make_shared<UI::ItemBox>( 0, 6, 3, 4 );
		...
		pItemBoxShield->SetOverrideBoxSize( TRUE, 65, 110 );
...
void CCostumeWindowHandler::OnBeforePutItemWeapon( UIEventArgs eArgs )
{
	if ( auto pcItemData = GetWeaponItemBox()->GetEventItemData() )
	{
		if ( pcItemData->bValid )
		{
			...
			bool bAllowedItem = IsAllowedWeapon( pcItemData );
...
void CCostumeWindowHandler::OnBeforePutItemShield( UIEventArgs eArgs )
{
	if ( auto pcItemData = GetShieldItemBox()->GetEventItemData() )
	{
		if ( pcItemData->bValid )
		{
			...
			bool bAllowedItem = IsAllowedShield( pcItemData );
...
bool CCostumeWindowHandler::IsAllowedWeapon( ItemData * pcItemData )
{
	EItemID eItemID = pcItemData->sItem.sItemID.ToItemID();
...
bool CCostumeWindowHandler::IsAllowedShield( ItemData * pcItemData )
{
	EItemID eItemID = pcItemData->sItem.sItemID.ToItemID();
