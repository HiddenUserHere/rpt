...
BOOL CGameCharacterStatus::UpdateCharacterStatusItem( BOOL bCheckHack )
{
	...
	CharacterDataLocal sCharacterDataLocal, sCharacterDataLocalItem1, sCharacterDataLocalItem2;
	ZeroMemory( &sCharacterDataLocal, sizeof( CharacterDataLocal ) );
	ZeroMemory( &sCharacterDataLocalItem1, sizeof( CharacterDataLocal ) );
	ZeroMemory( &sCharacterDataLocalItem2, sizeof( CharacterDataLocal ) );
	...
	for ( int i = 0; i < INVENTORY_DATACOUNT; i++ )
	{
		...
		if ( pcItemData->bValid )
		{
			...
			if ( pcItemData->iItemSlot && (pcItemData->sItem.bCanNotUse == FALSE) )
			{
				if ( pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Dagger )
				{
					AddStatusItem( pcItemData->iItemSlot == ITEMSLOT_RightHand ? sCharacterDataLocalItem1 : sCharacterDataLocalItem2, pcItemData );
				}
				else
	...
	HalfStatus( sCharacterDataLocalItem1, sCharacterDataLocalItem2, sCharacterDataLocal );
	...
	if ( Game::GetGameMode() == GAMEMODE_InGame )
	{
		if ( UIHANDLER->GetCharacterStatusWindow() )
			UIHANDLER->GetCharacterStatusWindow()->UpdateRequiredStatus();
		...
		*piEvade = 0;
		*piBlock = (int)(sCharacterDataLocal.fBlockRating + sCharacterDataLocal.fBlockRatingAdd);
		*piCritical = 0;
			...
			switch ( psSkill->iID )
			{
				...
				case SKILLID_Automation:
				{
					if ( INVENTORYITEMSLOT[ITEMSLOT_RightHand - 1].iItemIndex )
					{
						...
						if ( (pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Bow) || (pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Javelin) )
						{
							...
							if ( (CHARACTERGAME->GetCurrentMP() > 0) && (CHARACTERGAME->GetCurrentSP() > 0) )
								sCharacterDataLocal.iAttackSpeedAdd += iaMechT23AtkSpeed[psSkill->iLevel - 1];
				...
				case SKILLID_MetalArmor:
				{
					if ( bMetalArmorSkillActive )
					{
						...
						sCharacterDataLocal.iDefenseSkills += (int)(((float)sCharacterDataLocal.iTempDefense * (float)iaMechT31DefenseRtg[psSkill->iLevel - 1]) / 100.0f);
					}
				}
				break;
				...
				case SKILLID_HallofValhalla:
					...
					sCharacterDataLocal.iEvade += ((int *)0x00994D08)[psSkill->iLevel - 1];
					break;

				case SKILLID_SummonMuspell:
					sCharacterDataLocal.iEvade += ((int *)0x00997170)[psSkill->iLevel - 1];
					break;
				...
				case SKILLID_FrostJavelin:
				{
					if ( INVENTORYITEMSLOT[ITEMSLOT_RightHand - 1].iItemIndex )
					{
						...
						if ( pcItemData->sItem.sItemID.ToItemType() == ITEMTYPE_Javelin )
						{
							...
							psDamageFake->iMin += psaFrostJavelin[psSkill->iLevel - 1].iMin;
							psDamageFake->iMax += psaFrostJavelin[psSkill->iLevel - 1].iMax;
						}
						else
						{
				...
				case SKILLID_AdventMigal:
					psDamageFake->iMin += ((int *)0x00998BB0)[psSkill->iLevel - 1];
					psDamageFake->iMax += ((int *)0x00998BB0)[psSkill->iLevel - 1];

					SETSTATUSCOLOR( CHARSTATUSTYPECOLOR_Damage, CHARSTATUSCOLOR_Gold );
					break;

				case SKILLID_AdventMidranda:
					sCharacterDataLocal.iSkillWeaponSpeed += ((sCharacterDataLocal.iWeaponSpeed + sCharacterDataLocal.iAttackSpeedAdd + sCharacterDataLocal.iSkillWeaponSpeed) * iaShaT43AttackSpeed[psSkill->iLevel - 1]) / 100;
					break;

				case SKILLID_Inpes:
					sCharacterDataLocal.iSkillWeaponSpeed += ((sCharacterDataLocal.iWeaponSpeed + sCharacterDataLocal.iAttackSpeedAdd + sCharacterDataLocal.iSkillWeaponSpeed) * iaASNT33AttackSpeed[psSkill->iLevel - 1]) / 100;
					break;

				case SKILLID_Alas:
					{
						int * iSkillTable = (int *)0x00998160;
						int iPercentSkill = iSkillTable[psSkill->iLevel - 1];

						if ( READDWORD( 0x04B0DA54 ) == 1 )
							iPercentSkill >>= 1;

						sCharacterDataLocal.iEvade += iPercentSkill;
					}
					break;
				...
				case SKILLID_ChasingHunt:
				{
					if ( (UNITDATA->sRightHandTool.eItemID & 0xFFFF0000) == 0x01090000 )
					{
						...
						if ( psItemData )
						{
							...
							int iPercentSkill = iSkillTable1[psSkill->iLevel - 1];
							psDamageFake->iMin += (UNITDATA->sCharacterData.iMinDamage * iPercentSkill) / 100;

							iPercentSkill = iSkillTable2[psSkill->iLevel - 1];
							psDamageFake->iMax += (UNITDATA->sCharacterData.iMaxDamage * iPercentSkill) / 100;
						}
					}
				}
				break;
		...
		if ( psSkill->bActive && psSkill->iLevel )
		{
			switch ( psSkill->iID )
			{
				case SKILLID_EvasionMastery:
					sCharacterDataLocal.iEvade += ((int *)0x009943A8)[psSkill->iLevel - 1];
					break;
		...
		if ( psSkill->bActive && psSkill->iLevel )
		{
			switch ( psSkill->iID )
			{
				case SKILLID_PhysicalTraining:
				{
					...
					UNITDATA->sCharacterData.sSP.sMax += (short)fTrain;
				}
				break;
	...
	for ( int i = 0; i < MAX_SKILLS_TIMER; i++ )
	{
		...
		if ( psSkill->bActive )
		{
			switch ( psSkill->iID )
			{
				...
				case SKILLID_Berserker:
					UNITDATA->sCharacterData.iAbsorbRating += (int)((((int *)0x009957D0)[psSkill->iLevel - 1] * UNITDATA->sCharacterData.iAbsorbRating) / 100);
				...
				case SKILLID_Maximize:
				{
					...
					psDamageFake->iMin += (int)(((float)UNITDATA->sCharacterData.iMinDamage * (float)psSkill->PlusState[0]) / 100.0f);
					psDamageFake->iMax += (int)(((float)UNITDATA->sCharacterData.iMaxDamage * (float)psSkill->PlusState[0]) / 100.0f);
				...
				case SKILLID_PvPHonor:
				{
					if ( (MAP_ID != MAPID_Bellatra) && (MAP_ID != MAPID_BlessCastle) )
					{
						...
						psDamageFake->iMin += (int)(((float)UNITDATA->sCharacterData.iMinDamage * (float)iAddAttackPower) / 100.0f);
						psDamageFake->iMax += (int)(((float)UNITDATA->sCharacterData.iMaxDamage * (float)iAddAttackPower) / 100.0f);
					}
				}
				break;
				...
				case SKILLID_SwiftAxe:
				{
					if ( INVENTORYITEMSLOT[ITEMSLOT_RightHand - 1].iItemIndex >= 0 )
					{
						...
						{
							UNITDATA->sCharacterData.iAttackSpeed += (UNITDATA->sCharacterData.iAttackSpeed * iaFighterT33AttackSpeed[psSkill->iLevel - 1]) / 100;
						}
					}
				}
				break;
...
bool CGameCharacterStatus::HalfStatus( CharacterDataLocal & sCharacterDataLocalItem1, CharacterDataLocal & sCharacterDataLocalItem2, CharacterDataLocal & sCharacterDataLocal )
{
	sCharacterDataLocal.iAttackRatingSkill += (sCharacterDataLocalItem1.iAttackRatingSkill + sCharacterDataLocalItem2.iAttackRatingSkill) / 2;

	sCharacterDataLocal.iMinDamage += (sCharacterDataLocalItem1.iMinDamage + sCharacterDataLocalItem2.iMinDamage) / 2;
	sCharacterDataLocal.iMaxDamage += (sCharacterDataLocalItem1.iMaxDamage + sCharacterDataLocalItem2.iMaxDamage) / 2;

	sCharacterDataLocal.iCritical += (sCharacterDataLocalItem1.iCritical + sCharacterDataLocalItem2.iCritical) / 2;
	sCharacterDataLocal.iCriticalItems += (sCharacterDataLocalItem1.iCriticalItems + sCharacterDataLocalItem2.iCriticalItems) / 2;

	sCharacterDataLocal.iDefenseAdd += (sCharacterDataLocalItem1.iDefenseAdd + sCharacterDataLocalItem2.iDefenseAdd) / 2;

	sCharacterDataLocal.fAbsorbAdd += (sCharacterDataLocalItem1.fAbsorbAdd + sCharacterDataLocalItem2.fAbsorbAdd) / 2;
	sCharacterDataLocal.iCountAbsorbItems += (sCharacterDataLocalItem1.iCountAbsorbItems + sCharacterDataLocalItem2.iCountAbsorbItems) / 2;

	sCharacterDataLocal.fBlockRating += (sCharacterDataLocalItem1.fBlockRating + sCharacterDataLocalItem2.fBlockRating) / 2;

	sCharacterDataLocal.iEvade += (sCharacterDataLocalItem1.iEvade + sCharacterDataLocalItem2.iEvade) / 2;

	sCharacterDataLocal.iWeaponSpeed += (sCharacterDataLocalItem1.iWeaponSpeed + sCharacterDataLocalItem2.iWeaponSpeed) / 2;

	sCharacterDataLocal.iAttackRange += (sCharacterDataLocalItem1.iAttackRange + sCharacterDataLocalItem2.iAttackRange) / 2;

	sCharacterDataLocal.fMagicMastery += (sCharacterDataLocalItem1.fMagicMastery + sCharacterDataLocalItem2.fMagicMastery) / 2;

	sCharacterDataLocal.fMovementSpeed += (sCharacterDataLocalItem1.fMovementSpeed + sCharacterDataLocalItem2.fMovementSpeed) / 2;

	sCharacterDataLocal.fRegenHP += (sCharacterDataLocalItem1.fRegenHP + sCharacterDataLocalItem2.fRegenHP) / 2;

	sCharacterDataLocal.fRegenMP += (sCharacterDataLocalItem1.fRegenMP + sCharacterDataLocalItem2.fRegenMP) / 2;

	sCharacterDataLocal.fRegenSP += (sCharacterDataLocalItem1.fRegenSP + sCharacterDataLocalItem2.fRegenSP) / 2;

	sCharacterDataLocal.fIncreaseHPAdd += (sCharacterDataLocalItem1.fIncreaseHPAdd + sCharacterDataLocalItem2.fIncreaseHPAdd) / 2;

	sCharacterDataLocal.fIncreaseMPAdd += (sCharacterDataLocalItem1.fIncreaseMPAdd + sCharacterDataLocalItem2.fIncreaseMPAdd) / 2;

	sCharacterDataLocal.fIncreaseSPAdd += (sCharacterDataLocalItem1.fIncreaseSPAdd + sCharacterDataLocalItem2.fIncreaseSPAdd) / 2;

	sCharacterDataLocal.iPotionSpace += (sCharacterDataLocalItem1.iPotionSpace + sCharacterDataLocalItem2.iPotionSpace) / 2;

	sCharacterDataLocal.iLevelMP += (sCharacterDataLocalItem1.iLevelMP + sCharacterDataLocalItem2.iLevelMP) / 2;
	sCharacterDataLocal.iLevelHP += (sCharacterDataLocalItem1.iLevelHP + sCharacterDataLocalItem2.iLevelHP) / 2;

	sCharacterDataLocal.iLevelAttackRating += (sCharacterDataLocalItem1.iLevelAttackRating + sCharacterDataLocalItem2.iLevelAttackRating) / 2;

	sCharacterDataLocal.iLevelDamage += (sCharacterDataLocalItem1.iLevelDamage + sCharacterDataLocalItem2.iLevelDamage) / 2;

	sCharacterDataLocal.fPercentMPAdd += (sCharacterDataLocalItem1.fPercentMPAdd + sCharacterDataLocalItem2.fPercentMPAdd) / 2;
	sCharacterDataLocal.fPercentHPAdd += (sCharacterDataLocalItem1.fPercentHPAdd + sCharacterDataLocalItem2.fPercentHPAdd) / 2;
	sCharacterDataLocal.fPercentSPAdd += (sCharacterDataLocalItem1.fPercentSPAdd + sCharacterDataLocalItem2.fPercentSPAdd) / 2;

	sCharacterDataLocal.fBlockRatingAdd += (sCharacterDataLocalItem1.fBlockRatingAdd + sCharacterDataLocalItem2.fBlockRatingAdd) / 2;

	sCharacterDataLocal.iAttackSpeedAdd += (sCharacterDataLocalItem1.iAttackSpeedAdd + sCharacterDataLocalItem2.iAttackSpeedAdd) / 2;

	//Resistances
	for ( int i = 0; i < 8; i++ )
	{
		sCharacterDataLocal.iaElementalSpecDef[i] += (sCharacterDataLocalItem1.iaElementalSpecDef[i] + sCharacterDataLocalItem2.iaElementalSpecDef[i]) / 2;

		sCharacterDataLocal.iaElementalSpecAtk[i] += (sCharacterDataLocalItem1.iaElementalSpecAtk[i] + sCharacterDataLocalItem2.iaElementalSpecAtk[i]) / 2;

		sCharacterDataLocal.iaResistance[i] += (sCharacterDataLocalItem1.iaResistance[i] + sCharacterDataLocalItem2.iaResistance[i]) / 2;
	}
