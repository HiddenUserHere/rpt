...
DropItemView saItemsStage[MAX_ITEM_STAGE_GAME];

UI::ItemBox_ptr pItemBox = std::make_shared<UI::ItemBox>( 0, 0 );

const bool USE_OLD_MODE_TIMER = false;
...
GameCore::GameCore()
{
	...
	pcStarterCoreHandler				= new CStarterCoreHandler();
	...
	pcActionBarHandler					= new CActionBarHandler();
	...
	auto ReadChecksumFileCRC32 = []( const std::string & strFile ) -> std::uint32_t
	{
		...
		while ( is.read( &buffer[0], buffer.size() ) )
		{
			...
			for( std::size_t i = 0; i < count; ++i )
				result.update( buffer[i] );
		}

		return result.final();
	};

	Game::SetGameChecksum( ReadChecksumFileCRC32( std::string( AY_OBFUSCATE( "game.exe" ) ) ) );
	Game::SetDLLChecksum( ReadChecksumFileCRC32( std::string( AY_OBFUSCATE( "game.dll" ) ) ) );
...
//	CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)&CAntiCheat::DebugSelf, (new SDebugSelf{ GetCurrentProcessId(), GetCurrentThreadId() }), 0, NULL );
	...
	// Items Stage
	{
		...
		WRITEDWORD( 0x004019AC, (DWORD)(&saItemsStage[MAX_ITEM_STAGE_GAME-1]) + 12 );
		WRITEDWORD( 0x0061EBDA, (DWORD)(&saItemsStage[MAX_ITEM_STAGE_GAME-1]) + 12 );

		WRITEDWORD( 0x0061EC03, (DWORD)(&saItemsStage[0]) + 8 );
		WRITEDWORD( 0x0061EC15, (DWORD)(&saItemsStage[MAX_ITEM_STAGE_GAME-1]) + 8 );

		WRITEDWORD( 0x004182DD, (DWORD)(&saItemsStage[0]) + 168 );
		WRITEDWORD( 0x00453FBF, (DWORD)(&saItemsStage[0]) + 168 );
		WRITEDWORD( 0x0041833D, (DWORD)(&saItemsStage[MAX_ITEM_STAGE_GAME-1]) + 168 );
		WRITEDWORD( 0x004540D3, (DWORD)(&saItemsStage[MAX_ITEM_STAGE_GAME-1]) + 168 );
...
BOOL GameCore::OnKeyPress(CKeyboard * pcKeyboard)
{
	...
	if ( pcMountHandler->GetWindow()->OnKeyPress( pcKeyboard ) )
		return TRUE;
	if ( pcActionBarHandler->OnKeyDown( pcKeyboard ) )
		return TRUE;
...
void GameCore::OnResolutionChanged()
{
	...
	pcStarterCoreHandler->OnResolutionChanged();
	...
	pcActionBarHandler->OnResolutionChanged();
...
{
	if ( pcActionBarHandler->OnMouseClick( pcMouse ) )
		return TRUE;
	...
	if ( pcStarterCoreHandler->OnMouseClick( pcMouse ) )
		return TRUE;
...
BOOL GameCore::OnMouseScroll( class CMouse * pcMouse )
{
	...
	if ( pcTeleportCoreHandler->OnMouseScroll( pcMouse ) )
		return TRUE;
	if ( pcStarterCoreHandler->OnMouseScroll( pcMouse ) )
		return TRUE;
...
void GameCore::OnMouseMove( class CMouse * pcMouse )
{
	...
	pcUIControllerHandler->OnMouseMove( pcMouse );
	...
	pcStarterCoreHandler->OnMouseMove( pcMouse );
	...
	pcActionBarHandler->OnMouseMove( pcMouse );
}
...
void GameCore::Update( float fTime )
{
	...
	while ( fCount_14MS >= fFPS70 )
	{
		...
		fCount_14MS -= fTimeAdd;
	}

	while ( fCount_16MS >= fFPS60 )
	{
		double fTimeAdd = USE_OLD_MODE_TIMER ? fFPS60 : fCount_16MS;

		Core::Timer::Update( (float)fTimeAdd );

		GRAPHICS->GetRenderer()->Update( (float)fTimeAdd );

		pcPersonalShopHandler->GetWindow()->Update( (float)fTimeAdd );

		pcBattleRoyaleHandler->Update( (float)fTimeAdd );

		pcCaravanHandler->GetWindow()->Update( (float)fTimeAdd );

		pcHUDHandler->GetAchievementHandler()->GetWindow()->Update( (float)fTimeAdd );

		pcEffekseerHandler->Update( (float)fTimeAdd );

		pcDropItemAreaHandler->Update( (float)fTimeAdd );

		pcCarnivalHandler->Update( (float)fTimeAdd );

		pcTestWindowHandler->Update( (float)fTimeAdd );

		pcRankingHandler->Update( (float)fTimeAdd );
		
		pcMountHandler->GetWindow()->Update( (float)fTimeAdd );

		pcUIControllerHandler->Update( (float)fTimeAdd );

		pcActionBarHandler->Update( (float)fTimeAdd );
		...
		pcQuestMapHandler->Update( (float)fTimeAdd );

		pcKnockupUnitHandler->Update( (float)fTimeAdd );

		pcActionFieldHandler->Update( (float)fTimeAdd );

		pcTitleBoxHandler->Update( (float)fTimeAdd );

		//Map Dissolve
		if ( Game::GetGameMode() == GAMEMODE_InGame )
		{
			if ( STAGE1 && (STAGE1->fDissolveMapAmount > 0.0f) )
				STAGE1->fDissolveMapAmount -= (float)fTimeAdd / 2000.0f;
			if ( STAGE2 && (STAGE2->fDissolveMapAmount > 0.0f) )
				STAGE2->fDissolveMapAmount -= (float)fTimeAdd / 2000.0f;

			pcItemInfoBox->Update( (float)fTimeAdd );
		}

		fCount_16MS -= fTimeAdd;
	}
	
	// 100 ms
	while ( fCount_100MS >= f100MS )
	{
		double fTimeAdd = USE_OLD_MODE_TIMER ? f100MS : fCount_100MS;

		pcShamanHandler->Update( (float)fTimeAdd );
		pcSoundHandler->Update();
		GameCore::pcManageWindow->Tick();
		pcHUDHandler->Update( (float)fTimeAdd );
		pcSettingsHandler->GetWindow()->Update( (float)fTimeAdd );

		pcItemTimerHandler->Loop();

		pcBlessCastleHandler->Update( (float)fTimeAdd );

		pcTeleportCoreHandler->Update( (float)fTimeAdd );
		
		pcStarterCoreHandler->Update( (float)fTimeAdd );

		pcHalloweenHandler->Update( (float)fTimeAdd );

		pcChristmasTreeHandler->Update( (float)fTimeAdd );

		pcSocialWindowHandler->Update( (float)fTimeAdd );

		SocketGame::GetInstance()->Tick( TICKTYPE_100MilliSecond );

		pcGlamorousBrushHandler->Update( (float)fTimeAdd );

		pcMountHandler->Update( (float)fTimeAdd );

		Game::FlushLoaders();

		fCount_100MS -= fTimeAdd;
	}
	// 1000 ms
	while ( fCount_1SECO >= f1SECO )
	{
		double fTimeAdd = USE_OLD_MODE_TIMER ? f1SECO : fCount_1SECO;
		...
		pcItemBoxHandler->GetWindow()->Update( (float)fTimeAdd );

		MESSAGEBOX->Update();

		pcSocialWindowHandler->GetClan()->Update();

		pcAkatsukiHandler->Update( (float)fTimeAdd );

		pcMessageBalloon->Update( (float)fTimeAdd );

		pcWarehouseHandler->Update();
		...
		pcAntiCheatHandler->Update( (float)fTimeAdd );

		fCount_1SECO -= fTimeAdd;
	}
...
DWORD GameCore::OnHandlerFunctions( DWORD dwCode, DWORD * pdwOptional )
{
	...
	switch ( dwCode )
	{
        ...
        case 0x2C:
		{
			HUDHANDLER->GetInventoryWindow()->SetCostumeInventoryOpened( (BOOL)pdwOptional[0] );
		...
		case 0x2F:
			SKILLMANAGERHANDLER->OnLoadSkillsIcon();
			break;

		case 0x30:
			SKILLMANAGERHANDLER->OnRenderShortKeys( 0 );
			break;

		case 0x31:
			SKILLMANAGERHANDLER->OnRenderShortKeys( 17 );
			break;
		...
		case 0x32:
		{
			...
			if ( iCostumeItemIndex <= 0 )
				ITEMHANDLER->SetCharacterItem( ITEMID_None, ITEMSLOTFLAG_Armor, FALSE );
		}
		break;

		case 0x33:
			dwRet = TraceAttackPlayGate();
			break;
...
void GameCore::SendLoginData()
{
	...
	STRINGCOPY( sPacket.szNewHardwareID, Game::GetNewHardwareID() );
	...
	UINT uPacketChecksum = 0;
	for ( int i = 8; i < sizeof( PacketLoginUser ) - 12; i++ )
	{	
		if ( pPacket[i] != 0 )
		{
			pPacket[i] ^= 8;
		...
		uPacketChecksum += pPacket[i];
	}

	sPacket.uPacketChecksum = uPacketChecksum;

	SENDPACKETL( &sPacket );
}

void __stdcall GameCore::SendRequestInitialData( bool bLoginServer )
{
	PacketSimple sPacket;
	sPacket.iLength = sizeof( PacketSimple );
	sPacket.iHeader = PKTHDR_RequestInitialData;

	if ( bLoginServer )
		SENDPACKETL( &sPacket );
	else if ( SOCKETL != SOCKETG )
		SENDPACKETG( &sPacket );
}
