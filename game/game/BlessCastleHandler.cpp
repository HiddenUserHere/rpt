...
void CBlessCastleHandler::SetCastleLeaderClanID( int iClanID )
{
	WRITEINT( pfnBlessCastleClanID, iClanID );
}
...
BOOL CBlessCastleHandler::SetBlessCastleMasterClanID( int iClanID, int iUnkown )
{
	if ( ((iUnkown == 0) && (MAP_ID == MAPID_BlessCastle)) == false )
	{
		if ( GetCastleLeaderClanID() != iClanID )
		{
			UpdateCrown( iClanID );
		}

		SetCastleLeaderClanID( iClanID );
	}

	return TRUE;
}

BOOL CBlessCastleHandler::UpdateCrown( int iClanID )
{
	UpdateUnitDataCrown( UNITDATA, iClanID );

	for ( int i = 0; i < MAX_UNITS; i++ )
	{
		UnitData * pcUnitData = UNITGAME->pcaUnitData + i;
		if ( pcUnitData->bActive && pcUnitData->iID )
		{
			UpdateUnitDataCrown( pcUnitData, iClanID );
		}
	}
...
BOOL CBlessCastleHandler::UpdateUnitDataCrown( UnitData * pcUnitData, int iClanID )
{
	if ( pcUnitData->sCharacterData.iClanID && GetClanID( pcUnitData->sCharacterData.iClanID ) == iClanID )
	{
		pcUnitData->bBlessCastleCrown = TRUE;
	}
	else
	{
		pcUnitData->bBlessCastleCrown = FALSE;
	}

	return pcUnitData->bBlessCastleCrown;
}
