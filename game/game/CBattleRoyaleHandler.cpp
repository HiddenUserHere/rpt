...
void CBattleRoyaleHandler::HandlePacket( PacketBattleRoyaleBattleTime * psPacket )
{
	...
	if ( psPacket->bRemoveItems )
	{
		...
		CGameInventory::SetCostumeInventoryOpen( FALSE );
		...
		HUDHANDLER->GetInventoryWindow()->SetCostumeInventoryOpened( FALSE );
	...
	else if ( psPacket->bStart == FALSE )
	{
		...
		if ( pcaItemData )
		{
			...
			HUDHANDLER->GetInventoryWindow()->SetCostumeInventoryOpened( CGameInventory::GetIsCostumeInventoryOpen() );
		}
		...
		if ( InBattle() )
		{
			if ( CBattleRoyale::IsTeamRoyalTickets( sState.iBattleID ) )
			{
				...
				for (int i = 0; i < iSpawnCoordinatesCount; i++)
				{
					...
					s.iX = RandomI(sCoordinate.iWest, sCoordinate.iEast) << 8;
					s.iZ = RandomI(sCoordinate.iSouth, sCoordinate.iNorth) << 8;
					s.iY = 0;
