...
{
    extern inline int GetValueFromGameBaseTime( int iValue );

    pcUnitData->iMovementSpeed = GetValueFromGameBaseTime( 250 + 10 * pcUnitData->sCharacterData.iMovementSpeed );
    ...
    if ( (pcUnitData->bRunMode == FALSE) || pcUnitData->iProjectileType || (CHARACTERGAME->GetCurrentSP() == 0) )
    {
        ...
        {
            if ( ((pcUnitData->iWaterHeight == (-32767)) || (pcUnitData->sPosition.iY != pcUnitData->iWaterHeight)) )
            {
                if ( bRiding == false )
        ...
        if ( bRiding )
        {
            if ( pcUnitData->bWalking && (pcUnitData->GetAnimation() != ANIMATIONTYPE_Running) )
...
int EXECharacter::AnimateCharacter( UnitData * pcUnitData, int iAnimationType )
{
    ...
    if ( pcUnitData == UNITDATA )
    {
        if ( bRidingMount )
        {
            if ( iAnimationType == ANIMATIONTYPE_Die )
                MOUNTHANDLER->RemoveMount( pcUnitData, 6000, false );
            else if ( iAnimationType >= ANIMATIONTYPE_Attack )
            {
                if ( (iAnimationType == ANIMATIONTYPE_Attack) || (iAnimationType == ANIMATIONTYPE_Skill) )
                    MOUNTHANDLER->RemoveMount( pcUnitData, 3000, false );
                else if ( iAnimationType == ANIMATIONTYPE_Flinch )
                    MOUNTHANDLER->RemoveMount( pcUnitData, 3000, false );
                else if ( iAnimationType == ANIMATIONTYPE_Revive )
                {
                    MOUNTHANDLER->SetCooldownMount( 6000 );
                }
                else
                    return 0;
            }
        ...
        {
            if ( iAnimationType == ANIMATIONTYPE_Flinch )
                MOUNTHANDLER->SetCooldownMount( 2000 );
        }
    }
    ...
    {
        if ( (Game::GetGameMode() == GAMEMODE_Login) || (Game::GetGameMode() == GAMEMODE_Login) )
            eMapFlag = ModelAnimation::EMapTypeFlag::MAPTYPEFLAG_Town;
        else
...
//    if ( UNITDATA == pcUnitData )
    ...
    {
        if ( vAnimationID.size() > 0 )
            CHARACTERGAME->OnMotionChange( pcUnitData, vAnimationID.size() == 1 ? vAnimationID[0] : vAnimationID[rand() % vAnimationID.size()], pcUnitData->bAnimation, (EAnimationType)iAnimationType, bRidingMount );
    ...
    {
        if ( vAnimationID.size() > 0 )
            CHARACTERGAME->OnMotionChange( pcUnitData, vAnimationItemID.size() == 1 ? vAnimationItemID[0] : vAnimationItemID[rand() % vAnimationItemID.size()], pcUnitData->bAnimation, (EAnimationType)iAnimationType, bRidingMount );
