...
Skill * SkillManager::GetSkillHover( bool bIncludePassiveSkill )
{
	if ( SKILL_INDEXBUTTON >= 0 )
	{
		auto psSkill = &SKILLSCHARACTER[SKILL_INDEXBUTTON - 1];

		if( bIncludePassiveSkill )
			return psSkill->bActive ? psSkill : nullptr;
		else
		{
			if( psSkill->bActive && (psSkill->sSkillInfo.eSkillType != SKILLTYPE_Undefined) && (psSkill->sSkillInfo.eSkillType != SKILLTYPE_Passive) )
				return psSkill;
		}
	}
...
Skill * SkillManager::GetOnlySkillHoverLeveled( bool bIncludePassiveSkill )
{
	if ( auto psSkill = GetSkillHover( bIncludePassiveSkill ) )
	{
		if( psSkill->iLevel > 0 )
			return psSkill;
	}
...
void SkillManager::ResetSkillsShortcut()
{
	for ( int i = 1; i <= 20; i++ )
	{
		SKILLSCHARACTER[i].iShortKey = 0;
		SKILLSCHARACTER[i].iMouseIndexPosition = 0;
	}
}

void SkillManager::ResetSkillShortcut( int iSkillIndex )
{
	if ( (iSkillIndex >= 1) && (iSkillIndex <= 20) )
	{
		auto & sSkill = SKILLSCHARACTER[iSkillIndex];
		sSkill.iShortKey = 0;
		sSkill.iMouseIndexPosition = 0;
	}
}

void SkillManager::SetUseSkillSlot( int iSkillIndex, ESkillType eSlot )
{
	if ( (iSkillIndex >= 1) && (iSkillIndex <= 20) )
	{
		auto psSkill = &SKILLSCHARACTER[iSkillIndex - 1];
		if ( psSkill->bActive && (psSkill->iLevel > 0) )
		{
			if ( (eSlot == SKILLTYPE_Left) && ((psSkill->sSkillInfo.eSkillType == SKILLTYPE_Left) || (psSkill->sSkillInfo.eSkillType == SKILLTYPE_All)) )
			{
				SKILL_LEFTSIDE = psSkill;
			}
			else if ( (eSlot == SKILLTYPE_Right) && ((psSkill->sSkillInfo.eSkillType == SKILLTYPE_Right) || (psSkill->sSkillInfo.eSkillType == SKILLTYPE_All)) )
			{
				SKILL_RIGHTSIDE = psSkill;
			}
		}
	}
}

void SkillManager::SetSkillShortcut( int iSkillIndex, int iShortKey, int iMouseIndexPosition )
{
	if ( (iSkillIndex >= 1) && (iSkillIndex <= 20) )
	{
		auto & sSkill = SKILLSCHARACTER[iSkillIndex - 1];

		if ( sSkill.bActive && sSkill.iLevel )
		{
			static vector<int> macros = {
				VK_F1,
				VK_F2,
				VK_F3,
				VK_F4,
			...
			{
				if ( iShortKey == macros[i] )
				{
					for ( int j = 1; j <= 20; j++ )
					{
						if ( SKILLSCHARACTER[j].iShortKey == i + 1 )
						{
							SKILLSCHARACTER[j].iShortKey = 0;
							SKILLSCHARACTER[j].iMouseIndexPosition = 0;
							break;
						}
					}

					sSkill.iShortKey = i + 1;
					sSkill.iMouseIndexPosition = iMouseIndexPosition;
					break;
				}
			}
		}
...
BOOL SkillManager::SkillKeyDown( int iKeyDown )
{
	BOOL bRet = FALSE;
	...
	if ( WINDOW_SKILLS == FALSE )
		return FALSE;

	if ( auto psSkill = GetSkillHover( false ); (psSkill && psSkill->iLevel > 0) )
	{
		bool bIsSkillType = false;
		if ( SKILL_LEFTBUTTON_INDEX )
			bIsSkillType = ((psSkill->sSkillInfo.eSkillType == SKILLTYPE_Left) || (psSkill->sSkillInfo.eSkillType == SKILLTYPE_All));
		else if ( SKILL_RIGHTBUTTON_INDEX )
			bIsSkillType = ((psSkill->sSkillInfo.eSkillType == SKILLTYPE_Right) || (psSkill->sSkillInfo.eSkillType == SKILLTYPE_All));

		if ( bIsSkillType )
		{
			for ( size_t i = 0; i < macros.size(); i++ )
			{
				if ( iKeyDown == macros[i] )
				{
					psSkill->iShortKey = i + 1;
					psSkill->iMouseIndexPosition = SKILL_LEFTBUTTON_INDEX ? 1 : 2;
					SKILL_SHORTCUT_CLICK = TRUE;
					ACTIONBARHANDLER->SetSkillSlot( iKeyDown, SKILL_INDEXBUTTON, psSkill->iMouseIndexPosition );
					bRet = TRUE;
			...
			for ( int j = 1; j <= 20; j++ )
			{
				if ( j == SKILL_INDEXBUTTON - 1 )
					continue;

				if ( psSkill->iShortKey == SKILLSCHARACTER[j].iShortKey )
				{
					SKILLSCHARACTER[j].iShortKey = 0;
					break;
				}
...
Skill * SkillManager::GetSkill( UINT iSkillID )
{
	for ( int i = 0; i <= 20; i++ )
	{
		if ( SKILLSCHARACTER[i].iID == iSkillID )
			return SKILLSCHARACTER + i;
	}

	return nullptr;
}

Skill * SkillManager::GetSkillByIndex( UINT iSkillIndex )
{
	if ( (iSkillIndex >= 0) && (iSkillIndex <= 20) )
	{
		auto psSkill = SKILLSCHARACTER + iSkillIndex;
...
void SkillManager::CancelForceOrb( int iForceOrbID )
{
	...
	else
	{
		if ( sTempSkillTimerData[iForceOrbID].iID == 0x09010001 )
		{
			...
			SENDPACKETG( &sPacket );
			SetForceOrbParticle( UNITDATA, 0, UNITDATA->eForceOrbItemID );
...
NAKED void SkillManager::SetForceOrbParticle( UnitData * pcUnitData, int iTime, int iCode )
{
	JMP( 0x005C16B0 );
}

BOOL SkillManager::CancelSkill( int iSkillID )
{

	if ( MESSAGEBOX->GetType() == FALSE )
		return FALSE;
	
	TIMERSKILLHANDLER->KillTimer( iSkillID );
	PacketCancelSkill sPacket;
	sPacket.iLength = sizeof( PacketCancelSkill );
	sPacket.iHeader = PKTHDR_CancelSkill;
	sPacket.iID = iSkillID;
	SENDPACKETG( &sPacket );
	...
	ChatGame::AddChatBoxText( AY_OBFUSCATE( "> Skill buff canceled!" ) );
...
BOOL SkillManager::CanUseSkill( Skill * psSkill )
{
	...
	//Assassin Skills?
	if ( (psSkill->iID & 0xFF000000) == 0x10000000 )
	{
		if ( UI::SkillInfoBox::IsActiveSkill( (ESkillID)psSkill->iID ) || UI::SkillInfoBox::IsDebuffSkill( (ESkillID)psSkill->iID ) )
		{
			if ( (ItemID( UNITDATA->sRightHandTool.eItemID ).ToItemType() != ITEMTYPE_Dagger) || (ItemID( UNITDATA->sLeftHandTool.eItemID ).ToItemType() != ITEMTYPE_Dagger) )
				return FALSE;
		}
	}
...
bool SkillManager::SkillPlay( struct Skill * psSkill, UnitData * pcUnitData )
{
	...
	bool bFlag = (psSkill->iID == SKILLID_Healing) || (psSkill->iID == SKILLID_EnchantWeapon) ||
		(psSkill->iID == SKILLID_VirtualLife) || (psSkill->iID == SKILLID_TriumphofJavelin) || (psSkill->iID == SKILLID_IceMeteorite);
...
BOOL SkillManager::OnMouseClick( class CMouse * pcMouse )
{
	if ( pcMouse->GetEvent() == EMouseEvent::ClickDownR && *(int*)(0x0367DE8C) )
	{
		...
		if ( SKILLINFOBOX->IsCancelableSkill( (ESkillID)sTempSkillTimerData[iID].iID ) )
		{
			MESSAGEBOX->SetBoxType( CMessageBox::BOXTYPEID_YesNo );
			MESSAGEBOX->SetTitle( "Are you sure?" );
			MESSAGEBOX->SetDescription( "Do you want to cancel the Skill?" );
			MESSAGEBOX->SetEvent( std::bind( &SkillManager::CancelSkill, SKILLMANAGERHANDLER, sTempSkillTimerData[iID].iID ) );
			MESSAGEBOX->Show();
			return TRUE;
		}
	}
...
void SkillManager::OnClickSkill()
{
	...
	SKILL_LEFTBUTTON_INDEX = 0;
	//SKILL_RIGHTBUTTON_INDEX = 0;
...
bool SkillManager::IsLeftSkill( int iIndex )
{
	if ( (iIndex >= 0) && (iIndex <= 20) )
	{
		if ( SKILL_LEFTSIDE == &SKILLSCHARACTER[iIndex] )
			return true;
	}
...
bool SkillManager::IsRightSkill( int iIndex )
{
	if ( (iIndex >= 0) && (iIndex <= 20) )
	{
		if ( SKILL_RIGHTSIDE == &SKILLSCHARACTER[iIndex] )
			return true;
	}
...
{
	if ( (psPacket->iUserID == 0) && (psPacket->iUnk2 == 0) && (psPacket->iUnk3 == 0) )
	{
		switch ( UNITDATA->sCharacterData.iClass )
		{
			case CHARACTERCLASS_Archer:
			...
			case CHARACTERCLASS_Shaman:
				...
				{
					case SKILLPLAYID_T51:
						CREEDHANDLER->OnCast( psPacket->iUserID );
						break;
				}
				break;
...
std::vector<UI::Image *>						vSkillShortKeyImages;

void SkillManager::OnLoadSkillsIcon()
{
	const std::vector<std::string> vImagesPath =
	{
		"f1",
		"f2",
		"f3",
	...
	for ( size_t i = 0; i < vImagesPath.size(); i++ )
	{
		std::string strPath = "game\\images\\skills\\shortkeys\\" + vImagesPath[i] + ".png";

		vSkillShortKeyImages.push_back( UI::ImageLoader::LoadImage( strPath.c_str() ) );
	}
}

void SkillManager::OnRenderShortKeys( int iSkillStartIndex )
{
	size_t iSize = vSkillShortKeyImages.size();

	int iMaxSkills = SKILLS_TIERCOUNT;
	if( iSkillStartIndex == 0 )
		iMaxSkills = SKILLS_TIERCOUNT < 17 ? SKILLS_TIERCOUNT : 17;

	for ( int i = iSkillStartIndex; i < iMaxSkills; i++ )
	{
		if ( SKILLSCHARACTER[i].bActive )
		{
			if ( SKILLSCHARACTER[i].iShortKey )
			{
				if ( iSize > ( UINT )(SKILLSCHARACTER[i].iShortKey - 1) )
				{
					UI::ImageRender::Render( vSkillShortKeyImages[SKILLSCHARACTER[i].iShortKey - 1], USESKILLS.saSkillBox[i].BoxRect.left + 9, (RESOLUTION_HEIGHT - 600) + USESKILLS.saSkillBox[i].BoxRect.top - 6 + (256 - WINDOW_SKILLMOVE), 16, 16, 0, 0, -1 );
				}
			}
		}
	}
...
bool SkillManager::CanLoopSkill( ECharacterClass eCharacterClass, const ActiveSkill & sActiveSkill )
{
	bool bRet = false;
	...
	{
		case CHARACTERCLASS_Priestess:
			if ( sActiveSkill.cSkillTinyID == SKILLPLAYID_T52 )
				bRet = true;
			break;
...
NAKED BOOL SkillManager::ChecksumSkills()
{
	JMP( 0x0040FC70 );
}
