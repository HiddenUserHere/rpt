...
BOOL DXGraphicEngine::ResolutionChange( BOOL bFirstTime, int iWidth, int iHeight )
{
	...
	*/
	...
	const float DEGREES_PT = 41.0f;
	const float RADIANS_PT = DEGREES_PT * D3DX_PI / 180.0f;
	
	const float ASPECT_RATIO = (float)iWidth / (float)iHeight;
	const float HALF_WIDTH = tan( RADIANS_PT / 2.0f );
	const float HALF_HEIGHT = tan( RADIANS_PT / 2.0f );
	const float WH_RATIO = HALF_WIDTH / HALF_HEIGHT;
	
	
	float fOVY = RADIANS_PT;

	if ( WH_RATIO > ASPECT_RATIO )
	{
		fOVY = 2.0f * atan( HALF_WIDTH / ASPECT_RATIO );
	}

	//fOVY = fOVY * 180.0f / D3DX_PI;

	fnSetDxProjection( fOVY, iWidth, iHeight, 20.f, 4000.f );
	GRAPHICENGINE->GetCamera()->SetProjection( fOVY, ASPECT_RATIO, 20.f, 4000.f );
	GRAPHICENGINE->GetFrameCamera()->SetProjection( fOVY, ASPECT_RATIO, 20.f, 4000.f );
