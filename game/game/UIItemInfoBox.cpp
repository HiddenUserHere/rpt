...
void ItemInfoBox::Init()
{
	...
	pTextAging->SetFont( "Arial", 14, 0, FALSE, FALSE, D3DCOLOR_ARGB(255,20,20,20) );
...
BOOL ItemInfoBox::FormatItemInfo( ItemData * pcItemData )
{
	...
	{
		AddString( 0, "70 Boost Item", D3DCOLOR_ARGB( 255, 132, 50, 254 ) );
		AddString( 1, "" );
	}
...
/*			AddString( 0, "Easter Event Item", D3DCOLOR_RGBA( 0, 128, 128, 255 ) );
		...
		case ITEMID_ExpUp72H:
			AddString( 0, "25% Additional Experience", D3DCOLOR_ARGB( 255, 245, 240, 218 ) );
			AddString( 1, "" );
			break;
		case ITEMID_HPUp12H:
		case ITEMID_HPUp24H:
		...
		case ITEMID_MountRaptor3:
			AddString( 0, "Common Mount", D3DCOLOR_ARGB( 255, 180, 100, 175 ) );
			AddString( 1, "" );
			AddString( 0, "A cute common mount!", D3DCOLOR_ARGB( 255, 245, 240, 218 ) );
			AddString( 1, "" );
			break;
		...
		case ITEMID_MountRudolf:
			AddString(0, "Rare Mount", D3DCOLOR_ARGB(255, 180, 100, 175));
			AddString(1, "");
			AddString(0, "A Rare ride able mount!", D3DCOLOR_ARGB(255, 245, 240, 218));
			AddString(1, "");
			break;
		...
		case ITEMID_MountIceTiger:
			AddString(0, "Epic Mount", D3DCOLOR_ARGB(255, 180, 100, 175));
			AddString(1, "");
			AddString(0, "A Epic ride able mount!", D3DCOLOR_ARGB(255, 245, 240, 218));
			AddString(1, "");
			break;
		...
		case ITEMID_MountLion5:
			AddString(0, "Legendary Mount", D3DCOLOR_ARGB(255, 180, 100, 175));
			AddString(1, "");
			AddString(0, "A legendary ride able mount!", D3DCOLOR_ARGB(255, 245, 240, 218));
			AddString(1, "");
			break;
		...
		case ITEMID_TeleportCore:
			...
			AddString( 1, "" );
			break;
		case ITEMID_StarterCore:
			AddString(0, "Teleport to a starter field", D3DCOLOR_ARGB(255, 245, 240, 218));
			AddString(1, "");
			break;
...
BOOL ItemInfoBox::FormatItemTimeInfo( ItemData * pcItemData )
{
	...
	if ( pcItemData->sItem.bLockedItemType == 50 )
	{
		AddString( 0, "" );
		AddString( 1, "" );
		AddString( 0, std::string( AY_OBFUSCATE( "Item is locked" ) ), D3DCOLOR_ARGB( 255, 125, 180, 175 ) );
		AddString( 1, "\n " );
		bResult = TRUE;
	}
	...
	if( *( BOOL* )0x036286EC )
	{
		...
		if( cItemWrapper.sItem.eCraftType == ITEMCRAFTTYPE_Aging )
		{
			...
			if( cItemWrapper.sItem.sMatureBar.sMax )
			{
				...
				if( pImageAgingBar )
					UI::ImageRender::Render( pImageAgingBar, ( iX + 92 ) - ( 130 >> 1 ), iY + 26 + iYExtra, 130, 19, 0, 0, -1 );
			
				if( pImageAgingBarGage )
					UI::ImageRender::Render( pImageAgingBarGage, ( ( iX + 92 ) - ( 125 >> 1 ) ) - 1, iY + 33 + iYExtra, iBarLenght, 4, 0, 0, -1 );
			}
			else
			{
				if( pImageAgingBarNum )
					UI::ImageRender::Render( pImageAgingBarNum, ( iX + 92 ) - ( 26 >> 1 ), iY + 26 + iYExtra, 23, 19, 0, 0, -1 );
			}
...
void ItemInfoBox::Update( float fTime )
{
	...
	{
		float fNewTime = fTime / (1000.0f / 70.0f);
		if( bFadeToColor )
		{
			fProgress += 0.01f * fNewTime;
