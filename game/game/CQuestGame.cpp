	...
	if ( pcQuestData->GetItemCounter( 0 ) > 0 )
	{
		for ( int i = 0; i < QUEST_MONSTERITEM_MAX; i++ )
		{
			...
			if ( pcQuestData->GetItemCounter( i ) > 0 )
			{
				...
				if ( pc )
					pcQuestData->SetItemName( i, pc->szBaseName );
			}
		}
	}
...
void CQuestGame::OnQuestT5Start()
{
	...
	{
		TITLEBOX( AY_OBFUSCATE("You must be Lv.80 or higher to get this quest!") );
	}
}
...
void CQuestGame::SkipNewPlayerQuests(BOOL bChatMessage)
{
