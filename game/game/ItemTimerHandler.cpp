...
{
	ItemTimer * pcEXP = new ItemTimer( ITEMTIMERTYPE_ExpUp, ITEMID_None, "EXP Potion", "25% Additional Experience", 0, 0, NULL, NULL );
...
void CItemTimerHandler::HandlePacket( PacketRequestBeginnerItemsTimer * psPacket )
{
	...
	{
		MESSAGEBOX->SetBoxType( CMessageBox::BOXTYPEID_YesNo );
		MESSAGEBOX->SetTitle( "Beginner's Boost!" );
		MESSAGEBOX->SetDescription( "Do you wish to boost to level 70?" );
...
ItemTimer::ItemTimer( EItemTimerType iType, EItemID iItemID, const char * pszName, const char * pszDescription, DWORD dwTimeLeft, DWORD dwTimeTotal, UI::Image * pcImage, UI::Image * pcImageBackground )
{
	this->iType				= iType;
