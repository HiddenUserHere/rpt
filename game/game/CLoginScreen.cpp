...
CLoginScreen::CLoginScreen()
{
	...
	iCurrentStateLogin = LOGINSTATE_None;
	iNextStateLogin = LOGINSTATE_None;
}
...
//	UnblockBMPBPT();
	...
	//Video Selector
	{
		...
		pCheckBoxVideoLow->SetSelected( TRUE );
	...
	//Server Selector
	{
		...
		pCheckBoxOfficialServer->SetSelected( TRUE );
		...
		EWorldID eWorldID = FileExist( std::string( AY_OBFUSCATE( "testserver.ini" ) ) ) ? WORLDID_Test : WORLDID_Fortress;

		if ( FileExist( std::string( AY_OBFUSCATE( "localserver.ini" ) ) ) )
		{
			eWorldID = WORLDID_TestLocal;
		}
...
void CLoginScreen::HandlePacket( SocketData * pcSocketData, PacketAccountLoginCode * psPacket )
{
    switch ( psPacket->iCode )
    {
        ...
        case ACCOUNTLOGIN_IncorrectPassword:
            GetWindow()->AddLoginMessage( "Account or Password is incorrect", 3000 );
            break;
...
{
    if ( psPacket->iVersion > READINT( 0x04AF7FFC ) )
    {
        if ( pcSocketData == SOCKETL )
            SOCKETGAME->CloseServerL();
        else
...
void CLoginScreen::OnCheckScreen( int iID, UIEventArgs e )
{
	if ( iOldCheckBox == iID )
	{
		pWindowLoginScreenSelector->GetElement<UI::CheckBox>( iID )->SetSelected( TRUE );
		return;
	}
...
//			DXVIDEORENDERER->SetRepeatBegin( 3.97 );
	...
	//Reset Checkbox
	pWindowLoginScreenSelector->GetElement<UI::CheckBox>( CHECKBOXID_ImageOnly )->SetSelected( iID == CHECKBOXID_ImageOnly ? TRUE : FALSE );
	pWindowLoginScreenSelector->GetElement<UI::CheckBox>( CHECKBOXID_VideoLow )->SetSelected( iID == CHECKBOXID_VideoLow ? TRUE : FALSE );
	pWindowLoginScreenSelector->GetElement<UI::CheckBox>( CHECKBOXID_VideoHigh )->SetSelected( iID == CHECKBOXID_VideoHigh ? TRUE : FALSE );
...
void CLoginScreen::OnCheckServer( int iID, UIEventArgs e )
{
	...
	{
		pWindowServerSelector->GetElement<UI::CheckBox>( iID )->SetSelected( TRUE );
		return;
	}
	...
	//Reset Checkbox
	pWindowServerSelector->GetElement<UI::CheckBox>( CHECKBOXID_OfficialServer )->SetSelected( iID == CHECKBOXID_OfficialServer ? TRUE : FALSE );
	pWindowServerSelector->GetElement<UI::CheckBox>( CHECKBOXID_TestServer )->SetSelected( iID == CHECKBOXID_TestServer ? TRUE : FALSE );
...
DWORD WINAPI CLoginScreen::ThreadLogin()
{
	...
	if ( SOCKETL )
	{
		GAMECORE->SendRequestInitialData( true );
...
DWORD __stdcall CLoginScreen::ThreadGame( int * piIndex )
{
	...
	if ( SOCKETG )
		GAMECORE->SendRequestInitialData( false );
	else
    {
        ((CLoginScreen *)GAMESCREEN->GetActiveScreen())->GetWindow()->AddLoginMessage( "Connecting failed", 3000 );
