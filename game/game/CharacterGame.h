...
#define TIME_NEXTSENDCHARACTERDATA				(*(DWORD*)0x00D0FC38)
...
private:
	...
	static CCharacterSaveHandler * pcCharacterSaveHandlerReceive;
	static CCharacterSaveHandler * pcCharacterSaveHandlerSend;
...
public:
	...
	void										SetDecreaseAutomation();
		
		
	static int									GetCurrentMP();
	static int									GetCurrentHP();
	static int									GetCurrentSP();
	
	static void									SetCurrentMP( int iMP );
	static void									SetCurrentHP( int iHP );
	static void									SetCurrentSP( int iSP );

	static void									ResetPotionGraph( int iID );

	static BOOL									ResetInventoryGold();
	...
	static BOOL									SendSavePacketData(PacketCharacterRecordData* psPacket, SocketData* pcSocketData);
	...
	static BOOL									ChangeModelSkin( UnitData * pcUnitData, BOOL bIsComicArmor, int iComicHeadNumber, const char * pszBodyModel, const char * pszHeadModel );

	static const char							* GetHeadModelSkin( UnitData * pcUnitData );

	void 									    SendCharacterData( UnitData * pcUnitData );

	static DWORD								GetCharacterDataChecksum( CharacterData * psCharacterData );
};
